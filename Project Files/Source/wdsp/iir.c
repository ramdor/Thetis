...
BQLP create_bqlp(int run, int size, double* in, double* out, double rate,
...
BQLP create_dbqlp(int run, int size, double* in, double* out, double rate,
    double fc, double Q, double gain, int nstages) {
    BQLP a = (BQLP)malloc0(sizeof(bqlp));
    a->run = run;
    a->size = size;
...
void xdbqlp(BQLP a) {
    ...
    if (a->run) {
        ...
        for (i = 0; i < a->size; i++) {
            ...
            for (n = 0; n < a->nstages; n++) {
                if (n > 0) a->x0[n] = a->y0[n - 1];
                a->y0[n] = a->a0 * a->x0[n] + a->a1 * a->x1[n]
                    + a->a2 * a->x2[n] + a->b1 * a->y1[n] + a->b2 * a->y2[n];
                a->y2[n] = a->y1[n];
                a->y1[n] = a->y0[n];
                a->x2[n] = a->x1[n];
                a->x1[n] = a->x0[n];
...
BQBP create_bqbp(int run, int size, double* in, double* out, double rate,
...
BQBP create_dbqbp(int run, int size, double* in, double* out, double rate,
    double f_low, double f_high, double gain, int nstages) {
    BQBP a = (BQBP)malloc0(sizeof(bqbp));
    a->run = run;
    a->size = size;
...
SPHP create_sphp(int run, int size, double* in, double* out, double rate,
    double fc, int nstages) {
            ...
            for (j = 0; j < 2; j++) {
                ...
                for (n = 0; n < a->nstages; n++) {
                    ...
                    a->y0[2 * n + j] = a->b0 * a->x0[2 * n + j]
                        + a->b1 * a->x1[2 * n + j] - a->a1 * a->y1[2 * n + j];
                    a->y1[2 * n + j] = a->y0[2 * n + j];
                    a->x1[2 * n + j] = a->x0[2 * n + j];
                }
                a->out[2 * i + j] = a->y0[2 * (a->nstages - 1) + j];
...
SPHP create_dsphp(int run, int size, double* in, double* out, double rate,
    double fc, int nstages) {
    SPHP a = (SPHP)malloc0(sizeof(sphp));
    a->run = run;
    a->size = size;
