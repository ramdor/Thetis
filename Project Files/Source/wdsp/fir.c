...
// impulse response of a zero frequency filter comprising a cascade of two
// resonators,
//    each followed by a detrending filter
double* zff_impulse(int nc, double scale) {
    // nc = number of coefficients (power of two)
    int n_resdet
        = nc / 2 - 1; // size of single zero-frequency resonator with detrender
    int n_dresdet = 2 * n_resdet - 1; // size of two cascaded units; when we
                                      // convolve these we get 2 * n - 1 length
