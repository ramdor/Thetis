...
void WDSPwisdom(char* directory)
{
	...
	if (!fftw_import_wisdom_from_filename(wisdom_file))
	{
		...
		while (psize <= MAX_WISDOM_SIZE_FILTER)
		{
			...
			tplan = fftw_plan_dft_1d(psize, (fftw_complex*)fftin, (fftw_complex*)fftout, FFTW_FORWARD, MY_PATIENCE);
			fftw_execute(tplan);
			...
			tplan = fftw_plan_dft_1d(psize, (fftw_complex*)fftin, (fftw_complex*)fftout, FFTW_BACKWARD, MY_PATIENCE);
			fftw_execute(tplan);
			fftw_destroy_plan(tplan);
			fprintf(stdout, "Planning COMPLEX BACKWARD FFT size %d\n", psize + 1);
			fflush(stdout);
			sprintf(status, "Planning COMPLEX BACKWARD FFT size %d\n", psize + 1);
			tplan = fftw_plan_dft_1d(psize + 1, (fftw_complex*)fftin, (fftw_complex*)fftout, FFTW_BACKWARD, MY_PATIENCE);
			fftw_execute(tplan);
			fftw_destroy_plan(tplan);
			psize *= 2;
		}
		...
		while (psize <= MAX_WISDOM_SIZE_DISPLAY)
		{
			...
			tplan = fftw_plan_dft_r2c_1d(psize, fftin, (fftw_complex*)fftout, MY_PATIENCE);
			fftw_execute(tplan);
			fftw_destroy_plan(tplan);
			psize *= 2;
		}
