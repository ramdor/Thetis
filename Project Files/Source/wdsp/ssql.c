...
FTOV create_ftov(int run, int size, int rate, int rsize, double fmax,
    ...
    a->inlast = 0.0;
    a->rcount = 0;
    a->div = a->fmax * 2.0 * a->rsize
        / a->rate; // fmax * 2 = zero-crossings/sec
...
void xftov(FTOV a) {
    // 'ftov' does frequency to voltage conversion looking only at zero
    // crossings of an
    ...
    if (a->run) {
        ...
        if ((a->inlast * a->in[0] < 0.0)
            && // different signs mean zero-crossing
            (fabs(a->inlast - a->in[0]) > a->eps)) {
            a->ring[a->rptr] = 1; // set the ring location to '1'
            a->rcount++; // increment the count
        }
        if (++a->rptr == a->rsize)
            a->rptr = 0; // increment and wrap the pointer as needed
        a->out[0] = min(
            1.0, (double)a->rcount / a->div); // calculate the output sample
        a->inlast
            = a->in[a->size - 1]; // save the last input sample for next buffer
...
void calc_ssql(SSQL a) {
    ...
    a->cvtr = create_ftov(1, a->size, a->rate, a->ftov_rsize, a->ftov_fmax,
        a->ibuff, a->ftovbuff);
    a->lpbuff = (double*)malloc0(a->size * sizeof(double));
    a->filt = create_dbqlp(
        1, a->size, a->ftovbuff, a->lpbuff, a->rate, 11.3, 1.0, 1.0, 1);
...
SSQL create_ssql(int run, int size, double* in, double* out, int rate,
    double tup, double tdown, double muted_gain, double tau_mute,
    double tau_unmute, double wthresh, double tr_thresh, int rsize,
    double fmax) {
    ...
    a->tr_thresh = tr_thresh; // value between tr_ss_unmute and tr_ss_mute,
                              // default = 0.8197
    a->tr_ss_mute = 1.0;
...
enum _ssqlstate { MUTED, INCREASE, UNMUTED, DECREASE };

void xssql(SSQL a) {
#if 0 // FIX ME. It's enabled by default, and we cannot control it in the GUI
      // yet
	if (a->run)
	{
...
#endif
    if (a->in != a->out) memcpy(a->out, a->in, a->size * sizeof(complex));
}
...
PORT void SetRXASSQLRun(int channel, int run) {
    EnterCriticalSection(&ch[channel].csDSP);
    rxa[channel].ssql.p->run = run;
    LeaveCriticalSection(&ch[channel].csDSP);
}

PORT void SetRXASSQLThreshold(int channel, double threshold) {
    // 'threshold' should be between 0.0 and 1.0
    // WU2O testing:  0.16 is a good default for 'threshold'; => 0.08 for
    // 'wthresh'
    EnterCriticalSection(&ch[channel].csDSP);
...
PORT void SetRXASSQLTauMute(int channel, double tau_mute) {
...
PORT void SetRXASSQLTauUnMute(int channel, double tau_unmute) {
