...
void calc_fmd(FMD a) {
    ...
    // detector limiter
    a->plim = create_wcpagc(1, // run - always ON
        5, // mode
...
void xfmd(FMD a) {
    if (a->run) {
        ...
        xsnotch(a->sntch);
        if (a->lim_run) {
            for (i = 0; i < 2 * a->size; i++) a->out[i] *= a->lim_pre_gain;
            xwcpagc(a->plim);
        }
    } else if (a->in != a->out)
...
PORT void SetRXAFMLimRun(int channel, int run) {
    FMD a;
    a = rxa[channel].fmd.p;
    EnterCriticalSection(&ch[channel].csDSP);
    if (a->lim_run != run) {
...
PORT void SetRXAFMLimGain(int channel, double gaindB) {
