...
void detector(int det_type, // detector type
    ...
    double fsclipL, double fsclipH, double det_offset) {
    int i, imin, ilim;
    int pix_count = 0;
    int rose, fell, next_pix_count, bcount, last_pix_count;
    double prev_maxi, mini, maxi, psum;
    if (pix_per_bin <= 1.0) {
        if (fsclipL == floor(fsclipL))
            imin = 0;
        else
            imin = 1;
        if (fsclipH == floor(fsclipH))
            ilim = m;
        else
            ilim = m - 1;
        switch (det_type) {
            ...
            case 2: // average - adjusted for window's equivalent noise
                    // bandwidth
                psum = 0.0;
                bcount = 0;
                for (i = imin; i < ilim; i++) {
            ...
            case 4: // rms
                ...
                for (i = imin; i < ilim; i++) {
                    ...
                    if (pix_count == last_pix_count) {
                        ...
                        pixels[last_pix_count]
                            = sqrt(psum / (double)bcount) * inv_enb;
                        psum = bins[i] * bins[i];
                        bcount = 1;
                    }
                    if (i == ilim - 1) {
    ...
    for (i = 0; i < a->num_pixout; i++) // for each output
    {
        ...
        if (k == i)
            ...
            detector(a->det_type[i], m, a->num_pixels, a->pix_per_bin,
                ...
                a->fsclipL, a->fsclipH, a->det_offset);
        else
            memcpy(
...
PORT void SetAnalyzer(int disp, // display identifier
    ...
    a->bin_per_pix = ((double)(a->num_stitch * (a->out_size - 1 - 2 * a->clip))
                         - 1.0 - a->fsclipL - a->fsclipH)
    ...
    if (a->av_mode[pixout] != mode) {
        ...
        switch (mode) {
            ...
            case 2:
                ...
                break;
            case 3:
                for (i = 0; i < dMAX_PIXELS; i++) a->av_sum[pixout][i] = -160.0;
                break;
            default:
                memset(
...
PORT double GetDisplayENB(int disp) {
    DP a = pdisp[disp];
    double enb;
    EnterCriticalSection(&a->SetAnalyzerSection);
    enb = 1.0 / a->inv_enb;
