...
PORT int SendStopToMetis() {
    ...
    destroy_pro(prop);
    prop = NULL; // MW0LGE_21g a change to ForceReset in setup.cs will cause
                 // calls to SendStopToMetis even if it potentially does not
                 // have a matching StartMetis. Consequently destroy_pro would
                 // fail. Destroy_pro does handle NULL state, so let us just set
                 // prop to null here
    return 0;
}
...
void MetisReadThreadMainLoop(void) {
    ...
    while (io_keep_running != 0) {
        ...
        if ((retVal == WSA_WAIT_FAILED) || (retVal == WSA_WAIT_TIMEOUT)) {
            ...
            destroy_pro(prop);
            prop = NULL; // this needed bacause of a change to ForceReset in
                         // setup.cs
            continue;
        } else {
...

                {
                    ...
                    for (frame = 0; frame < 2; frame++) {
                        ...
                        if ((bptr[0] == 0x7f) && (bptr[1] == 0x7f)
                                 ...
                                 isamp++) // for each set of samples
                            {
                                ...
                                if (mic_decimation_count
                                    ...
                                    prn->TxReadBufp[2 * mic_sample_count + 0]
                                        ...
                                        * (double)(bptr[k + 0] << 24
                                            | bptr[k + 1] << 16);
                                    prn->TxReadBufp[2 * mic_sample_count + 1]
                                        = 0.0;
                            ...
                            Inbound(
                                inid(1, 0), mic_sample_count, prn->TxReadBufp);
                        }
                    }
                }
            }
