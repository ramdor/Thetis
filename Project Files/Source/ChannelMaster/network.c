...
PORT int nativeInitMetis(
    ...
    while (initState > 0) {
        ...
        if (listenSock == (SOCKET)WSANOTINITIALISED) {
            ...
            if (initState
                == 2) // we alrady tried, and we are still not initialised
            {
            ...
            printf(
                "createSocket Error: socket failed %ld\n", WSAGetLastError());
            if (WSA_inited) WSACleanup(); // MW0LGE_22b
            return INVALID_SOCKET;
        } else {
            initState = 0;
            ...
            if (prn->wsaProcessEvents.lNetworkEvents & FD_READ) {
                ...
                switch (rc) {
                    case 1025:
                        ...
                        //          inactive Bit [4] - ADC4  Overload 1 =
                        //          active, 0 = inactive Bit [5] - ADC5 Overload
                        //          1 = active, 0 = inactive Bit [6] - ADC6
                        //          Overload 1 = active, 0 = inactive Bit [7] -
                        //          ADC7  Overload 1 = active, 0 = inactive
                        for (i = 0; i < MAX_ADC; i++)
...
void CmdHighPriority() { // port 1027
    ...
    // IO4---DLE
    // packetbuf[1400] = xvtr_enable | ((!(prn->user_dig_in & 0x01)) << 1 |
    // atu_tune << 2);
    packetbuf[1400] = xvtr_enable | (!audioamp_enable) << 1
        | atu_tune << 2; // MW0LGE_22b  // user_dig_in was gettin overritten by
                         // 1025 packet read
...
int sendOutbound(int id, double* out, OBB a) {
    ...
    {
        // note:  packet won't be sent until BOTH 'hsendIQSem' and 'hsendLRSem'
        // are released note:  'hobbuffsRun[0]' and 'hobbuffsRun[1]' are
        // released AFTER we write the packet and can therefore
        //        refill 'the buffers'outIQbufp' and 'outLRbufp'
        // note:  there are 63 I-Q samples and 63 L-R samples per P1 frame =>
        // 126 of each per packet
        if (pcm->xmtr[0].peer->run) // if eer/etr is running
        {
            if (id == 1) // TX I-Q data arriving
            {
                static int ptr = 0;
                memcpy(
                    (void*)(prn->outIQbufp + 720), out, sizeof(complex) * 126);
                // de-interleave the two sample streams
                for (int n = 0, i = ptr, j = 256 + ptr, k = 720; n < 63;
                     n++, i += 2, j += 2, k += 4) {
                ...
                ptr = (ptr + 126) % 252;
                if (ptr == 0) {
                    // release semaphore, indicating IQ data received and
                    // complete.
