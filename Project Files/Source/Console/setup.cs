...
#region Variable Declaration
    ...
    private bool m_bShown = false; // used by the selective restore system to
                                   // know accurate window state
#endregion
...
#region Constructor and Destructor
    ...
    public Setup(Console c) {
        ...
        if (Common.IsTimeOutEnabled)
            lblTimeout.Text = Common.DaysToTimeOut().ToString() + " days";
        //
        ...
        SetupDSPWarnings(
            false, false, false, false, false, false); // hide everything
        ...
        // MW0LGE_21h
        string sTip
            = "The feedback gain loop attempts to keep the sample buffer about ½ full."
            + System.Environment.NewLine
            + "This gives the maximum amount of margin before an overflow or underflow will occur."
            + System.Environment.NewLine
            + "If the gain is too high, the loop will respond too fast and strongly and we may "
            + System.Environment.NewLine
            + "end up with an over-full buffer (overflow) or empty buffer (underflow)."
            + System.Environment.NewLine
        ...
        sTip
            = "In the event of an underflow, the sample stream is slewed to zero, held there until there are more samples available,"
            + System.Environment.NewLine
            + "and then slewed back up to maximum output amplitude. In the event of an overflow,"
            + System.Environment.NewLine
            + "some samples are discarded and the ends of the before and after curves are blended along a raised-cosine trajectory."
            + System.Environment.NewLine
        ...
        // MW0LGE_21j
        sTip
            = "The minimum number of values that must be collected in the moving average filter (feedback loop) before"
            + System.Environment.NewLine
            + "its result is deemed stable enough to be of use. - NR0V";
        toolTip1.SetToolTip(pbVAC1PropFeedbackMinInfo, sTip);

        sTip
            = "The size/length of a moving average filter in the feedback loop. To reduce computation, it’s value must be a power of two."
            + System.Environment.NewLine
            + "Making this smaller will make the loop more responsive; however, you may need to reduce the feedback gain in that case."
            + System.Environment.NewLine
        ...
        sTip
            = "In controlling the sampling ratio, there is a feed-forward term which measures the number of samples being inserted into the resampler,"
            + System.Environment.NewLine
            + "the number of samples being withdrawn from the resampler, and essentially attempts to calculate the ratio of input/output sample rates."
            + System.Environment.NewLine
        ...
        sTip
            = "FF_Alpha is a smoothing parameter on the recursive filter. In summary, the ‘var’ ratio of the resampler is currently determined from two sources:"
            + System.Environment.NewLine
            + "(1) a feedback loop that attempts to keep the buffer half full, and "
            + System.Environment.NewLine
        ...
        // OC tab //MW0LGE_21j
        comboPin1TXActionHF.SelectedIndex = comboPin1TXActionVHF.SelectedIndex
            = comboPin1TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin2TXActionHF.SelectedIndex = comboPin2TXActionVHF.SelectedIndex
            = comboPin2TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin3TXActionHF.SelectedIndex = comboPin3TXActionVHF.SelectedIndex
            = comboPin3TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin4TXActionHF.SelectedIndex = comboPin4TXActionVHF.SelectedIndex
            = comboPin4TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin5TXActionHF.SelectedIndex = comboPin5TXActionVHF.SelectedIndex
            = comboPin5TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin6TXActionHF.SelectedIndex = comboPin6TXActionVHF.SelectedIndex
            = comboPin6TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;
        comboPin7TXActionHF.SelectedIndex = comboPin7TXActionVHF.SelectedIndex
            = comboPin7TXActionSWL.SelectedIndex
            = (int)TXPinActions.MOX_TUNE_TWOTONE;

        chkPin1TXPAHF.Checked = chkPin1TXPAVHF.Checked = chkPin1TXPASWL.Checked
            = false;
        chkPin2TXPAHF.Checked = chkPin2TXPAVHF.Checked = chkPin2TXPASWL.Checked
            = false;
        chkPin3TXPAHF.Checked = chkPin3TXPAVHF.Checked = chkPin3TXPASWL.Checked
            = false;
        chkPin4TXPAHF.Checked = chkPin4TXPAVHF.Checked = chkPin4TXPASWL.Checked
            = false;
        chkPin5TXPAHF.Checked = chkPin5TXPAVHF.Checked = chkPin5TXPASWL.Checked
            = false;
        chkPin6TXPAHF.Checked = chkPin6TXPAVHF.Checked = chkPin6TXPASWL.Checked
            = false;
        chkPin7TXPAHF.Checked = chkPin7TXPAVHF.Checked = chkPin7TXPASWL.Checked
            = false;
        ...
        initializing
            = true; // MW0LGE_21d stop the lg from notifying changed events
        ...
        /*/
        console.DeferUpdateDSP
            = true; // MW0LGE_21k9d need to set this again as it gets undone in
                    // load txprofile in ForceAllEvents (ForceTXProfile)
    ...
    private void OnRX2EnabledChanged(bool enabled) {
        ...
        clsContainerComboboxItem cci
            = (clsContainerComboboxItem)comboContainerSelect.SelectedItem;
        if (cci != null) {
            sId = cci.ID;
        }
...
#region Init Routines
    ...
    private void InitGeneralTab(List<string> recoveryList = null) {
        if (needsRecovering(recoveryList, "chkGeneralRXOnly"))
            chkGeneralRXOnly.Checked = console.RXOnly;
        if (needsRecovering(recoveryList, "chkGeneralDisablePTT"))
            chkGeneralDisablePTT.Checked = console.DisablePTT;
        if (needsRecovering(recoveryList, "chkWheelTunesOutsideSpectral"))
            chkWheelTunesOutsideSpectral.Checked
                = console.WheelTunesOutsideSpectral;
        if (needsRecovering(recoveryList, "chkAlsoUseSpecificMouseWheel"))
            chkAlsoUseSpecificMouseWheel.Checked
                = console.AlsoUseSpecificMouseWheel;
        if (needsRecovering(recoveryList, "chkZoomShiftModifier"))
            chkZoomShiftModifier.Checked = console.ZoomShiftModifier;
        if (needsRecovering(recoveryList, "chkExtended"))
            chkExtended.Checked = console.Extended;
    }

    private void InitADC(List<string> recoveryList = null) {
        if (needsRecovering(recoveryList, "radDDC0ADC0"))
            radDDC0ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radDDC0ADC1"))
            radDDC0ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radDDC0ADC2"))
            radDDC0ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC1ADC0"))
            radDDC1ADC0.Checked = false;
        if (needsRecovering(recoveryList, "radDDC1ADC1"))
            radDDC1ADC1.Checked = true;
        if (needsRecovering(recoveryList, "radDDC1ADC2"))
            radDDC1ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC2ADC0"))
            radDDC2ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radDDC2ADC1"))
            radDDC2ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radDDC2ADC2"))
            radDDC2ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC3ADC0"))
            radDDC3ADC0.Checked = false;
        if (needsRecovering(recoveryList, "radDDC3ADC1"))
            radDDC3ADC1.Checked = true;
        if (needsRecovering(recoveryList, "radDDC3ADC2"))
            radDDC3ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC4ADC0"))
            radDDC4ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radDDC4ADC1"))
            radDDC4ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radDDC4ADC2"))
            radDDC4ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC5ADC0"))
            radDDC5ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radDDC5ADC1"))
            radDDC5ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radDDC5ADC2"))
            radDDC5ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radDDC6ADC0"))
            radDDC6ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radDDC6ADC1"))
            radDDC6ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radDDC6ADC2"))
            radDDC6ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC0ADC0"))
            radP1DDC0ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC0ADC1"))
            radP1DDC0ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC0ADC2"))
            radP1DDC0ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC1ADC0"))
            radP1DDC1ADC0.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC1ADC1"))
            radP1DDC1ADC1.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC1ADC2"))
            radP1DDC1ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC2ADC0"))
            radP1DDC2ADC0.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC2ADC1"))
            radP1DDC2ADC1.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC2ADC2"))
            radP1DDC2ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC3ADC0"))
            radP1DDC3ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC3ADC1"))
            radP1DDC3ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC3ADC2"))
            radP1DDC3ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC4ADC0"))
            radP1DDC4ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC4ADC1"))
            radP1DDC4ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC4ADC2"))
            radP1DDC4ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC5ADC0"))
            radP1DDC5ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC5ADC1"))
            radP1DDC5ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC5ADC2"))
            radP1DDC5ADC2.Checked = false;

        if (needsRecovering(recoveryList, "radP1DDC6ADC0"))
            radP1DDC6ADC0.Checked = true;
        if (needsRecovering(recoveryList, "radP1DDC6ADC1"))
            radP1DDC6ADC1.Checked = false;
        if (needsRecovering(recoveryList, "radP1DDC6ADC2"))
    ...
    private void InitAdvancedAudioTab(List<string> recoveryList = null) {
        // MW0LGE_21j // mirrors btnVAC1AdvancedDefault and
        // btnVAC2AdvancedDefault
        if (needsRecovering(recoveryList, "udVAC1FeedbackGainIn"))
            udVAC1FeedbackGainIn.Value = (decimal)4.0e-6;
        if (needsRecovering(recoveryList, "udVAC1SlewTimeIn"))
            udVAC1SlewTimeIn.Value = (decimal)0.003;
        if (needsRecovering(recoveryList, "udVAC1PropMinIn"))
            udVAC1PropMinIn.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC1PropMaxIn"))
            udVAC1PropMaxIn.Value = 16384;
        if (needsRecovering(recoveryList, "udVAC1FFMinIn"))
            udVAC1FFMinIn.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC1FFMaxIn"))
            udVAC1FFMaxIn.Value = 262144;
        if (needsRecovering(recoveryList, "udVAC1FFAlphaIn"))
            udVAC1FFAlphaIn.Value = (decimal)0.01;
        if (needsRecovering(recoveryList, "chkVAC1OldVarIn"))
            chkVAC1OldVarIn.Checked = false;
        if (needsRecovering(recoveryList, "udVAC1FeedbackGainOut"))
            udVAC1FeedbackGainOut.Value = (decimal)4.0e-6;
        if (needsRecovering(recoveryList, "udVAC1SlewTimeOut"))
            udVAC1SlewTimeOut.Value = (decimal)0.003;
        if (needsRecovering(recoveryList, "udVAC1PropMinOut"))
            udVAC1PropMinOut.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC1PropMaxOut"))
            udVAC1PropMaxOut.Value = 16384;
        if (needsRecovering(recoveryList, "udVAC1FFMinOut"))
            udVAC1FFMinOut.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC1FFMaxOut"))
            udVAC1FFMaxOut.Value = 262144;
        if (needsRecovering(recoveryList, "udVAC1FFAlphaOut"))
            udVAC1FFAlphaOut.Value = (decimal)0.01;
        if (needsRecovering(recoveryList, "chkVAC1OldVarOut"))
            chkVAC1OldVarOut.Checked = false;
        //
        if (needsRecovering(recoveryList, "udVAC2FeedbackGainIn"))
            udVAC2FeedbackGainIn.Value = (decimal)4.0e-6;
        if (needsRecovering(recoveryList, "udVAC2SlewTimeIn"))
            udVAC2SlewTimeIn.Value = (decimal)0.003;
        if (needsRecovering(recoveryList, "udVAC2PropMinIn"))
            udVAC2PropMinIn.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC2PropMaxIn"))
            udVAC2PropMaxIn.Value = 16384;
        if (needsRecovering(recoveryList, "udVAC2FFMinIn"))
            udVAC2FFMinIn.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC2FFMaxIn"))
            udVAC2FFMaxIn.Value = 262144;
        if (needsRecovering(recoveryList, "udVAC2FFAlphaIn"))
            udVAC2FFAlphaIn.Value = (decimal)0.01;
        if (needsRecovering(recoveryList, "chkVAC2OldVarIn"))
            chkVAC2OldVarIn.Checked = false;
        if (needsRecovering(recoveryList, "udVAC2FeedbackGainOut"))
            udVAC2FeedbackGainOut.Value = (decimal)4.0e-6;
        if (needsRecovering(recoveryList, "udVAC2SlewTimeOut"))
            udVAC2SlewTimeOut.Value = (decimal)0.003;
        if (needsRecovering(recoveryList, "udVAC2PropMinOut"))
            udVAC2PropMinOut.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC2PropMaxOut"))
            udVAC2PropMaxOut.Value = 16384;
        if (needsRecovering(recoveryList, "udVAC2FFMinOut"))
            udVAC2FFMinOut.Value = 4096;
        if (needsRecovering(recoveryList, "udVAC2FFMaxOut"))
            udVAC2FFMaxOut.Value = 262144;
        if (needsRecovering(recoveryList, "udVAC2FFAlphaOut"))
            udVAC2FFAlphaOut.Value = (decimal)0.01;
        if (needsRecovering(recoveryList, "chkVAC2OldVarOut"))
            chkVAC2OldVarOut.Checked = false;
        //
    }

    private void InitDisplayTab(List<string> recoveryList = null) {
        if (needsRecovering(recoveryList, "udDisplayGridMax"))
            udDisplayGridMax.Value = Display.SpectrumGridMax;
        if (needsRecovering(recoveryList, "udDisplayGridMin"))
            udDisplayGridMin.Value = Display.SpectrumGridMin;
        if (needsRecovering(recoveryList, "udDisplayGridStep"))
            udDisplayGridStep.Value = Display.SpectrumGridStep;
        if (needsRecovering(recoveryList, "udRX2DisplayGridMax"))
            udRX2DisplayGridMax.Value = Display.RX2SpectrumGridMax;
        if (needsRecovering(recoveryList, "udRX2DisplayGridMin"))
            udRX2DisplayGridMin.Value = Display.RX2SpectrumGridMin;
        if (needsRecovering(recoveryList, "udRX2DisplayGridStep"))
            udRX2DisplayGridStep.Value = Display.RX2SpectrumGridStep;
        if (needsRecovering(recoveryList, "udDisplayFPS"))
            udDisplayFPS.Value = console.DisplayFPS;
        if (needsRecovering(recoveryList, "clrbtnWaterfallLow"))
            clrbtnWaterfallLow.Color = Display.WaterfallLowColor;
        if (needsRecovering(recoveryList, "clrbtnRX2WaterfallLow"))
            clrbtnRX2WaterfallLow.Color = Display.RX2WaterfallLowColor;
        if (needsRecovering(recoveryList, "udDisplayWaterfallLowLevel"))
            udDisplayWaterfallLowLevel.Value
                = (decimal)Display.WaterfallLowThreshold;
        if (needsRecovering(recoveryList, "udDisplayWaterfallHighLevel"))
            udDisplayWaterfallHighLevel.Value
                = (decimal)Display.WaterfallHighThreshold;
        if (needsRecovering(recoveryList, "udRX2DisplayWaterfallLowLevel"))
            udRX2DisplayWaterfallLowLevel.Value
                = (decimal)Display.RX2WaterfallLowThreshold;
        if (needsRecovering(recoveryList, "udRX2DisplayWaterfallHighLevel"))
            udRX2DisplayWaterfallHighLevel.Value
                = (decimal)Display.RX2WaterfallHighThreshold;
        if (needsRecovering(recoveryList, "udDisplayMeterDelay"))
            udDisplayMeterDelay.Value = console.MeterDelay;
        if (needsRecovering(recoveryList, "udDisplayPeakText"))
            udDisplayPeakText.Value = console.PeakTextDelay;
        if (needsRecovering(recoveryList, "udDisplayCPUMeter"))
            udDisplayCPUMeter.Value = console.CPUMeterDelay;
        if (needsRecovering(recoveryList, "udDisplayPhasePts"))
            udDisplayPhasePts.Value = Display.PhaseNumPts;
        if (needsRecovering(recoveryList, "udDisplayMultiPeakHoldTime"))
            udDisplayMultiPeakHoldTime.Value = console.MultimeterPeakHoldTime;
        if (needsRecovering(recoveryList, "udDisplayMultiTextHoldTime"))
            udDisplayMultiTextHoldTime.Value = console.MultimeterTextPeakTime;
        if (needsRecovering(recoveryList, "udTXGridMax"))
            udTXGridMax.Value = Display.TXSpectrumGridMax;
        if (needsRecovering(recoveryList, "udTXGridMin"))
            udTXGridMin.Value = Display.TXSpectrumGridMin;
        if (needsRecovering(recoveryList, "udTXGridStep"))
            udTXGridStep.Value = Display.TXSpectrumGridStep;
        if (needsRecovering(recoveryList, "udTXWFAmpMax"))
            udTXWFAmpMax.Value = Display.TXWFAmpMax;
        if (needsRecovering(recoveryList, "udTXWFAmpMin"))
        ...
        if (needsRecovering(recoveryList, "comboDisplayThreadPriority"))
...
#region Misc Routines
    ...
    private void RefreshSkinList() {
        ...
        if (!Directory.Exists(path)) {
            MessageBox.Show(
                ...
                Common.MB_TOPMOST); // MW0LGE_[2.9.0.7]);
            return;
        }
    ...
    private void getControlList(Control c, ref Dictionary<string, Control> a) {
        if (c.Controls.Count > 0) {
            foreach (Control c2 in c.Controls) getControlList(c2, ref a);
        }
    ...
    public new void Show() {// shadow method to always clear the list if someone
                            // somewhere shows the form

        // this may not be 100%, but best effort, we only want to do this if we
        // come from a 'hidden' state
        if (!m_bShown) clearUpdatedClickControlList();

    m_bShown = true;

    if (this.WindowState != FormWindowState.Normal)
        this.WindowState = FormWindowState.Normal; // MW0LGE_21i
...
chkContainerHighlight.Checked
    = false; // no point highlighting when setup closed
...
private bool inBothUpdateAndClick(string s) {
    return (
        m_lstUpdatedControls.Contains(s) && m_lstClickedControls.Contains(s));
}
...
private bool inClickOnly(string s) {
    return (
        !m_lstClickedControls.Contains(s) && m_lstClickedControls.Contains(s));
}
private void removeSpecialCases() {
    // this will remove all special cases from m_lstUpdatedControls if there
    // have been no associated click event for the control without a click we
    // know that we didn't do it through setup we cant just use this globally,
    // because setup changes can cause a number of other control in setup to
    // change which would not have the associated click
    ...
    foreach (string sControlName in toRemove) {
        if (inUpdateOnly(sControlName))
            m_lstUpdatedControls.Remove(sControlName);
    }
    ...
    foreach (string sControlName in toRemove) {
        if (m_lstUpdatedControls.Contains(sControlName))
            m_lstUpdatedControls.Remove(sControlName);
    }
}
...
private void registerAllControlsForStateMonitoring() {
    ...
    foreach (KeyValuePair<string, Control> kvp in controls) {
        ...
        if (c.GetType() == typeof(CheckBoxTS))
            ((CheckBoxTS)c).CheckedChanged
                += new System.EventHandler(checkBoxCheckedChangeHandler);
        else if (c.GetType() == typeof(ComboBoxTS))
            ((ComboBoxTS)c).SelectedIndexChanged
                += new System.EventHandler(comboboxSelectedIndexChangeHandler);
        else if (c.GetType() == typeof(NumericUpDownTS))
            ((NumericUpDownTS)c).ValueChanged
                += new System.EventHandler(numericUDValueChangeHandler);
        else if (c.GetType() == typeof(RadioButtonTS))
            ((RadioButtonTS)c).CheckedChanged
                += new System.EventHandler(radioButtonCheckedChangeHandler);
        else if (c.GetType() == typeof(TextBoxTS))
            ((TextBoxTS)c).TextChanged
                += new System.EventHandler(textBoxTextChangeHandler);
        else if (c.GetType() == typeof(TrackBarTS))
            ((TrackBarTS)c).ValueChanged
                += new System.EventHandler(trackBarValueChangeHandler);
        else if (c.GetType() == typeof(ColorButton))
            ((ColorButton)c).Changed
                += new System.EventHandler(colourButtonChangeHandler);
        else if (c.GetType() == typeof(ucLGPicker))
            ((ucLGPicker)c).Changed
                += new System.EventHandler(lgPickerChangeHandler);
    }
}
//-
...
public void SaveOptions() {
    ...
    // add this manually because soundfile string not stored in any control in
    // setup
...
private void InitTransmitTab(List<string> recoveryList = null) {
    if (needsRecovering(recoveryList, "comboTXTUNMeter"))
        comboTXTUNMeter.Text
            = "Fwd SWR"; // this needs to be setup in the case of new database
                         // and we havent hit tx/tune yet // MW0LGE_21a
}
...
private void handleOutdatedOptions(
    bool bGet, ref Dictionary<string, string> getDict) {
    if (bGet) {
        _oldSettings.Clear();
...
private void getOptions(List<string> recoveryList = null) {
    ...
    // MW0LGE moved to dictionary, as control names are unique, and does away
    // with old loop through every control to find the one we want
    Dictionary<string, Control> controls = new Dictionary<string, Control>();
    ...
    if (sortedList.Contains("comboPAProfile"))
        sortedList.Remove(
    ...
    if (_PAProfiles != null && a.ContainsKey("PAProfileCount")) {
        ...
        for (int n = 0; n < nProfiles; n++) {
            ...
            if (a.ContainsKey(sDataKey)) {
                ...
                if (_PAProfiles.ContainsKey(p.ProfileName))
                    _PAProfiles.Remove(
                        p.ProfileName); // scrap any dupes, this will be true
                                        // for defaults for example
    ...
    // MW0LGE We have overwritten controls with data that might not match the
    // current profile load in current profile
...
private bool checkTXProfileChanged2(
    DataRow drToCheck = null, bool bOnlyVac = false) {
    // bOnlyVac will only consider settings related to VAC

    DataRow dr;
    if (drToCheck == null) {
        // check everything in the TX profile
        DataRow[] rows = DB.ds.Tables["TxProfile"].Select("Name = '"
            + current_profile.Replace("'", "''")
            + "'"); // MW0LGE_21k9rc6 replace ' for ''
    ...
    if (!bOnlyVac) {
        if (DB.ConvertFromDBVal<int>(dr["FilterLow"])
            != (int)udTXFilterLow.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["FilterHigh"])
            != (int)udTXFilterHigh.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["TXEQNumBands"])
            != console.EQForm.NumBands)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["TXEQEnabled"])
            != console.EQForm.TXEQEnabled)
            return true;
        int[] eq = console.EQForm.TXEQ;
        if (DB.ConvertFromDBVal<int>(dr["TXEQPreamp"]) != eq[0]) return true;
        for (int i = 1; i < 11; i++)
            if (DB.ConvertFromDBVal<int>(dr["TXEQ" + i.ToString()]) != eq[i])
                return true;
        for (int i = 11; i < 21; i++)
            if (DB.ConvertFromDBVal<int>(dr["TxEqFreq" + (i - 10).ToString()])
                != eq[i])
                return true;

        if (DB.ConvertFromDBVal<bool>(dr["DXOn"]) != console.DX) return true;
        // if (DB.ConvertFromDBVal<int>(dr["DXLevel"]) != console.DXLevel)
        // return true;  //MW0LGE_22b
        if (DB.ConvertFromDBVal<bool>(dr["CompanderOn"]) != console.CPDR)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["CompanderLevel"]) != console.CPDRLevel)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["MicGain"]) != console.Mic) return true;
        if (DB.ConvertFromDBVal<int>(dr["FMMicGain"]) != console.FMMic)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["MicMute"]) != console.MicMute)
            return true;

        if (DB.ConvertFromDBVal<bool>(dr["Lev_On"])
            != chkDSPLevelerEnabled.Checked)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_Slope"])
            != (int)udDSPLevelerSlope.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_MaxGain"])
            != (int)udDSPLevelerThreshold.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_Attack"])
            != (int)udDSPLevelerAttack.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_Decay"])
            != (int)udDSPLevelerDecay.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_Hang"])
            != (int)udDSPLevelerHangTime.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Lev_HangThreshold"])
            != tbDSPLevelerHangThreshold.Value)
            return true;

        if (DB.ConvertFromDBVal<int>(dr["ALC_Slope"])
            != (int)udDSPALCSlope.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["ALC_MaximumGain"])
            != (int)udDSPALCMaximumGain.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["ALC_Attack"])
            != (int)udDSPALCAttack.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["ALC_Decay"])
            != (int)udDSPALCDecay.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["ALC_Hang"])
            != (int)udDSPALCHangTime.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["ALC_HangThreshold"])
            != tbDSPALCHangThreshold.Value)
            return true;

        // if (DB.ConvertFromDBVal<int>(dr["Power"]) != console.PWR) return
        // true;  //MW0LGE_21k9rc5 removed from tx profile as power is per band
        // now

        if (DB.ConvertFromDBVal<bool>(dr["VOX_On"]) != chkVOXEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Dexp_On"]) != chkDEXPEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_Threshold"])
            != (int)udDEXPThreshold.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_Attack"])
            != (int)udDEXPAttack.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["VOX_HangTime"])
            != (int)udDEXPHold.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_Release"])
            != (int)udDEXPRelease.Value)
            return true;
        if (DB.ConvertFromDBVal<decimal>(dr["Dexp_Attenuate"])
            != udDEXPExpansionRatio.Value)
            return true;
        if (DB.ConvertFromDBVal<decimal>(dr["Dexp_Hysterisis"])
            != udDEXPHysteresisRatio.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_Tau"]) != (int)udDEXPDetTau.Value)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Dexp_SCF_On"])
            != chkSCFEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_SCF_Low"])
            != (int)udSCFLowCut.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_SCF_High"])
            != (int)udSCFHighCut.Value)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Dexp_LookAhead_On"])
            != chkDEXPLookAheadEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["Dexp_LookAhead"])
            != (int)udDEXPLookAhead.Value)
            return true;

        if (DB.ConvertFromDBVal<int>(dr["Tune_Power"])
            != (int)udTXTunePower.Value)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Tune_Meter_Type"])
            != (string)comboTXTUNMeter.Text)
            return true;

        if (DB.ConvertFromDBVal<int>(dr["TX_AF_Level"]) != console.TXAF)
            return true;

        if (DB.ConvertFromDBVal<int>(dr["AM_Carrier_Level"])
            != (int)udTXAMCarrierLevel.Value)
            return true;

        if (DB.ConvertFromDBVal<bool>(dr["Show_TX_Filter"])
            != (bool)console.ShowTXFilter)
            return true;
    }

    if (DB.ConvertFromDBVal<bool>(dr["VAC1_On"])
        != (bool)chkAudioEnableVAC.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Auto_On"])
        != (bool)chkAudioVACAutoEnable.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_RX_GAIN"])
        != (int)udAudioVACGainRX.Value)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_TX_GAIN"])
        != (int)udAudioVACGainTX.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Stereo_On"])
        != (bool)chkAudio2Stereo.Checked)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC1_Sample_Rate"])
        != (string)comboAudioSampleRate2.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC1_Buffer_Size"])
        != (string)comboAudioBuffer2.Text)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_IQ_Output"])
        != (bool)chkAudioIQtoVAC.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_IQ_Correct"])
        != (bool)chkAudioCorrectIQ.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_PTT_OverRide"])
        != (bool)chkVACAllowBypass.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Combine_Input_Channels"])
        != (bool)chkVACCombine.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Latency_On"])
        != (bool)chkAudioLatencyManual2.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_Latency_Duration"])
        != (int)udAudioLatency2.Value)
        return true;

    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Latency_RBOut_On"])
        != (bool)chkAudioLatencyManual2_Out.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_Latency_RBOut_Duration"])
        != (int)udAudioLatency2_Out.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Latency_PAIn_On"])
        != (bool)chkAudioLatencyPAInManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_Latency_PAIn_Duration"])
        != (int)udAudioLatencyPAIn.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC1_Latency_PAOut_On"])
        != (bool)chkAudioLatencyPAOutManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC1_Latency_PAOut_Duration"])
        != (int)udAudioLatencyPAOut.Value)
        return true;

    if (DB.ConvertFromDBVal<string>(dr["VAC1_AudioDriver"])
        != (string)comboAudioDriver2.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC1_AudioInput"])
        != (string)comboAudioInput2.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC1_AudioOutput"])
        != (string)comboAudioOutput2.Text)
        return true;

    if (DB.ConvertFromDBVal<bool>(dr["VAC2_On"]) != (bool)chkVAC2Enable.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Auto_On"])
        != (bool)chkVAC2AutoEnable.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_RX_GAIN"]) != (int)udVAC2GainRX.Value)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_TX_GAIN"]) != (int)udVAC2GainTX.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Stereo_On"])
        != (bool)chkAudioStereo3.Checked)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC2_Sample_Rate"])
        != (string)comboAudioSampleRate3.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC2_Buffer_Size"])
        != (string)comboAudioBuffer3.Text)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_IQ_Output"])
        != (bool)chkVAC2DirectIQ.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_IQ_Correct"])
        != (bool)chkVAC2DirectIQCal.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Combine_Input_Channels"])
        != (bool)chkVAC2Combine.Checked)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Latency_On"])
        != (bool)chkVAC2LatencyManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_Latency_Duration"])
        != (int)udVAC2Latency.Value)
        return true;

    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Latency_RBOut_On"])
        != (bool)chkVAC2LatencyOutManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_Latency_RBOut_Duration"])
        != (int)udVAC2LatencyOut.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Latency_PAIn_On"])
        != (bool)chkVAC2LatencyPAInManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_Latency_PAIn_Duration"])
        != (int)udVAC2LatencyPAIn.Value)
        return true;
    if (DB.ConvertFromDBVal<bool>(dr["VAC2_Latency_PAOut_On"])
        != (bool)chkVAC2LatencyPAOutManual.Checked)
        return true;
    if (DB.ConvertFromDBVal<int>(dr["VAC2_Latency_PAOut_Duration"])
        != (int)udVAC2LatencyPAOut.Value)
        return true;

    if (DB.ConvertFromDBVal<string>(dr["VAC2_AudioDriver"])
        != (string)comboAudioDriver3.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC2_AudioInput"])
        != (string)comboAudioInput3.Text)
        return true;
    if (DB.ConvertFromDBVal<string>(dr["VAC2_AudioOutput"])
        != (string)comboAudioOutput3.Text)
        return true;

    if (!bOnlyVac) {
        if (DB.ConvertFromDBVal<string>(dr["Phone_RX_DSP_Buffer"])
            != (string)comboDSPPhoneRXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Phone_TX_DSP_Buffer"])
            != (string)comboDSPPhoneTXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_RX_DSP_Buffer"])
            != (string)comboDSPFMRXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_TX_DSP_Buffer"])
            != (string)comboDSPFMTXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_RX_DSP_Buffer"])
            != (string)comboDSPDigRXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_TX_DSP_Buffer"])
            != (string)comboDSPDigTXBuf.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["CW_RX_DSP_Buffer"])
            != (string)comboDSPCWRXBuf.Text)
            return true;

        if (DB.ConvertFromDBVal<string>(dr["Phone_RX_DSP_Filter_Size"])
            != (string)comboDSPPhoneRXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Phone_TX_DSP_Filter_Size"])
            != (string)comboDSPPhoneTXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_RX_DSP_Filter_Size"])
            != (string)comboDSPFMRXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_TX_DSP_Filter_Size"])
            != (string)comboDSPFMTXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_RX_DSP_Filter_Size"])
            != (string)comboDSPDigRXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_TX_DSP_Filter_Size"])
            != (string)comboDSPDigTXFiltSize.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["CW_RX_DSP_Filter_Size"])
            != (string)comboDSPCWRXFiltSize.Text)
            return true;

        if (DB.ConvertFromDBVal<string>(dr["Phone_RX_DSP_Filter_Type"])
            != (string)comboDSPPhoneRXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Phone_TX_DSP_Filter_Type"])
            != (string)comboDSPPhoneTXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_RX_DSP_Filter_Type"])
            != (string)comboDSPFMRXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["FM_TX_DSP_Filter_Type"])
            != (string)comboDSPFMTXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_RX_DSP_Filter_Type"])
            != (string)comboDSPDigRXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["Digi_TX_DSP_Filter_Type"])
            != (string)comboDSPDigTXFiltType.Text)
            return true;
        if (DB.ConvertFromDBVal<string>(dr["CW_RX_DSP_Filter_Type"])
            != (string)comboDSPCWRXFiltType.Text)
            return true;

        if (DB.ConvertFromDBVal<bool>(dr["Mic_Input_On"])
            != (bool)radMicIn.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Mic_Input_Boost"])
            != (bool)chk20dbMicBoost.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Line_Input_On"])
            != (bool)radLineIn.Checked)
            return true;
        if (DB.ConvertFromDBVal<decimal>(dr["Line_Input_Level"])
            != udLineInBoost.Value)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["CESSB_On"]) != chkDSPCESSB.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["Pure_Signal_Enabled"])
            != console.PureSignalEnabled)
            return true;

        // CFC
        if (DB.ConvertFromDBVal<bool>(dr["CFCEnabled"]) != chkCFCEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["CFCPostEqEnabled"])
            != chkCFCPeqEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<bool>(dr["CFCPhaseRotatorEnabled"])
            != chkPHROTEnable.Checked)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["CFCPhaseRotatorFreq"])
            != (int)udPhRotFreq.Value)
            return true;
        if (DB.ConvertFromDBVal<int>(dr["CFCPhaseRotatorStages"])
            != (int)udPHROTStages.Value)
            return true;
        int[] cfceq = CFCCOMPEQ;
        if (DB.ConvertFromDBVal<int>(dr["CFCPreComp"]) != cfceq[0]) return true;
        for (int i = 1; i < 11; i++)
            if (DB.ConvertFromDBVal<int>(dr["CFCPreComp" + (i - 1).ToString()])
                != cfceq[i])
                return true;
        if (DB.ConvertFromDBVal<int>(dr["CFCPostEqGain"]) != cfceq[11])
            return true;
        for (int i = 12; i < 22; i++)
            if (DB.ConvertFromDBVal<int>(
                    dr["CFCPostEqGain" + (i - 12).ToString()])
                != cfceq[i])
                return true;
        for (int i = 22; i < 32; i++)
            if (DB.ConvertFromDBVal<int>(dr["CFCEqFreq" + (i - 22).ToString()])
                != cfceq[i])
                return true;
    }
...
private void highlightTXProfileSaveItems(bool bHighlight) {
    ...
    // Common.HightlightControl(udTXTunePower, bHighlight);   // removed for now
    // as although these values are stored per TX profile
    // Common.HightlightControl(comboTXTUNMeter, bHighlight); // they are not
    // recovered from the profile. MW0LGE_21h
...
private void udpateTXProfileInDB(DataRow dr) {
    ...
    for (int i = 11; i < 21; i++) dr["TxEqFreq" + (i - 10).ToString()] = eq[i];
...
public void UpdateWaterfallBandInfo() {
    if (!initializing) {
        // MW0LGE_21e
        txtWaterFallBandLevel.Text
            = BandStackManager.BandToString(console.RX1Band);
        txtRX2WaterFallBandLevel.Text
            = BandStackManager.BandToString(console.RX2Band);

        // switch (console.RX1Band)
        //{
        //     case Band.B160M: txtWaterFallBandLevel.Text = "160 meters";
        //     break; case Band.B80M: txtWaterFallBandLevel.Text = "80 meters";
        //     break; case Band.B60M: txtWaterFallBandLevel.Text = "60 meters";
        //     break; case Band.B40M: txtWaterFallBandLevel.Text = "40 meters";
        //     break; case Band.B30M: txtWaterFallBandLevel.Text = "30 meters";
        //     break; case Band.B20M: txtWaterFallBandLevel.Text = "20 meters";
        //     break; case Band.B17M: txtWaterFallBandLevel.Text = "17 meters";
        //     break; case Band.B15M: txtWaterFallBandLevel.Text = "15 meters";
        //     break; case Band.B12M: txtWaterFallBandLevel.Text = "12 meters";
        //     break; case Band.B10M: txtWaterFallBandLevel.Text = "10 meters";
        //     break; case Band.B6M: txtWaterFallBandLevel.Text = "6 meters";
        //     break; case Band.WWV: txtWaterFallBandLevel.Text = "WWV"; break;
        //     case Band.BLMF: txtWaterFallBandLevel.Text = "LW/MW bands";
        //     break; case Band.B120M: txtWaterFallBandLevel.Text = "120
        //     meters"; break; case Band.B90M: txtWaterFallBandLevel.Text = "90
        //     meters"; break; case Band.B61M: txtWaterFallBandLevel.Text = "60
        //     meters"; break; case Band.B49M: txtWaterFallBandLevel.Text = "49
        //     meters"; break; case Band.B41M: txtWaterFallBandLevel.Text = "41
        //     meters"; break; case Band.B31M: txtWaterFallBandLevel.Text = "31
        //     meters"; break; case Band.B25M: txtWaterFallBandLevel.Text = "25
        //     meters"; break; case Band.B22M: txtWaterFallBandLevel.Text = "22
        //     meters"; break; case Band.B19M: txtWaterFallBandLevel.Text = "19
        //     meters"; break; case Band.B16M: txtWaterFallBandLevel.Text = "16
        //     meters"; break; case Band.B14M: txtWaterFallBandLevel.Text = "14
        //     meters"; break; case Band.B13M: txtWaterFallBandLevel.Text = "13
        //     meters"; break; case Band.B11M: txtWaterFallBandLevel.Text = "11
        //     meters"; break;
        ...
        //{
        //    case Band.B160M: txtRX2WaterFallBandLevel.Text = "160 meters";
        //    break; case Band.B80M: txtRX2WaterFallBandLevel.Text = "80
        //    meters"; break; case Band.B60M: txtRX2WaterFallBandLevel.Text =
        //    "60 meters"; break; case Band.B40M: txtRX2WaterFallBandLevel.Text
        //    = "40 meters"; break; case Band.B30M:
        //    txtRX2WaterFallBandLevel.Text = "30 meters"; break; case
        //    Band.B20M: txtRX2WaterFallBandLevel.Text = "20 meters"; break;
        //    case Band.B17M: txtRX2WaterFallBandLevel.Text = "17 meters";
        //    break; case Band.B15M: txtRX2WaterFallBandLevel.Text = "15
        //    meters"; break; case Band.B12M: txtRX2WaterFallBandLevel.Text =
        //    "12 meters"; break; case Band.B10M: txtRX2WaterFallBandLevel.Text
        //    = "10 meters"; break; case Band.B6M: txtRX2WaterFallBandLevel.Text
        //    = "6 meters"; break; case Band.WWV: txtRX2WaterFallBandLevel.Text
        //    = "WWV"; break; case Band.GEN: txtRX2WaterFallBandLevel.Text =
        //    "General"; break; default: txtRX2WaterFallBandLevel.Text = "2M &
        //    VHF+"; break;
        //}
    }
}
...
public void UpdateDisplayGridBandInfo() {
    if (!initializing) {
        // MW0LGE_21e
        txtDisplayGridBandLevel.Text
            = BandStackManager.BandToString(console.RX1Band);
        txtRX2DisplayGridBandLevel.Text
            = BandStackManager.BandToString(console.RX2Band);

        // MW0LGE_21e
        // switch (console.RX1Band)
        //{
        //     case Band.B160M: txtDisplayGridBandLevel.Text = "160 meters";
        //     break; case Band.B80M: txtDisplayGridBandLevel.Text = "80
        //     meters"; break; case Band.B60M: txtDisplayGridBandLevel.Text =
        //     "60 meters"; break; case Band.B40M: txtDisplayGridBandLevel.Text
        //     = "40 meters"; break; case Band.B30M:
        //     txtDisplayGridBandLevel.Text = "30 meters"; break; case
        //     Band.B20M: txtDisplayGridBandLevel.Text = "20 meters"; break;
        //     case Band.B17M: txtDisplayGridBandLevel.Text = "17 meters";
        //     break; case Band.B15M: txtDisplayGridBandLevel.Text = "15
        //     meters"; break; case Band.B12M: txtDisplayGridBandLevel.Text =
        //     "12 meters"; break; case Band.B10M: txtDisplayGridBandLevel.Text
        //     = "10 meters"; break; case Band.B6M: txtDisplayGridBandLevel.Text
        //     = "6 meters"; break; case Band.WWV: txtDisplayGridBandLevel.Text
        //     = "WWV"; break;
        //     // MW0LGE all this block added
        //     case Band.BLMF: txtDisplayGridBandLevel.Text = "LW/MW bands";
        //     break; case Band.B120M: txtDisplayGridBandLevel.Text = "120
        //     meters"; break; case Band.B90M: txtDisplayGridBandLevel.Text =
        //     "90 meters"; break; case Band.B61M: txtDisplayGridBandLevel.Text
        //     = "60 meters"; break; case Band.B49M:
        //     txtDisplayGridBandLevel.Text = "49 meters"; break; case
        //     Band.B41M: txtDisplayGridBandLevel.Text = "41 meters"; break;
        //     case Band.B31M: txtDisplayGridBandLevel.Text = "31 meters";
        //     break; case Band.B25M: txtDisplayGridBandLevel.Text = "25
        //     meters"; break; case Band.B22M: txtDisplayGridBandLevel.Text =
        //     "22 meters"; break; case Band.B19M: txtDisplayGridBandLevel.Text
        //     = "19 meters"; break; case Band.B16M:
        //     txtDisplayGridBandLevel.Text = "16 meters"; break; case
        //     Band.B14M: txtDisplayGridBandLevel.Text = "14 meters"; break;
        //     case Band.B13M: txtDisplayGridBandLevel.Text = "13 meters";
        //     break; case Band.B11M: txtDisplayGridBandLevel.Text = "11
        //     meters"; break;
        //     // - end lge
        ...
        // switch (console.RX2Band)
        //{
        //     case Band.B160M: txtRX2DisplayGridBandLevel.Text = "160 meters";
        //     break; case Band.B80M: txtRX2DisplayGridBandLevel.Text = "80
        //     meters"; break; case Band.B60M: txtRX2DisplayGridBandLevel.Text =
        //     "60 meters"; break; case Band.B40M:
        //     txtRX2DisplayGridBandLevel.Text = "40 meters"; break; case
        //     Band.B30M: txtRX2DisplayGridBandLevel.Text = "30 meters"; break;
        //     case Band.B20M: txtRX2DisplayGridBandLevel.Text = "20 meters";
        //     break; case Band.B17M: txtRX2DisplayGridBandLevel.Text = "17
        //     meters"; break; case Band.B15M: txtRX2DisplayGridBandLevel.Text =
        //     "15 meters"; break; case Band.B12M:
        //     txtRX2DisplayGridBandLevel.Text = "12 meters"; break; case
        //     Band.B10M: txtRX2DisplayGridBandLevel.Text = "10 meters"; break;
        //     case Band.B6M: txtRX2DisplayGridBandLevel.Text = "6 meters";
        //     break; case Band.WWV: txtRX2DisplayGridBandLevel.Text = "WWV";
        //     break; case Band.GEN: txtRX2DisplayGridBandLevel.Text =
        //     "General"; break; default: txtRX2DisplayGridBandLevel.Text = "2M
        //     & VHF+"; break;
        // }
    }
}
...
public int HermesAttenuatorDataRX2 {
    get {
        if (udHermesStepAttenuatorDataRX2 != null)
            return (int)udHermesStepAttenuatorDataRX2.Value;
        else
            return -1;
    }
    set {
...
public int ATTOnTX {
    ...
    set {
        if (udATTOnTX != null) {
            ...
            if (value < 0)
                value = 0; // MW0LGE [2.9.0.7] added after mi0bot source review
...
public int AGCRX2HangThreshold {
    get { return tbDSPAGCRX2HangThreshold.Value; }
    set {
...
public bool CustomRXAGCEnabled {
    set {
        ...
        if (value) {
            console.radio.GetDSPRX(0, 0).Force = true;
            console.radio.GetDSPRX(0, 1).Force = true;
            udDSPAGCHangTime_ValueChanged(
                this, EventArgs.Empty); // MW0LGE_21f moved above the decay
...
public bool CustomRX2AGCEnabled {
    set {
        ...
        if (value) {
            ...
            udDSPAGCRX2HangTime_ValueChanged(
                this, EventArgs.Empty); // MW0LGE_21f moved above the decay
            udDSPAGCRX2Decay_ValueChanged(this, EventArgs.Empty);
...
public double HPSDRFreqCorrectFactorViaAutoCalibration {
    ...
    set {
        ...
        // NetworkIO.FreqCorrectionFactor = (double)value; //MW0LGE_219krc6 now
        // forced even if the same value, to clear the _bCalibrating flag;
...
public int[] CFCCOMPEQ {
    ...
    set {
        ...
        tbCFCPEQGAIN.Value = Math.Max(
            tbCFCPEQGAIN.Minimum, Math.Min(tbCFCPEQGAIN.Maximum, value[11]));
        tbCFCEQ0.Value
            = Math.Max(tbCFCEQ0.Minimum, Math.Min(tbCFCEQ0.Maximum, value[12]));
        tbCFCEQ1.Value
            = Math.Max(tbCFCEQ1.Minimum, Math.Min(tbCFCEQ1.Maximum, value[13]));
        tbCFCEQ2.Value
            = Math.Max(tbCFCEQ2.Minimum, Math.Min(tbCFCEQ2.Maximum, value[14]));
        tbCFCEQ3.Value
            = Math.Max(tbCFCEQ3.Minimum, Math.Min(tbCFCEQ3.Maximum, value[15]));
        tbCFCEQ4.Value
            = Math.Max(tbCFCEQ4.Minimum, Math.Min(tbCFCEQ4.Maximum, value[16]));
        tbCFCEQ5.Value
            = Math.Max(tbCFCEQ5.Minimum, Math.Min(tbCFCEQ5.Maximum, value[17]));
        tbCFCEQ6.Value
            = Math.Max(tbCFCEQ6.Minimum, Math.Min(tbCFCEQ6.Maximum, value[18]));
        tbCFCEQ7.Value
            = Math.Max(tbCFCEQ7.Minimum, Math.Min(tbCFCEQ7.Maximum, value[19]));
        tbCFCEQ8.Value
            = Math.Max(tbCFCEQ8.Minimum, Math.Min(tbCFCEQ8.Maximum, value[20]));
        tbCFCEQ9.Value
            = Math.Max(tbCFCEQ9.Minimum, Math.Min(tbCFCEQ9.Maximum, value[21]));
        udCFC0.Value
...
private void InitHPSDR() {
    ...
    if (console.path_Illustrator != null) console.path_Illustrator.pi_Changed();
}
...
private void btnGeneralCalLevelStart_Click(object sender, System.EventArgs e) {
    DialogResult dr = MessageBox.Show(
        ...
        "Level Calibration Check", MessageBoxButtons.YesNo,
        MessageBoxIcon.Question);
    if (dr == DialogResult.No)
        return; // MW0LGE_[2.9.0.6] double check we want to do this, prevents
                // accidental click from changing config
    ...
    Thread t = new Thread(
        new ThreadStart(CalibrateLevel)) { Name = "Level Calibration Thread",
        IsBackground = true, Priority = ThreadPriority.AboveNormal };
    t.Start();
...
private void btnCalLevel_Click(object sender, System.EventArgs e) {
    ...
    Thread t = new Thread(
        new ThreadStart(CalibrateRX2Level)) { Name = "Level Calibration Thread",
...
private void comboOptFilterWidthMode_SelectedIndexChanged(
    ...
    switch (comboOptFilterWidthMode.Text) {
        case "Linear":
            ...
            break;
        case "Log": console.CurrentFilterWidthMode = FilterWidthMode.Log; break;
        case "Log10":
...
public void ForceAudioReset() {
    // pavel-demin_21a    [nothing would be forced if sample rates the same, so
    // pavel used init flag to bypass the check in both the
...
// access the check box for Andromeda to control auto-show of the gain setting
// form
    ...
    if (new_rate != old_rate || initializing || m_bForceAudio) {
        switch (NetworkIO.CurrentRadioProtocol) {
            case RadioProtocol.ETH:
                ...
                // wait for all inflight packets to arrive - need to experiment
                // with this delay value
                Thread.Sleep(20);
                ...
                //    (2) SetXcmInrate() is called by Audio.SampleRate1 which is
                //    called by console.SampleRate1
            ...
            case RadioProtocol.USB:
                ...
                // flush any buffers where these old-rate packets may be stored
                // ... this is done via operations in SetXcmInrate() in the
                // property SampleRate1:  SetXcmInrate() is called by
...
private void comboAudioSampleRateRX2_SelectedIndexChanged(
    ...
    // MW0LGE [2.9.07] always initialise rx2 even if P1. Thanks to Reid
    // (Gi8TME/Mi0BOT) and DH1KLM see
    // https://github.com/ramdor/Thetis-2.9.0/issues/66
    // if ((NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH) && (new_rate !=
    // old_rate || initializing || m_bForceAudio))
    if (new_rate != old_rate || initializing || m_bForceAudio) {
        // turn OFF the DSP channel so it gets flushed out (must do while data
        // is flowing to get slew-down and flush)
        ...
        if (console.PowerOn && console.VAC2Enabled && !initializing)
            Audio.EnableVAC2(false);
        // turn OFF the DDC for RX2; had to add rx2_enabled as a parameter to
        // UpdateReceivers() to do this --- THIS IS S HACK.  BETTER WAY?
        console.UpdateDDCs(false);
        // wait for all inflight packets to arrive
        Thread.Sleep(20); // need to experiment with this time

        // flush any buffers where these old-rate packets may be stored ... this
        // is done via operations in SetXcmInrate()
        ...
        // turn the DDC for RX2 back ON; had to add rx2_enabled as a parameter
        // to UpdateReceivers() to do this
        if (was_enabled) {
        ...
        // calculate and display the new bin_width
        double bin_width
            = (double)new_rate / (double)console.specRX.GetSpecRX(1).FFTSize;
...
private void udDisplayFPS_ValueChanged(object sender, System.EventArgs e) {
    ...
    // MW0LGE_21g ///*if(console.CurrentDisplayEngine ==
    // DisplayEngine.DIRECT_X)*/
...
private void udDisplayGridMax_ValueChanged(object sender, System.EventArgs e) {
    ...
    console.WaterfallUseRX1SpectrumMinMax
        = chkWaterfallUseRX1SpectrumMinMax
              .Checked; // MW0LGE_21d this will force an update
...
private void comboDisplayLabelAlign_SelectedIndexChanged(
    ...
    switch (comboDisplayLabelAlign.Text) {
        ...
        case "Off":
            ...
            break;
        default: Display.DisplayLabelAlign = DisplayLabelAlignment.LEFT; break;
    }
}
...
private void udDisplayScopeTime_ValueChanged(
    ...
    int samples
        = (int)((double)udDisplayScopeTime.Value * Audio.OutRate / 1000000.0);
...
// private void comboDisplayDriver_SelectedIndexChanged(object sender,
// System.EventArgs e)
//{
...
private void chkDSPLevelerEnabled_CheckedChanged(
    ...
    //
    console.SetupInfoBarButton(
        ucInfoBar.ActionTypes.Leveler, chkDSPLevelerEnabled.Checked);
}
...
//    //
//    console.SetupInfoBar(ucInfoBar.ActionTypes.TuneDrive,
//    chkTXTunePower.Checked);
//}
...
private bool loadTXProfile(String sProfileName) {
    //
    // NOTE: make sure you update checkTXProfileChanged2, if anything is
    // added/removed
    //
    //

    DataRow[] rows = DB.ds.Tables["TxProfile"].Select("Name = '"
        + sProfileName.Replace("'", "''")
        + "'"); // MW0LGE_21k9rc6 replace ' for ''
    ...
    // udTXTunePower.Value = (int)dr["Tune_Power"];  //MW0LGE not applied here
    // anymore, leave in save incase we want to use it comboTXTUNMeter.Text =
    // (string)dr["Tune_Meter_Type"];  //MW0LGE
    ...
    if (chkRestoreVAC1DeviceDetailsFromTXProfile.Checked) {
        comboAudioDriver2.Text
            = DB.ConvertFromDBVal<string>(dr["VAC1_AudioDriver"]);
        comboAudioInput2.Text
            = DB.ConvertFromDBVal<string>(dr["VAC1_AudioInput"]);
        comboAudioOutput2.Text
            = DB.ConvertFromDBVal<string>(dr["VAC1_AudioOutput"]);
    }
    ...
    if (chkRestoreVAC2DeviceDetailsFromTXProfile.Checked) {
        comboAudioDriver3.Text
            = DB.ConvertFromDBVal<string>(dr["VAC2_AudioDriver"]);
        comboAudioInput3.Text
            = DB.ConvertFromDBVal<string>(dr["VAC2_AudioInput"]);
        comboAudioOutput3.Text
            = DB.ConvertFromDBVal<string>(dr["VAC2_AudioOutput"]);
    }
    ...
    chkAudioEnableVAC.Checked
        = (bool)dr["VAC1_On"]; // moved here after setting to off MW0LGE_21k9d
    chkVAC2Enable.Checked
        = (bool)dr["VAC2_On"]; // moved here after setting to off MW0LGE_21k9d
...
public int TCPIPcatClientsConnectedChange {
    set {
        grpTCPIPcatServer.Text
            = "TCP / IP CAT Server (" + value.ToString() + " clients)";
    }
}
...
private void comboTXProfileName_SelectedIndexChanged(
    ...
    if (chkAutoSaveTXProfile.Checked) {
        ...
        if (comboTXProfileName.Focused
            && checkTXProfileChanged2() /*CheckTXProfileChanged()*/) // MW0LGE_21k8
                                                                     // swap the
                                                                     // order so
                                                                     // focus is
                                                                     // needed
...
private void btnTXProfileDelete_Click(object sender, System.EventArgs e) {
    ...
    DataRow[] rows = DB.ds.Tables["TxProfile"].Select("Name = '"
        + comboTXProfileName.Text.Replace("'", "''")
        + "'"); // MW0LGE_21k9rc6 replace ' for ''
...
private void clrbtnDataLine_Changed(object sender, System.EventArgs e) {
    ...
    Display.DataLineColor = Color.FromArgb(
        tbDataLineAlpha.Value, clrbtnDataLine.Color); // MW0LGE_21b
    rebuildLGBrushes();
}
...
public void initCATandPTTprops() {
    ...
    console.CAT2Parity
        = SDRSerialPort.StringToParity((string)comboCAT2parity.SelectedItem);
    ...
    console.CAT3Parity
        = SDRSerialPort.StringToParity((string)comboCAT3parity.SelectedItem);
    ...
    console.CAT4Parity
        = SDRSerialPort.StringToParity((string)comboCAT4parity.SelectedItem);
...
public void copyAriesCATPropsToDialogVars() {
    ...
    if (comboAriesCATPort.Items.Contains(port)) comboAriesCATPort.Text = port;
}
...
private void chkCATPTTEnabled_CheckedChanged(
    ...
    if (chkCATPTTEnabled
            .Checked) // if it's enabled don't allow changing settings on port
    {
        if (comboCATPTTPort.Text.StartsWith("CAT"))
...
private void comboCATPTTPort_SelectedIndexChanged(
    ...
    if (comboCATPTTPort.Text.StartsWith("COM"))
        console.CATPTTBitBangPort
            = Int32.Parse(comboCATPTTPort.Text.Substring(3));
    if (!comboCATPTTPort.Focused) chkCATPTTEnabled_CheckedChanged(sender, e);
}
...
private async void chkTestIMD_CheckedChanged(
    ...
    if (chkTestIMD.Checked) {
        ...
        // MW0LGE_21a change to delay Freq2 output. Fixes problems with some
        // Amps frequency counters
        ...
        int new_pwr = console.SetPowerUsingTargetDBM(
            out bool bUseConstrain, out double targetdBm, true, true, true);
        //
        if (console.TwoToneDrivePowerOrigin == DrivePowerSource.FIXED) {
...
private void btnCancel_Click(object sender, System.EventArgs e) {
    ...
    m_objSaveLoadThread = new Thread(
        new ThreadStart(PreRestoreOptions)) { Name = "Get Options Thread",
        IsBackground = true, Priority = ThreadPriority.Lowest };
    m_objSaveLoadThread.Start();
    this.Hide();
}
...
private bool CompleteImport() {
    //-W2PA Import more carefully, allowing DBs created by previous versions to
    // retain settings and options MW0LGE_[2.9.0.7] changed structure slightly
    bool success = DB.ImportAndMergeDatabase(
...
// klj. Allow user to see precisley what database they are using:
internal string DatabasePath() {
    return lblDataBase.Text;
}
internal void setDBPath(string dbPath) {
    lblDataBase.Text = dbPath;
}
...
private void txtKB_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e) {
    ...
    if (e.KeyCode == Keys.LWin || e.KeyCode == Keys.RWin) windows_key = true;
...
private void txtKB_KeyUp(object sender, System.Windows.Forms.KeyEventArgs e) {
    ...
    if (e.KeyCode == Keys.LWin || e.KeyCode == Keys.RWin) windows_key = false;
...
private void clrbtnSubRXFilter_Changed(object sender, System.EventArgs e) {
    Display.SubRXFilterColor
        = Color.FromArgb(tbMultiRXFilterAlpha.Value, clrbtnSubRXFilter.Color);
}
...
private void udRX2DisplayWaterfallAvgTime_ValueChanged(
    ...
    // (int)((float)udRX2DisplayWaterfallAvgTime.Value * 0.001 / buffer_time);
    // buffersToAvg = Math.Max(2, buffersToAvg);
    ...
    if (e.Control == true && e.Alt == true) {
        switch (e.KeyCode) {
            case Keys.A:
                ...
                // grpPAGainByBand.Visible = true; //MW0LGE_22b need to switch
                // profile here
...
private void chkDisplayPanFill_CheckedChanged(
    ...
    //
    console.SetupInfoBarButton(
        ucInfoBar.ActionTypes.DisplayFill, Display.PanFill);
}
...
private void comboAppSkin_SelectedIndexChanged(object sender, EventArgs e) {
    if (initializing)
        return; // prevents call on getoptions, need to force call after options
                // loaded MW0LGE
...
private void saveFileDialog1_FileOk(object sender, CancelEventArgs e) {
    // DB.ds.WriteXml(saveFileDialog1.FileName,
    // XmlWriteMode.WriteSchema);//MW0LGE_[2.9.0.7]
...
private void chkAlexPresent_CheckedChanged(object sender, System.EventArgs e) {
    ...
    if (chkAlexPresent.Checked) {
        ...
        //   if (console.RX2PreampPresent) console.comboRX2Preamp.Visible =
        //   true;
    }
    ...
    // if (chkHermesStepAttenuator.Checked) chkHermesStepAttenuator.Checked =
    // true;
...
private void chkPenOCrcv160_CheckedChanged(object sender, System.EventArgs e) {
    ...
    console.PennyExtCtrlEnabled
        = chkPennyExtCtrl.Checked; // need side effect of this to push change to
                                   // native code
}
...
private void InitAlexAntTables() {
    ...
    AlexRxOnlyAntCheckBoxes
        = new[] { new CheckBoxTS[] { chkAlex160R1, chkAlex160R2, chkAlex160XV },
...
private void ProcessAlexAntCheckBox(object sender, Band band) {
    ...
    // find a name for the "winning" check box. In some radios the signals add,
    // so multiple selections would be bad.
    if (cboxes[2].Checked)
        Name = RXAntChk3Name;
...
private void tpGeneralHardware_Paint(
    ...
    // MW0LGE_21d
    if (NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH) {
        lblProtocolInfo.Text = "Protocol 2 (v"
            + NetworkIO.ProtocolSupported.ToString("0\\.0") + ")";
        lblMetisCodeVersion.Text = NetworkIO.FWCodeVersion.ToString("0\\.0")
            + "." + NetworkIO.BetaVersion.ToString();
    } else {
...
public string GetFirmwareCodeVersionString() {
    ...
    if (NetworkIO.getHaveSync() == 1) {
        if (NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH) {
            sRet = "FW v" + NetworkIO.FWCodeVersion.ToString("0\\.0") + "."
                + NetworkIO.BetaVersion.ToString() + " Protocol 2";
        } else {
            sRet = "FW v" + NetworkIO.FWCodeVersion.ToString("0\\.0")
                + " Protocol 1";
        }
...
private void udHPSDRFreqCorrectFactor_ValueChanged(
    object sender, System.EventArgs e) {
    if (_freqCorrectFactorChangedViaAutoCalibration
        || !chkUsing10MHzRef.Checked) {
        ...
        NetworkIO.FreqCorrectionFactor
            = (double)udHPSDRFreqCorrectFactor10MHz.Value;
    }
}
    ...
    object sender, EventArgs e) // w5wc
{
    ...
    if (!console.initializing) {
        setButtonState(
            true, false); // MW0LGE this causes an update of the whole DB, so
                          // disable buttons while happening
...
private void clrbtnTXHGridColor_Changed(object sender, EventArgs e) {
    Display.TXHGridColor
        = Color.FromArgb(tbTXHGridColorAlpha.Value, clrbtnTXHGridColor.Color);
}
...
private void chkHermesStepAttenuator_CheckedChanged(
    ...
    if (chk != null) // only if we click it //MW0LGE [2.9.0.6]
    {
        ...
        console.GetDDC(
            out rx1, out rx2, out sync1, out sync2, out psrx, out pstx);
...
private void udHermesStepAttenuatorDataRX2_ValueChanged(
    object sender, EventArgs e) {
    ...
    if (AlexPresent && console.CurrentHPSDRModel != HPSDRModel.ANAN10
        ...
        udHermesStepAttenuatorDataRX2.Maximum = (decimal)31;
}
...
private void btnConfigure_Click(object sender, EventArgs e) {
    if (ConfigMidi2Cat == null) {
        ...
        ConfigMidi2Cat
            = new Midi2Cat.Midi2CatSetupForm(console.Midi2Cat.Midi2CatDbFile);
        ConfigMidi2Cat.Show();
...
private void tbDSPDetLimGain_Scroll(object sender, EventArgs e) {
    console.radio.GetDSPRX(0, 0).RXFMDETLIMGAIN
        = (double)tbDSPFMDetLimGain.Value;
    console.radio.GetDSPRX(0, 1).RXFMDETLIMGAIN
        = (double)tbDSPFMDetLimGain.Value;
    console.radio.GetDSPRX(1, 0).RXFMDETLIMGAIN
        = (double)tbDSPFMDetLimGain.Value;
}
...
private void chkClickTuneDrag_CheckedChanged(object sender, EventArgs e) {
    ...
    chkCTLimitDragMouseOnly.Enabled
        = chkClickTuneDrag.Checked && chkCTLimitDragToSpectral.Checked;
...
private void tbRX1APFBW_Scroll(object sender, EventArgs e) {
    ...
    if (radDSPRX1APFControls.Checked) console.APFBandwidth = tbRX1APFBW.Value;
}
...
private void tbRX2APFBW_Scroll(object sender, EventArgs e) {
    ...
    if (radDSPRX2APFControls.Checked) console.APFBandwidth = tbRX2APFBW.Value;
}
...
private void btnMNFEnter_Click(object sender, EventArgs e) {
    ...
    SaveNotchesToDatabase(); // aligns notch db with what has actually happened
                             // MW0LGE
}
...
unsafe private void btnMNFCancel_Click(object sender, EventArgs e) {
    ...
    if (AddActive) {
        AddActive = false;
        if (numnotches > 0) {
            // Add had potentially increased these values ... put them back the
            // way they were
            udMNFNotch.Value -= 1;
            udMNFNotch.Maximum -= 1;
        }
    ...
    if (numnotches > 0) {
        ...
        int active;
        WDSP.RXANBPGetNotch(
            WDSP.id(0, 0), (int)udMNFNotch.Value, &fcenter, &fwidth, &active);
...
unsafe public void UpdateNotchDisplay() {
    ...
    if (numnotches > 0) {
        ...
        // MW0LGE added because udMNFFreq could be set to some value even though
        // no MNotchDB entries this caused by generic form save, saving out the
        // control value
        udMNFFreq.Value = 0;
        udMNFWidth.Value = 0;
        btnMNFDelete.Enabled = false;
...
unsafe public void RestoreNotchesFromDatabase() {
    ...
    for (int i = 0; i < MNotchDB.Count; i++) {
        ...
        // HERE:  READ VALUES OF fcenter, fwidth, and active FOR NOTCH[i] FROM
        // THE DATABASE
...
private void chkCFCPeqEnable_CheckedChanged(object sender, EventArgs e) {
    ...
    //
    console.SetupInfoBarButton(
        ucInfoBar.ActionTypes.CFCeq, chkCFCPeqEnable.Checked);
}
...
private void timer_VAC_Monitor_Tick(object sender, EventArgs e) {
    ...
    if (!lblVAC1ovfl.Visible && !lblVAC2ovfl.Visible
        && !lblAdvancedAudioWarning.Visible) {
    ...
    unsafe {
        ivac.getIVACdiags(
            0, 0, &underflows, &overflows, &var, &ringsize, &nring);
    }
    ...
    if (Audio.VAC1ControlFlagOut) txtVAC1OldVarOut.Text = var.ToString("F6");

    unsafe {
        ivac.getIVACdiags(
            0, 1, &underflows, &overflows, &var, &ringsize, &nring);
    }
    ...
    if (Audio.VAC1ControlFlagIn) txtVAC1OldVarIn.Text = var.ToString("F6");

    unsafe {
        ivac.getIVACdiags(
            1, 0, &underflows, &overflows, &var, &ringsize, &nring);
    }
    ...
    unsafe {
        ivac.getIVACdiags(
            1, 1, &underflows, &overflows, &var, &ringsize, &nring);
    }
...
private void TbDataFillAlpha_Scroll(object sender, EventArgs e) {
    ...
    if (!initializing)
        lgLinearGradientRX1.ApplyGlobalAlpha(
            tbDataFillAlpha.Value); // also adjust LG alpha values, always do
                                    // this even if disabled MW0LGE_21a
}
...
private void ComboGanymedeCATPort_SelectedIndexChanged(
    ...
    if (comboGanymedeCATPort.Text == "None") {
        if (chkEnableGanymede.Checked) {
            if (comboGanymedeCATPort.Focused) chkEnableGanymede.Checked = false;
        }
...
private void ChkEnableAries_CheckedChanged(object sender, EventArgs e) {
    ...
    if (chkEnableAries.Checked && console.PTTBitBangEnabled
        ...
        MessageBox.Show("Aries CAT port cannot be the same as Bit Bang Port",
            "Port Selection Error", MessageBoxButtons.OK, MessageBoxIcon.Error,
            MessageBoxDefaultButton.Button1,
            Common.MB_TOPMOST); // MW0LGE_[2.9.0.7]
        chkEnableAries.Checked = false;
...
private void checkEnableAriesQuickTune_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkQSOTimerPlaySoundOnExpiry_CheckedChanged(
    object sender, EventArgs e) {
    bool bEnabled
        = chkQSOTimerEnabled.Checked && chkQSOTimerPlaySoundOnExpiry.Checked;
...
private void setQSOTimerDuration() {
    int nDurationSecs
        = ((int)udQSOTimerMinutes.Value * 60) + (int)udQSOTimerSeconds.Value;
    if (nDurationSecs < 5) {
        nDurationSecs = 5;
...
private void comboRadioModel_SelectedIndexChanged(object sender, EventArgs e) {
    ...
    switch (comboRadioModel.Text) {
        ...
        case "ANAN-100B":
            ...
            chkAutoPACalibrate.Checked = false;
            chkAutoPACalibrate.Visible = false;
            // grpANAN100BPAGainByBand.BringToFront(); //MW0LGE_22b new PA
            // system
            labelRXAntControl.Text = " EXT2  EXT1  XVTR";
            RXAntChk1Name = "EXT2";
            RXAntChk2Name = "EXT1";
        ...
        case "ANAN-7000DLE":
            ...
            chkAutoPACalibrate.Visible = false;
            chkBypassANANPASettings.Visible = true;
            // grpANAN7000DPAGainByBand.BringToFront(); //MW0LGE_22b new PA
            // system
        ...
        case "ANAN-8000DLE":
            ...
            chkAutoPACalibrate.Visible = false;
            chkBypassANANPASettings.Visible = true;
            // grpANAN8000DPAGainByBand.BringToFront(); //MW0LGE_22b new PA
            // system
            labelRXAntControl.Text = "  BYPS  EXT1  XVTR";
            RXAntChk1Name = "BYPS";
            RXAntChk2Name = "EXT1";
    ...
    if (old_model != console.CurrentHPSDRModel) {
        ...
        if (!initializing)
            updatePAProfileCombo("Default - "
                + console.CurrentHPSDRModel.ToString()); // MW0LGE_22b
    }
...
private void setupADCRadioButtions() {
    ...
    switch (console.CurrentHPSDRModel) {
        ...
        case HPSDRModel.ANAN100B: bADC0 = true; break;
        default:
            bADC0 = true;
            bADC1 = true;
            break;
    ...
    foreach (Control c in cc.Controls) {
        if (c.GetType() == typeof(RadioButtonTS)) {
            ...
            if (rb.Name.EndsWith("ADC0")) {
                rb.Visible = bADC0;
                // check if ADC1 and ADC2 are false, if so then this will be an
                // 10,10E,100 or 100B, so select this ADC
...
private void chkIncludeOtherSampleRates_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkActivePeakHoldRX1_CheckedChanged(object sender, EventArgs e) {
    ...
    //
    console.SetupInfoBarButton(ucInfoBar.ActionTypes.ActivePeaks,
        chkActivePeakHoldRX1.Checked
            | (console.RX2Enabled && chkActivePeakHoldRX2.Checked));
}

private void udActivePeakHoldDurationRX1_ValueChanged(
    object sender, EventArgs e) {
    Display.SpectralPeakHoldDelayRX1
        = (double)udActivePeakHoldDurationRX1.Value;
}
...
private void udActivePeakHoldDurationRX2_ValueChanged(
    object sender, EventArgs e) {
    Display.SpectralPeakHoldDelayRX2
        = (double)udActivePeakHoldDurationRX2.Value;
}

private void clrbtnActiveSpectralPeak_Changed(object sender, EventArgs e) {
    Display.DataPeaksFillColor = Color.FromArgb(
        tbActiveSpectralPeakAlpha.Value, clrbtnActiveSpectralPeak.Color);
}
...
private void tbActiveSpectralPeakAlpha_Scroll(object sender, EventArgs e) {
    ...
    toolTip1.SetToolTip(
        tbActiveSpectralPeakAlpha, tbActiveSpectralPeakAlpha.Value.ToString());
}
...
private void clrbtnGripperColour_Changed(object sender, EventArgs e) {
    if (initializing) return;
    lgLinearGradientRX1.ColourForSelectedGripper
        = clrbtnGripperColour.Color; // c;
...
private void chkPanadpatorGradient_CheckedChanged(object sender, EventArgs e) {
    ...
    clrbtnDataLine.Enabled = !chkPanadpatorGradient.Checked
        || !(chkPanadpatorGradient.Checked && chkDataLineGradient.Checked);

    Display.UseLinearGradient = chkPanadpatorGradient.Checked;
    Display.UseLinearGradientForDataLine = chkDataLineGradient.Checked;
}

private void chkSpecWarningLEDRenderDelay_CheckedChanged(
    object sender, EventArgs e) {
    Display.ShowFrameRateIssue = chkSpecWarningLEDRenderDelay.Checked;
}

private void chkSpecWarningLEDGetPixels_CheckedChanged(
    object sender, EventArgs e) {
...
private void tmrPLLLockChecker_Tick(object sender, EventArgs e) {
    if (NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH) {
        lblPLLLock.Text
            = NetworkIO.GetPLLLock() ? "PLL Locked" : "PLL Not Locked";
    } else {
        lblPLLLock.Text = "";
    }
}
...
private void stopStartN1MMSpectrum() {
    if (!N1MM.IsStarted
        && (chkN1MMEnableRX1.Checked || chkN1MMEnableRX2.Checked)) {
        N1MM.Start();
    } else if (N1MM.IsStarted
        && (!chkN1MMEnableRX1.Checked && !chkN1MMEnableRX2.Checked)) {
        N1MM.Stop();
    }
}
...
private void tbRX1WaterfallOpacity_Scroll(object sender, EventArgs e) {
    ...
    toolTip1.SetToolTip(tbRX1WaterfallOpacity,
        tbRX1WaterfallOpacity.Value.ToString() + "% opacity");
}
...
private void tbRX2WaterfallOpacity_Scroll(object sender, EventArgs e) {
    ...
    toolTip1.SetToolTip(tbRX2WaterfallOpacity,
        tbRX2WaterfallOpacity.Value.ToString() + "% opacity");
}
...
public void UpdateDDCTab() {
    ...
    if (NetworkIO.CurrentRadioProtocol == RadioProtocol.ETH) // P2
    {
        ...
        lblP1assignment.Font
            = new Font(lblP1assignment.Font, FontStyle.Regular);
    ...
    } else if (NetworkIO.CurrentRadioProtocol == RadioProtocol.USB) // P1
    {
        ...
        lblP2assignment.Font
            = new Font(lblP2assignment.Font, FontStyle.Regular);
    ...
    //{
    //     lblP1assignment.ForeColor = SystemColors.ControlText;
    //     lblP1assignment.Font = new Font(lblP1assignment.Font,
    //     FontStyle.Regular);
    ...
    //        case HPSDRHW.OrionMKII: // AMAM-7000DLE 7000DLEMkII ANAN-8000DLE
    //        OrionMkII
    //            switch (tot)
    //            {
    //                case 0: // off off off
    ...
    //{
    //    lblP2assignment.ForeColor = SystemColors.ControlText;
    //    lblP2assignment.Font = new Font(lblP2assignment.Font,
    //    FontStyle.Regular);
    ...
    //
    bool bUpdate = rx1 != -1 || rx2 != -1 || sync1 != -1 || sync2 != -1
        || psrx != -1 || pstx != -1;
    //
    ...
    if (bUpdate) {
        foreach (Control c in tpADC.Controls) {
            ...
            if (l != null) {
                if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(rx1.ToString())) {
                    l.Text = "RX1";
                    l.BackColor = SystemColors.ControlDark;
                } else if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(rx2.ToString())) {
                    l.Text = "RX2";
                    l.BackColor = SystemColors.ControlDark;
                } else if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(sync1.ToString())) {
                    l.Text = "RX1 Sync1";
                    l.BackColor = SystemColors.ControlDark;
                } else if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(sync2.ToString())) {
                    l.Text = "RX1 Sync2";
                    l.BackColor = SystemColors.ControlDark;
                } else if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(psrx.ToString())) {
                    l.Text = "PS-rx";
                    l.BackColor = SystemColors.ControlDark;
                } else if (l.Name.StartsWith("lblRxDDC")
                    && l.Name.EndsWith(pstx.ToString())) {
                    l.Text = "PS-tx";
                    l.BackColor = SystemColors.ControlDark;
                } else {
...
private void chkHighlightTXProfileSaveItems_CheckedChanged(
    object sender, EventArgs e) {
...
private void pbVAC1FeedbackGainInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(toolTip1.GetToolTip(pbVAC1FeedbackGainInfo),
        pbVAC1FeedbackGainInfo, 10 * 1000);
}

private void pbVAC1SlewTimeInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(
        toolTip1.GetToolTip(pbVAC1SlewTimeInfo), pbVAC1SlewTimeInfo, 10 * 1000);
}
// end resampler advanced settings

private void chkNetworkThrottleIndexTweak_CheckedChanged(
    object sender, EventArgs e) {
    if (initializing) return;

    bool bNetworkThrottleIndex
        = NetworkThrottle.GetNetworkThrottle(out int throttle, false);

    bool bOK = false;
    if (bNetworkThrottleIndex) {
        if (chkNetworkThrottleIndexTweak.Checked) {
            if (throttle != unchecked((int)0xffffffffu)) {
                bOK = NetworkThrottle.SetNetworkThrottle(
    ...
    if (bOK) {
        MessageBox.Show(
            "NetworkThrottleIndex has been changed. You need to restart/reboot for this to take effect.",
            "Reboot", MessageBoxButtons.OK, MessageBoxIcon.Information,
            MessageBoxDefaultButton.Button1, Common.MB_TOPMOST);
    }
    updateNetworkThrottleCheckBox();
}
private void updateNetworkThrottleCheckBox() {
    ...
    if (bNetworkThrottleIndex) {
        chkNetworkThrottleIndexTweak.Checked
            = throttle == unchecked((int)0xffffffffu);
    }
}

private void tbBandstackOverlayAlpha_Scroll(object sender, EventArgs e) {
    toolTip1.SetToolTip(
        tbBandstackOverlayAlpha, tbBandstackOverlayAlpha.Value.ToString());
    clrbtnBandstackOverlay_Changed(this, EventArgs.Empty);
}

private void clrbtnBandstackOverlay_Changed(object sender, EventArgs e) {
    Display.BandstackOverlayColor = Color.FromArgb(
        tbBandstackOverlayAlpha.Value, clrbtnBandstackOverlay.Color);
}

private void udVAC1GrapherSwing_ValueChanged(object sender, EventArgs e) {
    ucVAC1VARGrapherOut.PlusMinusSwing = ucVAC1VARGrapherIn.PlusMinusSwing
        = (double)udVAC1GrapherSwing.Value;
}
...
private void chkVAC1GrapherAuto_CheckedChanged(object sender, EventArgs e) {
    ...
    ucVAC1VARGrapherOut.AutoSwing = ucVAC1VARGrapherIn.AutoSwing
        = chkVAC1GrapherAuto.Checked;
}
...
private void chkVAC2GrapherAuto_CheckedChanged(object sender, EventArgs e) {
    ...
    ucVAC2VARGrapherOut.AutoSwing = ucVAC2VARGrapherIn.AutoSwing
        = chkVAC2GrapherAuto.Checked;
}

private void udVAC2GrapherSwing_ValueChanged(object sender, EventArgs e) {
    ucVAC2VARGrapherOut.PlusMinusSwing = ucVAC2VARGrapherIn.PlusMinusSwing
        = (double)udVAC2GrapherSwing.Value;
}
...
private void pbVAC1PropFeedbackMinInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(toolTip1.GetToolTip(pbVAC1PropFeedbackMinInfo),
        pbVAC1PropFeedbackMinInfo, 10 * 1000);
}

private void pbVAC1PropFeedbackMaxInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(toolTip1.GetToolTip(pbVAC1PropFeedbackMaxInfo),
        pbVAC1PropFeedbackMaxInfo, 10 * 1000);
}

private void pbVAC1FFMinInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(
        toolTip1.GetToolTip(pbVAC1FFMinInfo), pbVAC1FFMinInfo, 10 * 1000);
}

private void pbVAC1FFMaxInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(
        toolTip1.GetToolTip(pbVAC1FFMaxInfo), pbVAC1FFMaxInfo, 10 * 1000);
}

private void pbVAC1FFAlphaInfo_Click(object sender, EventArgs e) {
    toolTip1.Show(
        toolTip1.GetToolTip(pbVAC1FFAlphaInfo), pbVAC1FFAlphaInfo, 10 * 1000);
}

private void comboPin1TXActionHF_SelectedIndexChanged(
    object sender, EventArgs e) {
    if (sender is ComboBoxTS) {
        ComboBoxTS combo = sender as ComboBoxTS;
        bool bOk = int.TryParse(combo.Name.Substring(8, 1), out int pin);
        if (bOk)
            Penny.getPenny().setTXPinAction(
                0, pin, (TXPinActions)combo.SelectedIndex);
    }
}

private void comboPin1TXActionVHF_SelectedIndexChanged(
    object sender, EventArgs e) {
    if (sender is ComboBoxTS) {
        ComboBoxTS combo = sender as ComboBoxTS;
        bool bOk = int.TryParse(combo.Name.Substring(8, 1), out int pin);
        if (bOk)
            Penny.getPenny().setTXPinAction(
                1, pin, (TXPinActions)combo.SelectedIndex);
    }
}

private void comboPin1TXActionSWL_SelectedIndexChanged(
    object sender, EventArgs e) {
    if (sender is ComboBoxTS) {
        ComboBoxTS combo = sender as ComboBoxTS;
        bool bOk = int.TryParse(combo.Name.Substring(8, 1), out int pin);
        if (bOk)
            Penny.getPenny().setTXPinAction(
                2, pin, (TXPinActions)combo.SelectedIndex);
    }
}
...
private void chkPin1TXPAHF_CheckedChanged(object sender, EventArgs e) {
    if (sender is CheckBoxTS) {
        ...
        if (bOk) {
            ...
            console.RepositionExternalPAButton(
                chkbox.Checked || CheckForAnyExternalPACheckBoxes());
            console.PennyExtCtrlEnabled
...
public void UpdateForHotSwitch(bool tx) {
    ...
    foreach (Control c in grpPennyExtCtrl.Controls) {
        ...
        if (cb != null)
            if (cb.Name.Length >= 12
                && cb.Name.Substring(0, 12) == "chkPenOCxmit")
                cb.Enabled = enable;
    }
    foreach (Control c in grpPennyExtCtrlVHF.Controls) {
        CheckBox cb = c as CheckBoxTS;
    ...
    foreach (Control c in grpExtCtrlSWL.Controls) {
        CheckBox cb = c as CheckBoxTS;
        if (cb != null)
            if (cb.Name.Length >= 9 && cb.Name.Substring(0, 9) == "chkOCxmit")
                cb.Enabled = enable;
    }
}
...
public bool AutoAGCRX1 {
    ...
    set { chkAutoAGCRX1.Checked = value; }
}
public bool AutoAGCRX2 {
    get { return chkAutoAGCRX2.Checked; }
    set { chkAutoAGCRX2.Checked = value; }
}
...
private void chkWaterfallUseNFForAGCRX1_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkWaterfallUseNFForAGCRX2_CheckedChanged(
    object sender, EventArgs e) {
...
private void tmrCFCOMPGain_Tick(object sender, EventArgs e) {
    ...
    unsafe {
        fixed(double* ptrCompValues = &CFCCompValues[0])
            WDSP.GetTXACFCOMPDisplayCompression(
                WDSP.id(1, 0), ptrCompValues, &ready);
    }
...
private void chkCTLimitDragToSpectral_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkCTLimitDragMouseOnly_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkFillActivePeakHoldRX1_CheckedChanged(
    object sender, EventArgs e) {
    Display.ActivePeakFillRX1 = chkFillActivePeakHoldRX1.Checked;
}

private void chkFillActivePeakHoldRX2_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkIgnoreMasterAFChangeForMonitor_CheckedChanged(
    object sender, EventArgs e) {
    console.IgnoreAFChangeForMonitor
        = chkIgnoreMasterAFChangeForMonitor.Checked;
}
...
private void chkShowMHzOnCursor_CheckedChanged(object sender, EventArgs e) {
    ...
    //
    console.SetupInfoBarButton(
        ucInfoBar.ActionTypes.CursorInfo, chkShowMHzOnCursor.Checked);
}

private void chkLimitFilterEdgesToSidebands_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkShowPhaseAngularMean_CheckedChanged(
    object sender, EventArgs e) {
...
private void txtTCPIPCATServerBindIPPort_TextChanged(
    object sender, EventArgs e) {
    if (!initializing && chkTCPIPCatServerListening.Checked)
        lblToggleToUse.Visible = true;

    updateTCPIPPort();
}

private void chkTCPIPCatServerListening_CheckedChanged(
    object sender, EventArgs e) {
    if (initializing) return;
    // this is setup in init, after the ip and port have been recovered and
    // inializing is false
...
public void SetupDSPWarnings(bool bufferSizeDifferentRX,
    bool filterSizeDifferentRX, bool filterTypeDifferentRX,
    bool bufferSizeDifferentTX, bool filterSizeDifferentTX,
    bool filterTypeDifferentTX) {
    pbWarningBufferSize.Visible
        = bufferSizeDifferentRX || bufferSizeDifferentTX;
    lblWarningBufferSize.Visible
        = bufferSizeDifferentRX || bufferSizeDifferentTX;

    pbWarningFilterSize.Visible
        = filterSizeDifferentRX || filterSizeDifferentTX;
    lblWarningFilterSize.Visible
        = filterSizeDifferentRX || filterSizeDifferentTX;

    pbWarningBufferType.Visible
        = filterTypeDifferentRX || filterTypeDifferentTX;
    lblWarningBufferType.Visible
        = filterTypeDifferentRX || filterTypeDifferentTX;
}
public long DSPChangeDuration {
    set {
        lblTimeToMakeDSPChange.Text
            = value.ToString() + " ms @ " + DateTime.UtcNow.ToString() + " utc";
    }
}

private void txtTCIServerBindIPPort_TextChanged(object sender, EventArgs e) {
    if (!initializing && chkTCIServerListening.Checked)
        lblToggleToUseTCI.Visible = true;
...
private void chkUseRX1vfoaForRX2vfoa_CheckedChanged(
    object sender, EventArgs e) {
    console.TCIuseRX1vfoaForRX2vfoa = chkUseRX1vfoaForRX2vfoa.Checked;
}

private void chkTCIsendInitialStateOnConnect_CheckedChanged(
    object sender, EventArgs e) {
    console.TCIsendInitialStateOnConnect
        = chkTCIsendInitialStateOnConnect.Checked;
}

private void chkWelcomeMessageTCPIPCat_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkShowTCISpots_CheckedChanged(object sender, EventArgs e) {
    ...
    //
    console.SetupInfoBarButton(ucInfoBar.ActionTypes.ShowSpots,
        chkShowTCISpots.Checked /* | console.SpotForm*/);
}
...
private void chkLegacyDXBuffers_CheckedChanged(object sender, EventArgs e) {
    ...
    // if (!initializing)
    //{
    //     DialogResult dr = MessageBox.Show("This change will take effect when
    //     Thetis is restarted.",
    //         "Legacy DX Buffers",
...
private void chkSpotOwnCallAppearance_CheckedChanged(
    object sender, EventArgs e) {
    txtOwnCallsign.Enabled = chkSpotOwnCallAppearance.Checked;
    clrbtnOwnCallApearance.Enabled = chkSpotOwnCallAppearance.Checked;

    SpotManager2.OwnCallApearance(chkSpotOwnCallAppearance.Checked,
        txtOwnCallsign.Text, clrbtnOwnCallApearance.Color);
}
...
private void setDBtip(object sender) {
    ...
    switch (tb.Name) {
        case "tbCFCPRECOMP": db = tbCFCPRECOMP.Value; break;
        case "tbCFC0": db = tbCFC0.Value; break;
        case "tbCFC1": db = tbCFC1.Value; break;
        case "tbCFC2": db = tbCFC2.Value; break;
        case "tbCFC3": db = tbCFC3.Value; break;
        case "tbCFC4": db = tbCFC4.Value; break;
        case "tbCFC5": db = tbCFC5.Value; break;
        case "tbCFC6": db = tbCFC6.Value; break;
        case "tbCFC7": db = tbCFC7.Value; break;
        case "tbCFC8": db = tbCFC8.Value; break;
        case "tbCFC9": db = tbCFC9.Value; break;
        // EQ
        case "tbCFCPEQGAIN": db = tbCFCPEQGAIN.Value; break;
        case "tbCFCEQ0": db = tbCFCEQ0.Value; break;
        case "tbCFCEQ1": db = tbCFCEQ1.Value; break;
        case "tbCFCEQ2": db = tbCFCEQ2.Value; break;
        case "tbCFCEQ3": db = tbCFCEQ3.Value; break;
        case "tbCFCEQ4": db = tbCFCEQ4.Value; break;
        case "tbCFCEQ5": db = tbCFCEQ5.Value; break;
        case "tbCFCEQ6": db = tbCFCEQ6.Value; break;
        case "tbCFCEQ7": db = tbCFCEQ7.Value; break;
        case "tbCFCEQ8": db = tbCFCEQ8.Value; break;
        case "tbCFCEQ9": db = tbCFCEQ9.Value; break;
...
private void chkNoFadeOverUnderWarning_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkSwapREDBluePSAColours_CheckedChanged(
    object sender, EventArgs e) {
...
private void udHPSDRFreqCorrectFactor10MHz_ValueChanged(
    object sender, EventArgs e) {
    udHPSDRFreqCorrectFactor_ValueChanged(this, EventArgs.Empty);
}
    ...
    using (OpenFileDialog ofd = new OpenFileDialog()) {
        ...
        if (ofd.FileName != "" && dr == DialogResult.OK) {
            if (File.Exists(ofd.FileName)) {
                await Task.Run(() => lgLinearGradientRX1.EncodedText
...
private async void btnSaveGradient_Click(object sender, EventArgs e) {
    using (SaveFileDialog sfd = new SaveFileDialog()) {
        ...
        if (sfd.FileName != "" && dr == DialogResult.OK) {
            await Task.Run(() => File.WriteAllText(sfd.FileName,
                               lgLinearGradientRX1.EncodedText, Encoding.UTF8));
        }
    }
}
...
private void chkEmulateExpertSDR3Protocol_CheckedChanged(
    object sender, EventArgs e) {
    console.EmulateExpertSDR3Protocol = chkEmulateExpertSDR3Protocol.Checked;
}

private void chkDisableRearSpeakerJacksAudioAmplifier_CheckedChanged(
    object sender, EventArgs e) {
    console.EnableAudioAmplifier
        = !chkDisableRearSpeakerJacksAudioAmplifier.Checked;
}

private void radUseDriveSliderTune_CheckedChanged(object sender, EventArgs e) {
    if (radUseDriveSliderTune.Checked)
...
private void radUseTuneSliderTune_CheckedChanged(object sender, EventArgs e) {
    if (radUseTuneSliderTune.Checked)
...
private void radUseFixedDriveTune_CheckedChanged(object sender, EventArgs e) {
    if (radUseFixedDriveTune.Checked)
...
private void setupTuneAnd2ToneRadios() {
    switch (console.TuneDrivePowerOrigin) {
        ...
        case DrivePowerSource.TUNE_SLIDER:
            ...
            break;
        case DrivePowerSource.FIXED: radUseFixedDriveTune.Checked = true; break;
    }
    ...
    switch (console.TwoToneDrivePowerOrigin) {
        ...
        case DrivePowerSource.FIXED:
            radUseFixedDrive2Tone.Checked
                = true; // MW0LGE_[2.9.0.7] fixed, was setting incorrect radio
                        // button
            break;
    }
}
...
private void radUseDriveSlider2Tone_CheckedChanged(object sender, EventArgs e) {
    if (radUseDriveSlider2Tone.Checked)
...
private void radUseTuneSlider2Tone_CheckedChanged(object sender, EventArgs e) {
    if (radUseTuneSlider2Tone.Checked)
...
private void radUseFixedDrive2Tone_CheckedChanged(object sender, EventArgs e) {
    if (radUseFixedDrive2Tone.Checked)
        console.TwoToneDrivePowerOrigin = DrivePowerSource.FIXED;
}
private void chkLimitPowerCATTCIMsgs_CheckedChanged(
    object sender, EventArgs e) {
...
private void updatePAControls(PAProfile p) {
    ...
    btnDeletePAProfile.Enabled = p.ProfileName.StartsWith("Default")
        ? false
        : true; // unable to delete Default
...
private bool validatePAProfileName(string sProfileName) {
    if (sProfileName.StartsWith("Default")) {
        DialogResult dr = MessageBox.Show("The PA profile name " + sProfileName
                + " can not start with 'Default'.",
            "PA Profile Name", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        return false;
    }
...
private void btnCopyPAProfile_Click(object sender, EventArgs e) {
    ...
    string sProfileName = InputBox.Show("Copy Profile - " + curP.ProfileName,
        "Please enter a profile name:", "");

    if (sProfileName == "") return;
    if (!validatePAProfileName(sProfileName)) return;

    PAProfile newP = new PAProfile(sProfileName, HPSDRModel.FIRST,
        false); // we dont really want it associated with a model
...
private void btnNewPAProfile_Click(object sender, EventArgs e) {
    string sProfileName
        = InputBox.Show("New Profile", "Please enter a profile name:", "");

    if (sProfileName == "") return;
    if (!validatePAProfileName(sProfileName)) return;

    PAProfile p = new PAProfile(sProfileName, HPSDRModel.FIRST,
        false); // we dont really want it associated with a model
    p.ResetGainDefaultsForModel(
        console.CurrentHPSDRModel); // set the initial values based on current
                                    // model, all we can do
...
private void btnDeletePAProfile_Click(object sender, EventArgs e) {
    ...
    DialogResult dr = MessageBox.Show("Are you sure you want to delete the "
            + comboPAProfile.Text + " PA Profile?\n\n"
...
private void updateNUDgains(PAProfile p) {
    // if (_adjustingBand == Band.FIRST) return;  //MW0LGE_[2.9.7.0] fix issue
    // where changing global gains would not take when out of ham band
...
//
private Dictionary<string, LabelTS> _PADriveLabels
    = new Dictionary<string, LabelTS>();
...
private void updateDriveLabels(PAProfile p = null) {
    ...
    if (_PADriveLabels.Count == 0) {
        foreach (Control c in panelAdjustGain.Controls) {
            ...
            if (lbl != null && lbl.Name.StartsWith("lblPAAdjust"))
                _PADriveLabels.Add(lbl.Name, lbl);
        }
    }
...
private void btnResetPAProfile_Click(object sender, EventArgs e) {
    DialogResult dr = MessageBox.Show("Are you sure you want to reset the "
            + comboPAProfile.Text + " PA Profile?\n\n"
            + "The Default settings for this radio model will be applied if you do.",
        "Reset Profile?", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
...
private int mapBandToMeters(Band b) {
    switch (b) {
        case Band.B160M: return 160;
        case Band.B80M: return 80;
        case Band.B60M: return 60;
        case Band.B40M: return 40;
        case Band.B30M: return 30;
        case Band.B20M: return 20;
        case Band.B17M: return 17;
        case Band.B15M: return 15;
        case Band.B12M: return 12;
        case Band.B10M: return 10;
        case Band.B6M: return 6;
    }
    return 0;
}
private void nudPAProfileGain_ValueChanged(object sender, EventArgs e) {
    ...
    int nNumber = bVHF ? int.Parse(nud.Name.Substring(6))
                       : int.Parse(nud.Name.Substring(3, nud.Name.Length - 4));
    ...
    if (p.GetGainForBand(b) != fOld)
        console.PWR = console.PWR; // update the power, which causes these gain
...
private void initPAProfiles() {
    ...
    // add a bypass default, which is used when in PAbypass (part of special
    // calibrate)
    p = new PAProfile(_sPA_PROFILE_BYPASS, HPSDRModel.FIRST,
        true); // no specific model for this
    p.ResetGainDefaultsForModel(p.Model, true);
    _PAProfiles.Add(p.ProfileName, p);
}
private PAProfile getPAProfile(string sName) {
    if (_PAProfiles == null || _PAProfiles.Count == 0
        || !_PAProfiles.ContainsKey(sName))
        return null;
...
private void updatePAProfileCombo(string sSelectProfile = "") {
    ...
    foreach (KeyValuePair<string, PAProfile> pair in _PAProfiles) {
        ...
        if (sSelectProfile == _sPA_PROFILE_BYPASS) {
            ...
            if ((p.IsDefault && p.Model == console.CurrentHPSDRModel)
                || !p.IsDefault) // add any that are default for this current
                                 // model, or are not default, ie user added
                comboPAProfile.Items.Add(p.ProfileName);
        }
    }
...
private float getOldVariablePAgain(
    string sKey, ref Dictionary<string, string> getDict) {
...
private void handleOldPAGainSettings(ref Dictionary<string, string> getDict) {
    ...
    // here we have to map all the old settings, ~200 variables into the new
    // profile system, and remove them

    // use the existance of ProfileCount as a check to see we have already done
    // this
    if (getDict.ContainsKey("PAProfileCount")) return;

    bool bRemoveOld
        = true; // set to true if you want the old variables removed from the db
        ...
        switch (p.Model) {
            ...
            case HPSDRModel.ANAN10E:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN10PAGain" + mapBandToMeters(b).ToString();
                ...
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN10PAGainVHF" + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
            ...
            case HPSDRModel.ANAN100:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN100PAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting = "udANAN100PAGainVHF"
                        + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
            ...
            case HPSDRModel.ANAN100B:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN100BPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting = "udANAN100BPAGainVHF"
                        + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
            ...
            case HPSDRModel.ANAN100D:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANANPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANANPAGainVHF" + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
            ...
            case HPSDRModel.ANAN200D:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udOrionPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udOrionPAGainVHF" + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
            ...
            case HPSDRModel.ANAN7000D:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN7000DPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting = "udANAN7000DPAGainVHF"
                        + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
            ...
            case HPSDRModel.ANAN8000D:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udANAN8000DPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting = "udANAN8000DPAGainVHF"
                        + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
            ...
            case HPSDRModel.HERMES:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udHermesPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udHermesPAGainVHF" + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
            ...
            case HPSDRModel.HPSDR:
                if (p.Model == HPSDRModel.HPSDR
                    || (p.Model == HPSDRModel.FIRST
                        && p.ProfileName == _sPA_PROFILE_BYPASS)) {
                    for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                        Band b = (Band)n;
                        string sSetting
                            = "udPAGain" + mapBandToMeters(b).ToString();
                        float g = getOldVariablePAgain(sSetting, ref getDict);
                        if (g != 1000) p.SetGainForBand(b, g);
                        if (g != 1000 && bRemoveOld)
                            removeOldPASetting(sSetting);
                    }
                    for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                        Band b = (Band)n;
                        string sSetting
                            = "udPAGainVHF" + (n - (int)Band.VHF0).ToString();
                        float g = getOldVariablePAgain(sSetting, ref getDict);
                        if (g != 1000) p.SetGainForBand(b, g);
                        if (g != 1000 && bRemoveOld)
                            removeOldPASetting(sSetting);
            ...
            case HPSDRModel.ORIONMKII:
                for (int n = (int)Band.B160M; n <= (int)Band.B6M; n++) {
                    Band b = (Band)n;
                    string sSetting
                        = "udORIONMKIIPAGain" + mapBandToMeters(b).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
                }
                for (int n = (int)Band.VHF0; n <= (int)Band.VHF13; n++) {
                    Band b = (Band)n;
                    string sSetting = "udORIONMKIIPAGainVHF"
                        + (n - (int)Band.VHF0).ToString();
                    float g = getOldVariablePAgain(sSetting, ref getDict);
                    if (g != 1000) p.SetGainForBand(b, g);
                    if (g != 1000 && bRemoveOld) removeOldPASetting(sSetting);
...
private void enabledPAAdjust(bool bEnabled) {
    foreach (Control c in panelAdjustGain.Controls) {
        ...
        if (c != null) {
            if (c.Name.StartsWith("nudAdjustGain")) c.Enabled = bEnabled;
        }
    }
    ...
    foreach (Control c in grpGainByBandPA.Controls) {
        ...
        if (c != null) {
            if (c.Name.StartsWith("nud")) {
                bool bVHF = nud.Name.StartsWith("nudVHF");
                int nNumber = bVHF
                    ? int.Parse(nud.Name.Substring(6))
                    : int.Parse(nud.Name.Substring(3, nud.Name.Length - 4));
                Band b;
                if (bVHF)
                    b = (Band)((int)Band.VHF0 + nNumber);
...
private void nudAdjustGain_ValueChanged(object sender, EventArgs e) {
    ...
    if (console.MOX) {
        switch (console.TuneDrivePowerOrigin) {
            case DrivePowerSource.DRIVE_SLIDER:
                console.PWR
                    = nNumber + 10; // set drive to the value we are adjusting
                break;
            case DrivePowerSource.TUNE_SLIDER:
...
private void chkUseSUnitsForPBNPPBSNR_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkAdjustGridMinToNFRX1_CheckedChanged(
    object sender, EventArgs e) {
...
private void chkAdjustGridMinToNFRX2_CheckedChanged(
    object sender, EventArgs e) {
...
private void btnResetLevelCal_Click(object sender, EventArgs e) {
    DialogResult dr = MessageBox.Show(
        ...
        "Level Defaults", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

    if (dr == DialogResult.Yes) console.ResetLevelCalibration();
}
...
private void chkPreventTXonDifferentBandToRX_CheckedChanged(
    object sender, EventArgs e) {
    console.PreventTXonDifferentBandToRXband
        = chkPreventTXonDifferentBandToRX.Checked;
}
...
private MeterManager.clsMeter meterFromSelectedContainer() {
    clsContainerComboboxItem cci
        = comboContainerSelect.SelectedItem as clsContainerComboboxItem;
...
private void updateMeter2Controls(string sId = "") {
    ...
    foreach (KeyValuePair<string, ucMeter> kvp in MeterManager
                 .MeterContainers) {
        clsContainerComboboxItem cci = new clsContainerComboboxItem();
        cci.Text = "Container " + (i + 1).ToString() + " TRX"
            + kvp.Value.RX.ToString();
        cci.ID = kvp.Value.ID;
    ...
    btnMeterCopySettings.Enabled
        = bEnableControls && lstMetersInUse.Items.Count > 0;
...
private void updateMeterLists() {
    ...
    List<clsMeterTypeComboboxItem> notinuse
        = new List<clsMeterTypeComboboxItem>();

    for (int n = 1; n < (int)MeterType.LAST; n++) {
        MeterType mt = (MeterType)n;

        if (m.HasMeterType(mt)) {
            clsMeterTypeComboboxItem mtci
                = new clsMeterTypeComboboxItem(mt, m.GetOrderForMeterType(mt));
            inuse.Add(mtci);
        } else {
            clsMeterTypeComboboxItem mtci
                = new clsMeterTypeComboboxItem(mt, -1);
            notinuse.Add(mtci);
        }
    }
...
private void comboContainerSelect_SelectedIndexChanged(
    object sender, EventArgs e) {
    clsContainerComboboxItem cci
        = (clsContainerComboboxItem)comboContainerSelect.SelectedItem;
    if (cci == null) return;
    ...
    clrbtnContainerBackground.Color
        = MeterManager.GetContainerBackgroundColour(cci.ID);
...
private void btnAddMeterItem_Click(object sender, EventArgs e) {
    clsMeterTypeComboboxItem mti
        = lstMetersAvailable.SelectedItem as clsMeterTypeComboboxItem;
    if (mti == null) return;
...
private void lstMetersAvailable_SelectedIndexChanged(
    object sender, EventArgs e) {
...
private void btnRemoveMeterItem_Click(object sender, EventArgs e) {
    clsMeterTypeComboboxItem mti
        = lstMetersInUse.SelectedItem as clsMeterTypeComboboxItem;
    if (mti == null) return;
...
private void lstMetersInUse_DrawItem(object sender, DrawItemEventArgs e) {
    ...
    if (e.Index >= 0) {
        clsMeterTypeComboboxItem mtci
            = (clsMeterTypeComboboxItem)((ListBox)sender).Items[e.Index];
        if (mtci != null) {
            SolidBrush sb;
        ...
        g.DrawString(" " + ((ListBox)sender).Items[e.Index].ToString(), e.Font,
            sbt, e.Bounds, StringFormat.GenericDefault);
        sbt.Dispose();
...
private MeterManager.clsIGSettings updateMeterType() {
    ...
    if (mt == MeterType.SIGNAL_TEXT) {
        ...
        igs.IgnoreHistoryDuration
            = (int)nudMeterItemIgnoreHistoryDuration.Value;
        ...
        igs.ShowMarker
            = radMM24Clock.Checked; // use the show marker bool for this
    } else {
        ...
        igs.SubMarkerColour
            = Color.FromArgb(255, clrbtnMeterItemSubIndiciator.Color);
        ...
        igs.HistoryColor = Color.FromArgb(
            tbMeterItemHistoryAlpha.Value, clrbtnMeterItemHistory.Color);
        ...
        igs.SegmentedSolidLowColour
            = clrbtnMeterItemSegmentedSolidColourLow.Color;
        igs.SegmentedSolidHighColour
            = clrbtnMeterItemSegmentedSolidColourHigh.Color;

        igs.PeakHold = chkMeterItemPeakHold.Checked;
        igs.PeakHoldMarkerColor
            = Color.FromArgb(255, clrbtnMeterItemPeakHold.Color);
        igs.HistoryDuration = (int)nudMeterItemHistoryDuration.Value;
        igs.FadeOnRx = chkMeterItemFadeOnRx.Checked;
        igs.FadeOnTx = chkMeterItemFadeOnTx.Checked;
        ...
        if (mt == MeterType.ANANMM || mt == MeterType.MAGIC_EYE)
            igs.Average = chkMeterItemSignalAverage.Checked;
        if (mt == MeterType.ANANMM || mt == MeterType.CROSS)
            igs.DarkMode = chkMeterItemDarkMode.Checked;
    }
...
private void updateItemSettingsControlsForSelected() {
    ...
    if (mt == MeterType.SIGNAL_TEXT) {
        nudMeterItemUpdateRate.Value
            = igs.UpdateInterval < nudMeterItemUpdateRate.Minimum
        ...
        nudMeterItemHistoryDuration.Value
            = igs.HistoryDuration < nudMeterItemHistoryDuration.Minimum
            ? nudMeterItemHistoryDuration.Minimum
            : igs.HistoryDuration;
        ...
        radMM24Clock.Checked
            = igs.ShowMarker; // use the show marker bool for this
        if (!radMM24Clock.Checked && !radMM12Clock.Checked)
            radMM12Clock.Checked = true;
        ...
        updateHistoryControls(
            igs.ShowHistory, igs.HistoryColor, igs.ShowHistory);
        nudMeterItemHistoryDuration.Value
            = igs.HistoryDuration < nudMeterItemHistoryDuration.Minimum
            ? nudMeterItemHistoryDuration.Minimum
            : igs.HistoryDuration;
        ...
        if (igs.BarStyle == MeterManager.clsBarItem.BarStyle.Segments)
            chkMeterItemSegmented.Checked
                = true; // will cause solid to turn off
        ...
        updatePeakHoldControls(
            igs.PeakHold, igs.PeakHoldMarkerColor, igs.PeakHold);
        ...
        if (mt == MeterType.CROSS || mt == MeterType.ANANMM
            || mt == MeterType.PWR || mt == MeterType.REVERSE_PWR)
            nudMeterItemsPowerLimit.Value = (decimal)igs.MaxPower;
        if (mt == MeterType.CROSS || mt == MeterType.ANANMM)
            clrbtnMeterItemPowerScale.Color = igs.PowerScaleColour;

        // specific to mt
        bool bMagicEye = mt == MeterType.MAGIC_EYE;
        if (bMagicEye) {
            nudMeterItemEyeScale.Value
                = (decimal)igs.EyeScale; // prevents setting it to 0 as other
                                         // items will have 0 //FIX THIS
        ...
        clrbtnMeterItemHistory.Enabled
            = !bMagicEye && chkMeterItemHistory.Checked;
        chkMeterItemPeakHold.Enabled = !(bMagicEye || mt == MeterType.CROSS);
        clrbtnMeterItemPeakHold.Enabled = !(bMagicEye || mt == MeterType.CROSS)
            && chkMeterItemPeakHold.Checked;

        chkMeterItemShadow.Enabled
            = mt == MeterType.ANANMM || mt == MeterType.CROSS;
        chkMeterItemDarkMode.Enabled
            = mt == MeterType.ANANMM || mt == MeterType.CROSS;

        lblMMPowerLimit.Enabled
            = mt == MeterType.ANANMM || mt == MeterType.CROSS;
        clrbtnMeterItemPowerScale.Enabled
            = mt == MeterType.ANANMM || mt == MeterType.CROSS;

        nudMeterItemsPowerLimit.Enabled = mt == MeterType.ANANMM
            || mt == MeterType.CROSS || mt == MeterType.PWR
            || mt == MeterType.REVERSE_PWR;

        bool bEnable = mt == MeterType.ANANMM || mt == MeterType.CROSS
            || mt == MeterType.MAGIC_EYE;
        chkMeterItemSegmented.Enabled = !bEnable;
        chkMeterItemSolid.Enabled = !bEnable;

        clrbtnMeterItemSegmentedSolidColourLow.Enabled = !bEnable
            && (chkMeterItemSegmented.Checked || chkMeterItemSolid.Checked);
        clrbtnMeterItemSegmentedSolidColourHigh.Enabled = !bEnable
            && (chkMeterItemSegmented.Checked || chkMeterItemSolid.Checked);
        lblMMsegSolLow.Enabled = !bEnable
            && (chkMeterItemSegmented.Checked || chkMeterItemSolid.Checked);
        lblMMsegSolHigh.Enabled = !bEnable
            && (chkMeterItemSegmented.Checked || chkMeterItemSolid.Checked);
        ...
        chkMeterItemSignalAverage.Enabled
            = mt == MeterType.ANANMM || mt == MeterType.MAGIC_EYE;
        if (mt == MeterType.ANANMM || mt == MeterType.MAGIC_EYE)
            chkMeterItemSignalAverage.Checked = igs.Average;
        if (mt == MeterType.ANANMM || mt == MeterType.CROSS)
            chkMeterItemDarkMode.Checked = igs.DarkMode;
        //
    }
...
private void updateHistoryControls(
    bool showHistory, Color c, bool updateColor = false) {
...
private void updatePeakHoldControls(
    bool showPeakHold, Color c, bool updateColor = false) {
...
private void chkMeterItemSegmented_CheckedChanged(object sender, EventArgs e) {
    if (chkMeterItemSegmented.Checked)
        chkMeterItemSolid.Checked = false; // can only be one
...
private void nudMeterItemHistoryDuration_ValueChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void clrbtnMeterItemPeakValueColour_Changed(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void chkMeterItemSignalAverage_CheckedChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void chkMaintainNFAdjustDeltaRX2_CheckedChanged(
    object sender, EventArgs e) {
    console.MaintainNFAdjustDeltaRX2 = chkMaintainNFAdjustDeltaRX2.Checked;
}

private void chkMaintainNFAdjustDeltaRX1_CheckedChanged(
    object sender, EventArgs e) {
...
private void clrbtnContainerBackground_Changed(object sender, EventArgs e) {
    ...
    if (cci != null) {
        MeterManager.ContainerBackgroundColour(
            cci.ID, clrbtnContainerBackground.Color);
    }
}
...
private void chkMeterItemSolid_CheckedChanged(object sender, EventArgs e) {
    if (chkMeterItemSolid.Checked)
        chkMeterItemSegmented.Checked = false; // can only be one

    updateSegmentedSolidControls();
    updateMeterType();
}

private void clrbtnMeterItemSegmentedSolidColourHigh_Changed(
    object sender, EventArgs e) {
    updateMeterType();
}

private void clrbtnMeterItemSegmentedSolidColourLow_Changed(
    object sender, EventArgs e) {
    updateMeterType();
}

private void chkMeterItemShowIndicator_CheckedChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void chkMeterItemShowSubIndicator_CheckedChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void nudMeterItemEyeBezelScale_ValueChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private void nudMeterItemIgnoreHistoryDuration_ValueChanged(
    object sender, EventArgs e) {
    updateMeterType();
}
...
private bool canPasteSettings() {
    ...
    } else if (_itemGroupSettingsMeterType == MeterType.MAGIC_EYE
        || _itemGroupSettingsMeterType == MeterType.CROSS
        || _itemGroupSettingsMeterType == MeterType.ANANMM
        || _itemGroupSettingsMeterType == MeterType.SIGNAL_TEXT
        || _itemGroupSettingsMeterType == MeterType.VFO_DISPLAY
        || _itemGroupSettingsMeterType == MeterType.CLOCK) {
...
private void lblDataBase_MouseUp(object sender, MouseEventArgs e) {
    bool fail = true;
    try {
        System.Windows.Forms.Clipboard.SetText(lblDataBase.Text);
        fail = false;
    } catch {
        MessageBox.Show("Error copying to clipboard, sorry :=(");
    } finally {
        if (!fail) {
            MessageBox.Show(
