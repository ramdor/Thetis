...
namespace Thetis
{
    public class wbDisplay : PictureBox
    {
        ...
        // private Bitmap waterfall_bmp = null;					// saved waterfall picture
        // for display
        ...
        private float rx_display_cal_offset
            = -40.1f; // display calibration offset in dB
        ...
        // private bool average_on;							// True if the Average button
        // is pressed public bool AverageOn
        //{
        ...
        // private bool peak_on;							// True if the Peak button
        // is pressed public bool PeakOn
        //{
        //     get { return peak_on; }
        //     set
        //     {
        ...
        /*/
            ...
            // vfo_hz - rit_hz) / width * W); int filter_right_x =
            // (int)((float)(filter_high - Low + vfoa_sub_hz - vfo_hz - rit_hz) /
            // width * W);

            // make the filter display at least one pixel wide.
            // if (filter_left_x == filter_right_x) filter_right_x = filter_left_x +
            // 1;
            ...
            // DSPMode.CWU))
            //{
            //    //filterLeft = (int)((float)(filter_low - Low - f_diff) / width *
            //    W);
            //    //filterRight = (int)((float)(filter_high - Low - f_diff) / width
            //    * W);
            ...
            //        if (c.InBW((rf_freq + Low) * 1e-6, (rf_freq + High) * 1e-6))
            //        // is channel visible?
            //        {
            //            bool on_channel = console.RX1IsOn60mChannel(c); // only
            //            true if you are on channel and are in an acceptable mode
            ...
            //                    break;
            //                default:
            //                    on_channel = false; // make sure other modes do
            //                    not look as if they could transmit break;
            //            }
            ...
            //            c.BW / 2 - Low - rit) / width * W); int chan_right_x =
            //            (int)((float)(c.Freq * 1e6 - rf_freq + c.BW / 2 - Low -
            //            rit) / width * W);
            ...
            //            drawChannelBar(g, c, chan_left_x, chan_right_x, top, H -
            //            top, c1, c2);
            //        }
            //    }
            //}
            ...
            // center_line_x = (int)((float)(-f_diff - Low) / width * W); // locked
            // 0 line

            // if (center_line_x >= 0 && center_line_x <= W)
            //{
            //         g.DrawLine(grid_zero_pen, center_line_x, top, center_line_x,
            //         H); g.DrawLine(grid_zero_pen, center_line_x + 1, top,
            //         center_line_x + 1, H);
            // }

            // if (show_freq_offset)
            //{
            //         g.DrawString("0", font9, grid_zero_pen.Brush, center_line_x -
            //         5, (float)Math.Floor(H * .01));
            // }
                    ...
                    switch (current_region)
                    {
                        case FRSRegion.LAST:
                            if (!show_freq_offset)
                            {
                                ...
                                // vgrid - offsetL, freqScalePanRect.Top +
                        ...
                        //              actual_fgrid == 3.5 || actual_fgrid
                        //              == 4.0 || actual_fgrid == 7.0 ||
                        //              actual_fgrid == 7.3 || actual_fgrid
                        //              == 10.1 || actual_fgrid == 10.15 ||
                        //              actual_fgrid == 14.0 || actual_fgrid
                        //              == 14.35 || actual_fgrid == 18.068 ||
                        //              actual_fgrid == 18.168 || actual_fgrid
                        //              == 21.0 || actual_fgrid == 21.45 ||
                        //              actual_fgrid == 24.89 || actual_fgrid
                        //              == 24.99 ||
        ...
        unsafe private bool DrawWideBand(
            Graphics g, int rx)
        {
            int W = panRect.Width;
            int H = panRect.Height;
            ...
            // float slope = 0.0F;						// samples
            // to process per pixel int num_samples = 0;
            // // number of samples to process int
            // start_sample_index = 0;				// index to
            // begin looking at samples
        ...
        // 0.0f; unsafe private bool
        // DrawWaterfall(Graphics g, int rx)
        //{
        //     int W = waterfallRect.Width;
        //     int H = waterfallRect.Height;

        //    // if (grid_control) DrawWaterfallGrid(ref
        //    g, rx, bottom);
        ...
        //    }
        //    float slope = 0.0F;						//
        //    samples to process per pixel int
        //    num_samples = 0;
        //    // number of samples to process int
        //    start_sample_index = 0;				// index
        //    to begin looking at samples int Low = 0;
        //    int High = 0; float local_max_y =
        //    float.MinValue; bool local_mox = false;
        //    //if (rx == 1 && !tx_on_vfob && mox)
        //    local_mox = true;
        //    //if (rx == 2 && tx_on_vfob && mox)
        //    local_mox = true; float local_min_y_w3sz =
        //    float.MaxValue;  //added by w3sz float
        //    display_min_w3sz = float.MaxValue; //added
        //    by w3sz float display_max_w3sz =
        //    float.MinValue; //added by w3sz float
        //    min_y_w3sz = float.MaxValue;  //w3sz int R
        //    = 0, G = 0, B = 0; int grid_max = 0; int
        //    grid_min = 0; bool displayduplex = false;
        ...
        //    DisplayMode.PANAFALL && (nreceivers <= 2
        //    && display_duplex)) ||
        //        (CurrentDisplayMode ==
        //        DisplayMode.PANAFALL && nreceivers >
        //        2) ||
        //       (CurrentDisplayMode ==
        //       DisplayMode.PANADAPTER &&
        //       display_duplex)) displayduplex = true;
        ...
        //    {
        //        waterfall_minimum =
        //        rx1_waterfall_minimum; low_threshold =
        ...
        //        duration =
        //        (int)timer_waterfall.DurationMsec;
        ...
        //            start_sample_index = (BUFFER_SIZE
        //            >> 1) + ((Low * BUFFER_SIZE) /
        //            sample_rate); num_samples = ((High
        //            - Low) * BUFFER_SIZE /
        //            sample_rate); if
        ...
        //                float max = float.MinValue;
        //                float dval = i * slope +
        //                start_sample_index; int lindex
        //                = (int)Math.Floor(dval); int
        //                rindex = (int)Math.Floor(dval
        //                + slope);
        ...
        //            * bitmapData.Height;		// find
        //            buffer size Win32.memcpy(new
        //            IntPtr((int)bitmapData.Scan0 +
        //            bitmapData.Stride).ToPointer(),
        ...
        //            + (waterfall_minimum * 2)) / 10) +
        //            1; //wfagc
        //            // else
        //            //  RX2waterfallPreviousMinValue =
        //            ((RX2waterfallPreviousMinValue *
        //            8) + (waterfall_minimum * 2)) / 10
        //            + 1; //wfagc
        //        }

        //        g.DrawImageUnscaled(waterfall_bmp, 0,
        //        waterfallRect.Top);	// draw the
        ...
        public void CreateDisplayRegions()
        {
            ...
            dBmScalePanRect
                = new Rectangle(panRect.Right - 35,
                    displayTop, 35, panRect.Height);

            // secScaleWaterfallRect = new Rectangle(
            //         waterfallRect.Left,
            //         freqScalePanRect.Top +
            //         freqScalePanRect.Height, 45,
            //         waterfallRect.Height);

            // if (waterfallRect.Height == 0)
            // waterfallRect.Height = 1; waterfall_bmp = new
            // Bitmap(waterfallRect.Width,
            // waterfallRect.Height,
            // PixelFormat.Format24bppRgb);	// initialize
            // waterfall display
        ...
        private void getRegion(Point p)
        {
            ...
            // < snapMouse && this.PanRect.Contains(p))
            //{
            //     mouseRegion =
            //     DisplayRegion.filterRegionLow;
            //     // mouseDownFilterFrequencyLo =
            //     filterLowerFrequency;
            // }
            // else if (Math.Abs(p.X -
            // this.FilterRect.Right) < snapMouse &&
            // this.PanRect.Contains(p))
            //{
            //     mouseRegion =
            //     DisplayRegion.filterRegionHigh;
        ...
        private readonly Object m_objBufferLock
            = new Object();
        ...
        unsafe private void RunDisplay(int rx)
        {
            ...
            //        {
            //            using (Graphics grSrc =
            //            Graphics.FromImage(wbDisplay_buffer))
            //            {
                ...
                case DisplayRegion.freqScalePanadapterRegion:
                    if (e.Button == MouseButtons.Left)
                    {
                        ...
                        if (moveX)
                        {
                            ...
                            //  cmaster.Getrxa(adc).RXFreq +=
                            //  (long)deltaFreq;
                            //  cmaster.Getrxa(display_id +
                            //  2).RXFreq += (deltaFreq *
                            //  0.0000010);
        ...
        private double z_slider
            = 0.0; // range is 0.0 to 1.0
        public double
            ZoomSlider // set by Zoom Slider position
        {
            get { return z_slider; }
            set
            {
        ...
        public void create_wideband(int adc)
        {
            int success = 0;
            int wbid = 32 + adc;
            SpecHPSDRDLL.XCreateAnalyzer(wbid, ref success,
                16384, 1, 1, ""); // 16384
        }
        ...
        public void initWideband()
        {
            ...
            // displayed:
            low_freq = sample_rate / 4
                - (int)(0.5 * bw
                    - (double)span_clip_l * bin_width);
            high_freq = sample_rate / 4
                + (int)(0.5 * bw
                    - (double)span_clip_h * bin_width);

            SpecHPSDRDLL.SetAnalyzer(
                wbid, // id of this analyzer
                2, // number of outputs desired for this
                   // display
                1, // one fft, no spur elimination for this
                   // application
                0, // REAL data
                h_flip, // flip settings for spur
                        // elimination ffts
                fft_size, // fft_size
                512, // input buffer size
                window_type, // window_type
                kaiser_pi, // piAlpha for Kaiser window
                0, // overlap can't be used for
                   // discontinuous data
                clip, // bins to clip on EACH side
                span_clip_l, // additional bins to clip on
                             // the low side
                span_clip_h, // additional bins to clip on
                             // the high side
                pixels, // number of pixel values to
                        // generate
            ...
            SpecHPSDRDLL.SetDisplayAverageMode(
                wbid, 0, avm);
