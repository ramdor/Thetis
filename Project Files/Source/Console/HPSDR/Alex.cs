...
// this module contains code to support the Alex Filter and Antenna Selection
// board
//
//
...
public class Alex {
    ...
    private byte[] RxOnlyAnt
        = new byte[12]; // 1 = rx1, 2 = rx2, 3 = xv, 0 = none selected
    private bool LimitTXRXAntenna
        = false; // when set, antennas should stay on 1 (for external Aries ATU)
    ...
    public void SetAntennasTo1(bool IsSetTo1) { LimitTXRXAntenna = IsSetTo1; }
    ...
    public static Band AntBandFromFreq() {
        ...
        double freq = Console.getConsole().VFOAFreq; // was = 0.0 Vk4xv Txvr
                                                     // fix.

        if (c.RX1XVTRIndex >= 0)
            freq = c.XVTRForm.TranslateFreq(freq);
        else
            freq = Console.getConsole().VFOAFreq;
    ...
    public static Band AntBandFromFreqB() {
        ...
        double freq = Console.getConsole().VFOBFreq; // was = 0.0 Vk4xv Txvr
                                                     // fix.

        if (c.RX2XVTRIndex >= 0)
            freq = c.XVTRForm.TranslateFreq(freq);
        else
            freq = Console.getConsole().VFOBFreq;
    ...
    public void UpdateAlexAntSelection(
        Band band, bool tx, bool alex_enabled, bool xvtr) {
        ...
        System.Console.WriteLine(
            "Ant idx: " + idx); // moved into different check down below
        ...
        if (tx) {
            ...
            if (xvtr) {
                ...
                if (rx_only_ant >= 3)
                    rx_only_ant -= 3; // do not use XVTR ant port if not using
                                      // transverter
            }
        ...
        if (m_nOld_rx_only_ant != rx_only_ant || m_nOld_trx_ant != trx_ant
            || m_nOld_rx_out != rx_out || m_bOld_tx != tx) {
            NetworkIO.SetAntBits(rx_only_ant, trx_ant, rx_out, tx);
            System.Console.WriteLine("Ant Rx Only {0} , Tx Ant {1}, Rx Out {2}",
                rx_only_ant.ToString(), trx_ant.ToString(), rx_out.ToString());
        ...
        /*	if ( tx )
            {
                AlexEnableSavedState =
           Console.getConsole().SetupForm.SetAlexAntEnabled(false);
        ...
        //  Console.getConsole().SetupForm.txtAlexEnabled.Text =
        //  alex_enabled.ToString();
        //  Console.getConsole().SetupForm.txtAlexBits.Text =
        //  Convert.ToString(rc, 2);
