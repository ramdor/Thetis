...
#region Windows Form Designer generated code
    ...
    private void InitializeComponent() {
        ...
        this.checkBoxLoop.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.checkBoxLoop.CheckedChanged
            += new EventHandler(this.checkBoxLoop_CheckedChanged);
        //
        // btnAdd
        //
        ...
        this.lstPlaylist.SelectionMode
        ...
        this.lstPlaylist.DoubleClick
            += new EventHandler(this.lstPlaylist_DoubleClick);
        //
        // btnRemove
        //
        ...
        this.createBoxTS.Font = new Font("Microsoft Sans Serif", 6.75F,
        ...
        this.createBoxTS.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.createBoxTS.Visible = false;
        this.createBoxTS.CheckedChanged
            += new EventHandler(this.createBoxTS_CheckedChanged);
        //
        // TXIDBoxTS
        //
        this.TXIDBoxTS.Appearance = System.Windows.Forms.Appearance.Button;
        this.TXIDBoxTS.Enabled = false;
        this.TXIDBoxTS.Font = new Font("Microsoft Sans Serif", 6.75F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        this.TXIDBoxTS.CheckedChanged
            += new EventHandler(this.TXIDBoxTS_CheckedChanged);
        //
        // chkBoxMP3
        //
        ...
        // labelTS1
        //
        this.labelTS1.Font = new Font("Microsoft Sans Serif", 8.25F,
        ...
        this.chkQuickPlay.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.chkQuickPlay.CheckedChanged
            += new EventHandler(this.chkQuickPlay_CheckedChanged);
        //
        // chkQuickRec
        //
        ...
        this.chkQuickRec.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.chkQuickRec.CheckedChanged
            += new EventHandler(this.chkQuickRec_CheckedChanged);
        //
        // groupBoxTS1
        //
        ...
        this.udPreamp.Minimum
            = new decimal(new int[] { 70, 0, 0, -2147483648 });
        ...
        this.udPreamp.ValueChanged
            += new EventHandler(this.udPreamp_ValueChanged);
        ...
        this.checkBoxPause.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.checkBoxPause.CheckedChanged
            += new EventHandler(this.checkBoxPause_CheckedChanged);
        //
        // btnStop
        //
        ...
        this.checkBoxPlay.TextAlign
            = System.Drawing.ContentAlignment.MiddleCenter;
        this.checkBoxPlay.CheckedChanged
            += new EventHandler(this.checkBoxPlay_CheckedChanged);
        //
        // menuStripOptions
        //
    ...
    private int CurrentlyPlaying {
        ...
        set {
            ...
            if (currently_playing == lstPlaylist.Items.Count - 1) {
                if (!checkBoxLoop.Checked) btnNext.Enabled = false;
            } else
...
#region Misc Routines
    ...
    //=======================================================
    // ke9ns add   rec/playid & waterID &  scheduler turn on post audio (called
    // from console)
    public bool RECPLAY {
        get { return false; }
    ...
    public bool RECPLAY2 {
        ...
        set {
            ...
            //  quickmp3SR = waveOptionsForm.comboSampleRate.Text; // save SR
            //  value before reducing waveOptionsForm.comboSampleRate.Text =
            //  "48000"; // reduce file size
        }
    } // RECPLAY2
    ...
    public bool RECPLAY3 {
        ...
        set {
            ...
            waveOptionsForm.radRXPreProcessed.Checked
                = waveOptionsForm.temp_record;
            waveOptionsForm.radTXPreProcessed.Checked
                = waveOptionsForm.temp_record;

            // waveOptionsForm.comboSampleRate.Text = quickmp3SR; // restore
            // file size SR
        }
    } // RECPLAY3
    ...
    private bool OpenWaveFile(string filename, int id) {
        ...
        if (!File.Exists(filename)) {
            MessageBox.Show("Filename doesn't exist. (" + filename + ")",
                "Bad Filename", MessageBoxButtons.OK, MessageBoxIcon.Error);
            if (currently_playing >= 0)
        ...
        try {
            reader = new BinaryReader(File.Open(
        ...
        if (reader.BaseStream.Length == 0 || reader.PeekChar() != 'R') {
            ...
            MessageBox.Show("File is not in the correct format.",
                "Wrong File Format", MessageBoxButtons.OK,
        ...
        if (fmt.channels != 2) {
            ...
            MessageBox.Show("Wave File is not stereo.",
                "Wrong Number of Channels", MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            file_list.RemoveAt(currently_playing);
            return false;
        }
        ...
        WaveThing.wave_file_reader[id] = new WaveFileReader1(id, this,
            (int)fmt.format, // use floating point
            (int)fmt.sample_rate, (int)fmt.channels, (int)fmt.bits_per_sample,
            ref reader);
...
#region Event Handlers
    ...
    private void openFileDialog1_FileOk(object sender, CancelEventArgs e) {
        foreach (string s in openFileDialog1.FileNames) {
            if (!file_list.Contains(s)) file_list.Add(s);
        }
    ...
    private void checkBoxPlay_CheckedChanged(object sender, EventArgs e) {
        if (checkBoxPlay.Checked) {
            ...
            if (console.RX2Enabled) {
                ...
                if (File.Exists(filename2)) OpenWaveFile(filename2, 1);
            }
    ...
    public static string
        scheduleName; // ke9ns add for saving file name of recording
    public static string
        scheduleName1; // ke9ns add for saving file name of recording
    public static string
        scheduleName2; // ke9ns add for saving file name of recording
    ...
    private void checkBoxRecord_CheckedChanged(object sender, EventArgs e) {
        if (checkBoxRecord.Checked) {
            ...
            WaveThing.wave_file_writer[0] = new WaveFileWriter(
                0, 2, waveOptionsForm.SampleRate, file_name);
            ...
            if (console.RX2Enabled) {
                ...
                WaveThing.wave_file_writer[1] = new WaveFileWriter(
                    1, 2, waveOptionsForm.SampleRate, file_name2);
            }
        }
        ...
        if (!checkBoxRecord.Checked) {
            ...
            // MessageBox.Show("The file has been written to the following
            // location:\n"+m_file_name);
        }
    }
    ...
    private void btnRemove_Click(object sender, EventArgs e) {
        if (lstPlaylist.Items.Count == 0
            || lstPlaylist.SelectedIndices.Count == 0)
            return;
        ...
        foreach (int i in lstPlaylist.SelectedIndices) {
            if (i == currently_playing && checkBoxPlay.Checked) {
                ...
                DialogResult dr = MessageBox.Show((string)lstPlaylist.Items[i]
                        + " is currently playing.\n"
                        + "Stop playing and remove from Playlist?",
                    "Stop and Remove?", MessageBoxButtons.YesNo,
    ...
    private void checkBoxPause_CheckedChanged(object sender, EventArgs e) {
        if (checkBoxPlay.Checked) Audio.WavePlayback = !checkBoxPause.Checked;
    ...
    private void udPreamp_ValueChanged(object sender, EventArgs e) {
        ...
        Audio.WavePreamp
            = Math.Pow(10.0, (int)udPreamp.Value / 20.0); // convert to scalar
    }
    ...
    //	to the proper value for recording off the air (and playing back over the
    //air) 	and then return these back to their original values at the end of the
    //invocation
    //  of these functions.
    //
    ...
    //
#pragma warning disable CS0414 // The field 'WaveControl.temp_record' is
                               // assigned but its value is
    // never used
    private readonly bool temp_record = false;
#pragma warning restore CS0414 // The field 'WaveControl.temp_record' is
                               // assigned but its value is
    // never used
    private bool temp_play = false;
    private bool temp_mon = false;
#pragma warning disable CS0414 // The field 'WaveControl.temp_pre' is assigned
                               // but its value is
    // never used
    private readonly byte temp_pre = 0; // ke9ns add for quickplay function
#pragma warning restore CS0414 // The field 'WaveControl.temp_pre' is assigned
                               // but its value is
    // never used
    ...
    private void chkQuickPlay_CheckedChanged(object sender, EventArgs e) {
        ...
        if (chkQuickPlay.Checked) {
            ...
            if (!OpenWaveFile(file_name, 0)) {
                ...
                Audio.RecordTXPreProcessed
                    = temp_play; // return to original state
    ...
    private void chkQuickRec_CheckedChanged(object sender, EventArgs e) {
        if (chkQuickRec.Checked) {
            ...
            //                 quickmp3SR =
            //                 waveOptionsForm.comboSampleRate.Text;
            //
            ...
            //                     + "QuickAudio"); // ke9ns add create sub
            //                     directory
            //                     System.IO.Directory.CreateDirectory(console.AppDataPath
            //                     + "QuickAudioMP3"); // ke9ns add create sub
            //                     directory
            //
            //                     m_file_name = console.AppDataPath +
            //                     "QuickAudio" + "\\SDRQuickAudio" +
            //                     QAC.ToString() + ".wav";
            //
            //                     quickmp3 = console.AppDataPath +
            //                     "QuickAudioMP3" + "\\SDRQuickAudio" +
            ...
            //
            //                     quickmp3 = console.AppDataPath +
            //                     "SDRQuickAudio.mp3"; // ke9ns add mp3
            //
            //                 }
        ...
        if (!chkQuickRec.Checked) {
            ...
            //{
            //    MessageBox.Show("The 'Over the Air' Quick audio recording has
            //    been successfully created.\n" +
            //        "Key the radio with either PTT or MOX and click on the
            //        Play button to play back the Quick audio recording over
            //        the air.");
            //    // MessageBox.Show("The file has been written to the following
            //    location:\n"+m_file_name);
            //}

            //                 Audio.RecordRXPreProcessed = temp_record;
            //                 //return to original state
            //                 waveOptionsForm.comboSampleRate.Text =
            //                 quickmp3SR; // restore file size
            ...
            //                         WaveFileReader(m_file_name)) // closes
            //                         reader when done using
            //                         //using (var writer = new
            //                         LameMP3FileWriter(quickmp3,
            //                         reader.WaveFormat, LAMEPreset.VBR_90)) //
            //                         closes writer when done using (90=90%
            ...
            //
            //                     }
            //                     Debug.WriteLine("DONE WITH MP3 CREATION" +
            //                     quickmp3);
            //
            //                 }
    ...
    //=========================================================================================
    //=========================================================================================
    // ke9ns add:  Play waterfall ID wave file here (below createBoxTS creates
    // the wave file)
    //=========================================================================================
    //=========================================================================================
    //=========================================================================================
    ...
    private void TXIDBoxTS_CheckedChanged(
        object sender, EventArgs e) // ke9ns ADD TX waterfall ID
    {

        string file_name
            = console.AppDataPath + "ke9ns.wav"; // TEXT to waterfall image only
        ...
        if (TXIDBoxTS.Checked) {
            ...
            // the wave file will have been created if the callsign text box is
            // green. The code should prevent you coming here if its not green.
            ...
            //    console.MON = temp_mon;
            //    Audio.RecordTXPreProcessed = temp_play; //return to original
            //    state console.TXEQ = temp_txeq;               // set TX Eq
            //    back to original state return;
            //}
        ...
        else // this is what signals the end of playback
        {
            ...
            // d   console.TXIDMenuItem.Checked = false;  // turn off TX
            // waterfall ID here

        } //  txidboxts not checked

        Audio.WavePlayback
            = TXIDBoxTS.Checked; // this trigger audio.cs to play audio this is
                                 // read in console.cs
        console.WavePlayback
            = TXIDBoxTS.Checked; // this triggers the audio playback

    } // TXIDBoxTS checkchanged

    //=========================================================================
    public bool CreatePlay // ke9ns ADD for TX waterfall ID  (comes from
                           // console.callsignTextBox)
    {
    ...
    //             when mouse moved in/out a thread routine starts which
    ...
    private void createBoxTS_CheckedChanged(object sender, EventArgs e) {
        if (createBoxTS.Checked) {
            ...
            if ((console.Callsign != console.LastCall)
                || (console.RX1DSPMode
                    != BandL)) // check if we need to create a new wave file or
                               // use the old one.
            {
                Thread t = new Thread(new ThreadStart(CreateWaterfallID)) { Name
                    = "Create Waterfall ID wave file Thread",
                    IsBackground = true, Priority = ThreadPriority.Normal };
                t.Start();
            } // console.Callsign != console.LastCall) || (console.RX1DSPMode !=
              // BandL))
            else {
                // dd console.callsignTextBox.BackColor =
                // Color.MediumSpringGreen;  // green if your last callsign is
                // still a valid wave console.menuStrip1.Invalidate();
    ...
    //============================================================================================
    //============================================================================================
    // ke9ns add   THREAD process to create wave file from text or bitmap image
    // here
    //============================================================================================
    //============================================================================================
    private void CreateWaterfallID() {
        ...
        string file_name
            = console.AppDataPath + "ke9ns.wav"; // TEXT to waterfall image only
        string file_name1
            = console.AppDataPath + "ke9ns.bmp"; // image file to waterfall

        // dd console.callsignTextBox.BackColor = Color.PaleVioletRed; // let
        // user know your creating a new wave file
        // console.menuStrip1.Invalidate(); console.menuStrip1.Update();

        // console.LastCall = console.Callsign;        // check if changed
        // callsign, mode, or sample rate samplesPerSecondL =
        // console.SampleRate1; BandL = console.RX1DSPMode;
        ...
        if (console.Callsign.EndsWith(".") == true) {
            ...
            file_name1 = console.AppDataPath + console.Callsign
                + "bmp"; //    image file to waterfall
        }
        ...
        if (IMAGE == 0) // text = 0
        {
            ...
            ke9ns_bmp = new Bitmap(
                xm, ym, PixelFormat.Format24bppRgb); // initialize bitmap for
                                                     // call sign insertion
            ...
            cl = g1.MeasureString(console.Callsign,
                new Font("Arial", fontS)); //  temp used to determine the size
                                           //  of the string when in LSB and you
                                           //  need to reserve a certain space
            ...
            } else // not vfoa
            {
                if (console.RX2DSPMode == DSPMode.LSB)
                    n4 = 1; //  for (int n = 1; n != xm; n=n+1)  // displays
                            //  right to left  (LSB)

            } // vfoA

            g1.DrawString(console.Callsign, new Font("Arial", fontS),
                Brushes.AntiqueWhite, cl.Width,
                cl.Height); // determine USB or LSB, then draw callsign into
                            // bitmap
            ...
            for (int y = ym - 1; y >= 0;
                 y--) // image is saved in correct direction
            {

                for (int n = 0; n != xm; n++) //
                {
                    // look at picture data and pick color based on index value
                    // found, AND invert so white = BLACK (no signal) , white =
                    // full signal (thats the 255- part) the /255 is to convert
                    // down to a

                    if (n4 == 0) // USB
                    {
                        Color pixel = ke9ns_bmp.GetPixel(
                            n, y); // bitmap is correct x direction but y is
                                   // backwards
                        ap[n, ym - y - 1] = (byte)(((bw * (float)pixel.B)
                            + (gw * (float)pixel.G) + (rw * (float)pixel.R)));
                    } else // LSB
                    {
                        Color pixel = ke9ns_bmp.GetPixel(
                            n, y); // Both x and y are backwards
                        ap[xm - n, ym - y - 1] = (byte)(((bw * (float)pixel.B)
                            + (gw * (float)pixel.G) + (rw * (float)pixel.R)));
                    }
                } // X
            ...
            FileStream stream1
                = new FileStream(file_name1, FileMode.Open); // open BMP  file
            BinaryReader reader = new BinaryReader(stream1);

            reader.ReadChars(10); // ignore BM characters to start bitmap header
            long hrdlen = reader.ReadInt32(); // offset in bitmap to first byte
                                              // of image data
            ...
            int bpp = reader.ReadInt16(); // reader.ReadBytes(2); // bpp assumed
                                          // to be 8bpp
            reader.ReadBytes(24); // ignore rest of header to get to color
                                  // values (255 in 8bpp set)
            ...
            if (color24 == 1) // 24bpp color grayscale (no color index needed)
            {
                     ...
                     y++) // image is saved in correct direction
                {
                    for (int n = 0; n != xm; n++) //
                    {
                        // look at picture data and pick color based on index
                        // value found, AND invert so white = BLACK (no signal)
                        // , white = full signal (thats the 255- part) the /255
                        // is to convert down to a

                        if (n4 == 0) // USB
                        {
                            ap[n, y] = (byte)(255
                                - (byte)((bw * (float)reader.ReadByte())
            ...
            else // 8bpp 256 color grayscale
            {
                ...
                ap = new byte[xm4 + 10,
                    ym + 10]; // get bitmap data
                              // ap1 = new float[xm4 + 10, ym + 10];  // convert
                              // to grayscale

                // color mapping
                for (int n = 0; n < 256; n++) {
                    col[n] = (byte)((bw * (float)reader.ReadByte())
                        + (gw * (float)reader.ReadByte())
                        + (rw * (float)reader.ReadByte()));
                ...
                } else // not vfoa
                {
                    if (console.RX2DSPMode
                        == DSPMode.LSB) //  for (int n = 1; n != xm; n=n+1)  //
                                        //  displays right to left  (LSB)
                    {
                        n1 = 1;
                        n2 = xm;
                        n3 = 1;
                        n4 = 1;
                    } else //  for (int n = (int)xm; n != 0; n--)  // displays
                           //  correctly left to right (USB)
                    {
                        n2 = 0;
                        n1 = xm;
                        n3 = -1;
                ...
                {
                    ri = reader.ReadBytes(
                        (int)xm4); // get entire line of bitmap X
                    ...
                    for (int n = n1; n != n2; n = n + n3) //
                    {
                        ...
                        {
                            ap[n1 - n, y]
                                = (byte)((byte)255 - col[ri[xm - n]]); // flip
                            if (ap[n1 - n, y] < 5) ap[n1 - n, y] = 0;

                        } else // LSB
                        {
                            ap[n, y] = (byte)((byte)255
                                - col[ri[xm - n]]); // dont flip
        ...
        FileStream stream
            = new FileStream(file_name, FileMode.Create); // create a wav file
        BinaryWriter writer = new BinaryWriter(
            stream); // create a stream to write to the wav file

        //=================================================
        // .wav header
        const int RIFF
            = 0x46464952; // 0x46464952  (0x52494646 big-endian form).
        const int WAVE
            = 0x45564157; // 0x45564157  (0x57415645 big-endian form).
        ...
        const int samplesPerSecond
            = 48000; // console.SampleRate1;    (fixed flex routine to resample
                     // audio to correct SR)
        const short bitsPerSample = 16; // 16

        short frameSize = (short)(tracks
            * ((bitsPerSample + 7) / 8)); // tracks 2.875  @ 8000 sps

        int bytesPerSecond
            = samplesPerSecond * frameSize; // 23000 bytes per second
        ...
        int t2 = 2; // divide the time for each line (this controls how long
                    // each line takes)

        if (IMAGE == 1) {
            float t3 = ((float)xm
                / (float)ym); // picture ratio to maintain in waterfall (80w x
                              // 80h = 1) (200w x 150h = 1.33) (80w x 20h = 4)
                              // float t4 = ((float)bandpass / (float)xm);  //
                              // number of freqs in width of images (200w = 13hz
                              // per pixel)longer time .076pix/hz,  (80w= 30hz
                              // per pixel)shorter time.033pix/hz
            t2 = (int)((float)xm * 2 / 20
                / t3); // t2 small means long vertical , t2 big means short (20
                       // is max)
        ...
        int samples = (samplesPerSecond * 2) * waveSize
            / t2; // file size               //88200 * 4 = 352800

        int dataChunkSize = samples * frameSize / 2; // 46000

        int fileSize = (waveSize) + headerSize + formatChunkSize + headerSize
            + dataChunkSize;
        ...
        //==============================================================
        // used if you want the waterfall to be as wide as your transmission
        // bandpass
        //  int lowtx = console.TXFilterLow;
        //  int hightx = console.TXFilterHigh;
        //   int tottx = hightx - lowtx; // band width

        int hzperpixel = (int)(((float)bandpass / (float)xm)
            + .5); // keep image 2khz wide always  25hz/pix

        int sample2 = samplesPerSecond
            / t2; // samples2 = the amount of time spent on each y1 line

        // ap[,] = vales between 0 and 1 (0=dark, 255=white)

        bright = bright / (float)xm; // correct brightness level of PCM audio by
                                     // how many freq points go into each pass
        ...
        for (int y1 = 0; y1 < ym;
             y1++) // each line (bottom of bitmap is first line out)
        {

            for (int n = 0; n < sample2; n++) //  (generate tone)
            {
                double t = (double)n
                    / (double)(samplesPerSecond * 2); // used to generate a tone
                ...
                for (float freq = lowtx; freq < (bandpass + lowtx); freq = freq
                         + hzperpixel) // add up all the frequencies from each
                                       // line(row) of the bitmap into 1 signal
                {
                    temp7 = temp7
                        + ((double)ap[i++, y1]
                            * (Math.Sin(t * (double)freq * 2.0
                                * Math.PI))); // generate individual tone
                ...
                writer.Write(
                    s); // left 16bits
                        //   writer.Write(s);  // right 16bits (this channel is
                        //   needed by Flex Quickplay routine)
        ...
        // dd console.callsignTextBox.BackColor = Color.MediumSpringGreen;  //
        // green if you created it or its still a valid wave
        // console.menuStrip1.Invalidate(); console.menuStrip1.Update();
...
public class Chunk {
    ...
    public static Chunk ReadChunk(ref BinaryReader reader) {
        ...
        {
            RIFFChunk riff = new RIFFChunk { chunk_id = data,
                file_size = reader.ReadInt32(),
                riff_type = reader.ReadInt32() };
            return riff;
        } else if (data == 0x20746D66) // fmt chunk
        {
            fmtChunk fmt = new fmtChunk { chunk_id = data,
                chunk_size = reader.ReadInt32(), format = reader.ReadInt16(),
                channels = reader.ReadInt16(), sample_rate = reader.ReadInt32(),
        ...
        {
            dataChunk data_chunk = new dataChunk { chunk_id = data,
                chunk_size = reader.ReadInt32() };
            return data_chunk;
        } else {
            Chunk c = new Chunk { chunk_id = data };
            return c;
        }
    }
}
...
unsafe public class WaveFileWriter {
    ...
    private UInt32 length_counter; // klj: was int, failed over 2GB size! Proper
                                   // use of unsigned gets you to 4GB
    ...
    public WaveFileWriter(int wfw_id, short chan, int samp_rate, string file) {
        id = wfw_id;
        // get rates and sizes for recording the receiver and recording the
        // transmitter pre/post choices must have been selected in advance of
        // clicking record it would be good to disable the pre/post buttons as
        // soon as record is clicked
        ...
        int OUT_BLOCK = (int)Math.Ceiling(IN_BLOCK * (double)sample_rate
            / (double)Math.Min(rcvr_size, xmtr_size));
        ...
        Thread t = new Thread(new ThreadStart(ProcessRecordBuffers)) { Name
            = "Wave File Write Thread",
            IsBackground = true, Priority = ThreadPriority.Normal };
        t.Start();
    }
    ...
    private void ProcessRecordBuffers() {
        WriteWaveHeader(
            ref writer, channels, sample_rate, format_tag, bit_depth, 0);
        ...
        WriteWaveHeader(ref writer, channels, sample_rate, format_tag,
            bit_depth, length_counter);
        writer.Flush();
        writer.Close();
    }
    ...
    private void WriteBuffer(ref BinaryWriter writer, ref UInt32 count) {
        ...
        switch (bit_depth) {
            case 32: Write_32(length, ref count, out_cnt); break;
            case 24: Write_24(length, ref count, out_cnt); break;
            case 16: Write_16(length, ref count, out_cnt); break;
            case 8: Write_8(length, ref count, out_cnt); break;
        }
    }

    private static Random rnd
        = new Random(); // generates values between 0.0 and 1.0
    private void Write_32(int length, ref UInt32 count, int out_cnt) {
        ...
        for (int i = 0; i < length; i++) {
            switch (format_tag) {
                ...
                case 1:
                    if (dither) {
                        ...
                        byte_buf[i * 4 + 2]
                            = (byte)(((uint)intSample >> 16) & 0xFF);
                        byte_buf[i * 4 + 1]
                            = (byte)(((uint)intSample >> 8) & 0xFF);
            ...
            if (!dither) {
                ...
                for (int j = 0; j < 4; j++) byte_buf[i * 4 + j] = temp[j];
            }
        }
    ...
    private void Write_8(int length, ref UInt32 count, int out_cnt) {
        ...
        for (int i = 0; i < length; i++) {
            byte_buf[i]
                = (byte)(dither8(out_buf[i] * 0x80) + 128); // 128.0f) + 128);
    ...
    private void WriteWaveHeader(ref BinaryWriter writer, short channels,
        int sample_rate, short format_tag, short bit_depth,
        UInt32 data_length) {
        ...
        writer.Write(
            (int)(channels * sample_rate * bit_depth / 8)); // dwAvgBytesPerSec
...
unsafe public class WaveFileReader1 {
    ...
    public WaveFileReader1(int wfr_id, WaveControl form, int fmt, int samp_rate,
        int chan, int bit_depth, ref BinaryReader binread) {
        id = wfr_id;
        wave_form = form;
        format = fmt;
        sample_rate = samp_rate;
        ...
        // OUT_BLOCK = 2048;
        // IN_BLOCK =
        // (int)Math.Floor(OUT_BLOCK*(double)sample_rate/Audio.SampleRate1);
        // OUT_BLOCK =
        // (int)Math.Ceiling(IN_BLOCK*Audio.SampleRate1/(double)sample_rate);
        IN_BLOCK = 2048;
        // OUT_BLOCK =
        // (int)Math.Ceiling(IN_BLOCK*Audio.SampleRate1/(double)sample_rate);
        OUT_BLOCK = (int)Math.Ceiling(
            IN_BLOCK * (double)max_rate / (double)sample_rate);
        ...
        switch (format) {
            case 1:
                switch (bitdepth) {
                    case 32: io_buf_size = IN_BLOCK * 4 * 2; break;
                    case 24: io_buf_size = IN_BLOCK * 3 * 2; break;
                    case 16: io_buf_size = IN_BLOCK * 2 * 2; break;
                    case 8: io_buf_size = IN_BLOCK * 2; break;
                }
                break;
            case 3:
          ...
          {
                     resamp_l = DttSP.NewResamplerF(sample_rate,
          Audio.SampleRate1); if (channels > 1) resamp_r =
          DttSP.NewResamplerF(sample_rate, Audio.SampleRate1);
                 } */
                break;
        }
        ...
        Thread t = new Thread(
            new ThreadStart(ProcessBuffers)) { Name = "Wave File Read Thread",
            IsBackground = true, Priority = ThreadPriority.Normal };
    ...
    private void ProcessBuffers() {
        while (playback == true) // while we didn't end-of-file the last time
                                 // through, loop here
        { //   'playback' also gets set 'false' if play button is UNchecked
            while (rb_l.WriteSpace() >= OUT_BLOCK
                && !eof) // while there's lots of writespace and we haven't
                         // ended the file, loop here
            {
                ReadBuffer(ref reader);
            ...
            Thread.Sleep(10); // if we haven't ended the file, but, there was
                              // not lots of write space,
    ...
    private void ReadBuffer(ref BinaryReader reader) {
        ...
        int val = reader.Read(io_buf, 0,
            io_buf_size); // read byte data into 'io_buf'
                          // note:  io_buf has been sized to hold IN_BLOCK L&R
                          // samples
        if (val
            < io_buf_size) // 'if' there wasn't enough data to fill the buffer
        {
            eof = true; // set the END-OF-FILE flag; this is the last time we
                        // need to call ReadBuffer(...)
            switch (format) {
                case 1: // ints
                    switch (bitdepth) {
                        case 32: num_reads = val / 8; break;
                        case 24: num_reads = val / 6; break;
                        case 16: num_reads = val / 4; break;
                        case 8: num_reads = val / 2; break;
                    }
                    break;
                case 3: // floats
                    num_reads = val
                        / 8; // 'num_reads' is the number of L&R samples read
                    break;
            }
        }
                ...
                case 1: // ints
                    switch (bitdepth) {
                        case 32: // 32-bit signed integer
                                 //  buf_l_in[i] =
                                 //  (float)((double)BitConverter.ToInt32(io_buf,
                                 //  i * 8) / 2147483648.0); buf_r_in[i] =
                                 //  (float)((double)BitConverter.ToInt32(io_buf,
                                 //  i * 8 + 4) / 2147483648.0);
                        ...
                        case 16: // 16-bit signed integer
                            buf_l_in[i] = (float)((double)BitConverter.ToInt16(
                                                      io_buf, i * 4)
                                / 32767.0);
                            buf_r_in[i] = (float)((double)BitConverter.ToInt16(
                                                      io_buf, i * 4 + 2)
                                / 32767.0);
                            break;
                        case 8: // 8-bit unsigned integer
                            buf_l_in[i]
                                = ((io_buf[i * 2] & 0xFF) - 128) / 128.0f;
                            buf_r_in[i]
                                = ((io_buf[i * 2 + 1] & 0xFF) - 128) / 128.0f;
                            break;
                    }
                    break;
                case 3: // floats
                    buf_l_in[i] = BitConverter.ToSingle(
                        io_buf, i * 8); // put samples in 'float' buffers,
                                        // buf_l_in[] and buf_r_in[]
        ...
        if (num_reads < IN_BLOCK) //'if' there are not 'IN_BLOCK'samples
        {
            for (int j = i; j < IN_BLOCK; j++)
                buf_l_in[j] = buf_r_in[j]
                    = 0.0f; // pad to 'IN_BLOCK' samples with zeros

            playback = false; // set 'playback' to false; we've done our last
                              // read from this file
        ...
        if (!Audio.MOX) // 'true' means data is headed for a receiver channel
        {
            if (sample_rate != rcvr_rate) // 'if' we need to resample
            {
                fixed(float* in_ptr = &buf_l_in[0], out_ptr = &buf_l_out[0])
                    WDSP.xresampleFV(
                        in_ptr, out_ptr, IN_BLOCK, &out_cnt, rcvr_resamp_l);
                if (channels > 1) {
                    fixed(float* in_ptr = &buf_r_in[0], out_ptr = &buf_r_out[0])
                        WDSP.xresampleFV(
                            in_ptr, out_ptr, IN_BLOCK, &out_cnt, rcvr_resamp_r);
                }
            } else {
                buf_l_in.CopyTo(
                    buf_l_out, 0); // whether we resample or not, data goes to
                                   // 'buf_l_out[]' & 'buf_r_out[]'
                buf_r_in.CopyTo(
                    buf_r_out, 0); // we end up with 'out_cnt' samples
            }
        } else // data is headed for the transmitter channel input
        {
            if (sample_rate != xmtr_rate) // 'if' we need to resample
            {
                fixed(float* in_ptr = &buf_l_in[0], out_ptr = &buf_l_out[0])
                    WDSP.xresampleFV(
                        in_ptr, out_ptr, IN_BLOCK, &out_cnt, xmtr_resamp_l);
                if (channels > 1) {
                    fixed(float* in_ptr = &buf_r_in[0], out_ptr = &buf_r_out[0])
                        WDSP.xresampleFV(
                            in_ptr, out_ptr, IN_BLOCK, &out_cnt, xmtr_resamp_r);
                }
            } else {
                buf_l_in.CopyTo(
                    buf_l_out, 0); // whether we resample or not, data goes to
        ...
        // lock (ringLock)
        {
            rb_l.Write(buf_l_out,
                out_cnt); // write the 'out_cnt' samples into the ring buffers
                          // 'rb_l' and 'rb_r'
            if (channels > 1)
            ...
            total_samps_written
                += out_cnt; // sum the total samples written to the ring(s)
        }
    }
    ...
    unsafe public void GetPlayBuffer(float* left, float* right) {
        ...
        if (count < size) {
            for (int i = count; i < size; i++) left[i] = right[i] = 0.0f;
        }
        total_samps_read += size; // sum the total samples read from the ring(s)
        if (total_samps_read
            >= total_samps_written) // check to see if we're done with playback
        {
            Thread t = new Thread(new ThreadStart(NextPlayback)) { Name
                = "Wave File Next Playback",
                IsBackground = true, Priority = ThreadPriority.Normal };
            t.Start();
        }
    }
