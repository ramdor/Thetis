...
#region Standard CAT Methods A - F
    ...
    public string AG(string s) {
        ...
        } else if (s.Length == parser.nGet) // if this is a read command
        {
            ...
            //				return AddLeadingZeros(console.AF);		// Get the
            //console setting
    ...
    public string DN() {
        ...
        //			wheel_tune_list = new double[13];		// initialize wheel
        //tuning list array 			wheel_tune_list[0]  =  0.000001;
        // wheel_tune_list[1]  = 0.000010; 			wheel_tune_list[2]  =
        // 0.000050; 			wheel_tune_list[3]  = 0.000100;
        // wheel_tune_list[4]  =  0.000250; 			wheel_tune_list[5]  =
        // 0.000500; 			wheel_tune_list[6]  =  0.001000; 			wheel_tune_list[7]
        // = 0.005000; 			wheel_tune_list[8]  =  0.009000;
        // wheel_tune_list[9]  = 0.010000; 			wheel_tune_list[10] =
        // 0.100000; 			wheel_tune_list[11] =  1.000000;
    ...
    public string FA(string s) {
        //			if(s.Length == parser.nSet)
        //			{
        //				s = s.Insert(5,separator);		//reinsert the global
        //numeric separator 				console.VFOAFreq = double.Parse(s);
        // return "";
        //			}
        //			else if(s.Length == parser.nGet)
        //				return StrVFOFreq("A");
...
#region Standard CAT Methods G - M
    ...
    public string IF() {
        ...
        if (retval) split = "1";
        // Get the mode
        //			temp = Mode2KString(console.RX1DSPMode);   //possible fix
        //for
        ...
        rtn += f;
        //			rtn += StrVFOFreq("A");						// VFO A frequency
        //11 bytes
        ...
        //			rtn += temp;
        //			rtn += Mode2KString(console.RX1DSPMode);	// current mode
        // 1 bytes
        ...
        //			cmd_string += rit+xit;
        //
        //			// Skip the memory channel stuff, the SDR1K doesn't use
        //banks and channels per se
        //			// (P6 - 1 byte, P7 - 2 bytes)
        //			cmd_string += "000";
        //
        //			// Set the current MOX state (P8 - 1 byte)(what the heck is
        //this
        // for?) 			if(console.MOX) 				cmd_string += "1"; 			else
        // cmd_string += "0";
        //
        //			// Get the SDR mode.  (P9 - 1 byte)
        //			temp = Mode2KString(console.RX1DSPMode);
        //			if(temp.Length == 1)	// if the answer is not an error
        //message
        //?; 				cmd_string += temp; 			else 				cmd_string += " ";
        //// return a blank if it's an error
        //
        ...
        //			// The Scan code might be implemented but the frequency
        //range would
        ...
        //			cmd_string += ZZSP("");
        //
        //			// Set the remaining CTCSS tone and shift bits to 0 (P13 -
        //P15, 4 bytes) 			cmd_string += "0000";
        //
        //			return cmd_string;
    }
...
#region Extended CAT Methods ZZA - ZZF
    ...
    public string ZZAR(string s) {
        ...
        if (s.Length == parser.nSet) {
            if (console.RF != n && console.AutoAGCRX1)
                console.AutoAGCRX1 = false; // turn off 'auto agc' only if
    ...
    public string ZZAS(string s) {
        ...
        if (s.Length == parser.nSet) {
            if (console.RX2RF != n && console.AutoAGCRX2)
                console.AutoAGCRX2 = false; // turn off 'auto agc' only if
                                            // different MW0LGE_21k8
            console.RX2RF = n;
    ...
    public string ZZFM() {
        ...
        if (radio == "HPSDR" || radio == "HERMES") {
            ...
            || radio == "ANAN7000D"
            || radio == "ANAN8000D") // DH1KLM_21a added 7000D
            return "1";
        else
            return parser.Error1;
    }
...
#region Extended CAT Methods ZZR - ZZZ
    ...
    public string ZZRY(string s) {
                                     ...
                                     // setting this prefix
        {
            ...
            att = Math.Max(0, att);
            att = Math.Min(31, att);
            console.SetupForm.HermesAttenuatorDataRX2
                = att; // RX2ATT = att;        // Set the console control //
                       // MW0LGE_21d step atten changes
        ...
        } else if (s.Length == parser.nGet) // if this is a read command
        {
            return AddLeadingZeros(
                console.SetupForm
                    .HermesAttenuatorDataRX2 /*RX2ATT*/); // Get the console
                                                          // setting //
                                                          // MW0LGE_21d step
                                                          // atten changes
        } else {
    ...
    public string ZZTO(string s) {
        ...
        if (s.Length == parser.nSet) // check the min/max control settings
        {
            ...
            switch (console.TuneDrivePowerOrigin) {
                case DrivePowerSource.DRIVE_SLIDER: console.PWR = tl; break;
                case DrivePowerSource.TUNE_SLIDER: console.TunePWR = tl; break;
    ...
    public string ZZUP(string s) //-MW0LGE_21j  xPA on/off
    {
        if (s.Length == parser.nSet && (s == "0" || s == "1")) {
            switch (s) {
                case "0": console.CATxPA = false; break;
                case "1": console.CATxPA = true; break;
            }
    ...
    public string ZZVB(string s) {
        ...
        if (s != "") {
            n = Convert.ToInt32(s);
            n = Math.Max(-40, n);
            n = Math.Min(40, n); // DH1KLM_21g Changed value from 20 to 40 like
                                 // it is in VAC2 RX Gain
        }
    ...
    public string ZZZM(string s) {
        if (s.Length == parser.nGet) {
            // add command to the return string and terminator, because it is
...
#region Band Methods
    ...
    private void BandUp() {
        ...
        Debug.Print("BandUp: current=" + current.ToString()
            + " next=" + nextband.ToString());
    }
    ...
    private void BandDown() {
        ...
        Debug.Print("BandDown: current=" + current.ToString()
            + " next=" + nextband.ToString());
        console.SetCATBand(nextband);
    }
