...
namespace Thetis
{
    ...
    public sealed class clsAudioRecordPlayback : IDisposable
    {
        ...
        private static bool looksLikeFilenameOnly(string path)
        {
            if (string.IsNullOrWhiteSpace(path)) return false;
            string p = path.Trim();
            if (containsDirectorySeparator(p)) return false;
        ...
        private string resolveRecordPath(string record_id, string full_path, string prefix, out string filename, out string error)
        {
            filename = null;
            error = null;

            string p = null;
            if (!string.IsNullOrWhiteSpace(full_path)) p = full_path.Trim();
            ...
            if (looksLikeFilenameOnly(p))
            {
                ...
                string relFolder = Path.Combine(_audio_folder, p);
                return makeUniquePathInFolder(relFolder, prefix, out filename);
            }

            if (Path.IsPathRooted(p) || containsDirectorySeparator(p))
            {
                if (!hasTrailingSeparator(p) && hasExtension(p))
                {
        ...
        private string resolvePlayPath(string full_path, out string error)
        {
            error = null;
...
//            _playbackSetting = new Dictionary<string, bool>();
//            _PrePlaybackSetting = new Dictionary<string, bool>(); // temp copy of current config, restored after play finished
...
//        private void OnPreMox(int rx, bool oldMox, bool newMox)
//        {
//            // if mox changes, and we are playing/recording, then stop
//            if(oldMox != newMox)
//            {
//                try { StopRecord(out string _); } catch { }
//                try { StopPlayback(out string _); } catch { }
...
//            }
//        }
//        public string RecordToFileFromWDSP(string folder, int wfw_id, out string error, string full_file_path = null, bool remove_if_file_exists = false)
//        {
//            error = null;
...
//                    {
//                        string safe_folder = folder ?? string.Empty;
//                        string folder_path = Path.Combine(_audio_folder, safe_folder);
...
//                            full_path = Path.Combine(folder_path, filename);
...
//                    {
//                        try { File.Delete(full_path); } catch { }
//                    }
...
//                    _active_record_filename = full_path;
...
//                    WaveThing.wave_file_writer[_active_record_wfw_id] = new WaveFileWriter(_active_record_wfw_id, 2, SampleRate, full_path, recRXPreProc, recTXPreProc, formatTag, bitDepth);
//                    WaveThing.wave_file_writer[_active_record_wfw_id].DitherEnabled = DitherEnabled;
//                    WaveThing.wave_file_writer[_active_record_wfw_id].DitherAmount = DitherAmount;
...
//        public string RecordToFileFromPCAudio(string folder, int pcAudioDeviceInputId, out string error, string full_file_path = null, bool remove_if_file_exists = false)
//        {
//            error = null;
...
//            }
//        }
//        public bool PlayFileViaWDSP(string folder, string filename, int wfw_id, out string error, string full_file_path = null)
//        {
//            bool ret = false;
//            bool restore = false;
//            error = null;
...
//                    string safe_folder = folder ?? string.Empty;
//                    string folder_path = Path.Combine(_audio_folder, safe_folder);
//                    string fullPath = string.IsNullOrEmpty(full_file_path) ? Path.Combine(folder_path, filename) : full_file_path;
...
//            return ret;
//        }
//        public bool PlayFileViaPCAudio(string folder, string filename, int pcAudioDeviceOutputId, out string error, string full_file_path = null)
//        {
//            bool ret = false;
//            bool restore = false;
//            error = null;
