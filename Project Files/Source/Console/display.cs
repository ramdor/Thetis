...
#region Variable Declaration
    ...
    private static SharpDX.Direct2D1.Bitmap _waterfall_bmp_dx2d
        = null; // MW0LGE
...
#region Properties
    ...
    public static DisplayMode CurrentDisplayModeBottom {
        ...
        set {
            ...
            if (bDifferent) {
                ...
                if (value == DisplayMode.PANAFALL
                    || value == DisplayMode.WATERFALL)
                    ResetWaterfallBmp2();
            }
        }
    }
    ...
    private static void OnBandChangeHandler(
        int rx, Band oldBand, Band newBand) {
        if (rx == 1)
            FastAttackNoiseFloorRX1 = true;
        else
            FastAttackNoiseFloorRX2 = true;
    ...
    // private static HiPerfTimer m_objBlobsActivePeakDisplayDelayTimer = new
    // HiPerfTimer();
    private static void processBlobsActivePeakDisplayDelay() {
        if (!m_bDelayRX1Blobs && !m_bDelayRX2Blobs && !m_bDelayRX1SpectrumPeaks
            && !m_bDelayRX2SpectrumPeaks)
            return;
    ...
    private static void OnAttenuatorDataChanged(
        int rx, int oldAtt, int newAtt) {
        if (rx == 1)
            FastAttackNoiseFloorRX1 = true;
        else
            FastAttackNoiseFloorRX2 = true;
    }
    private static void OnPreampModeChanged(
        int rx, PreampMode oldMode, PreampMode newMode) {
        if (rx == 1)
            FastAttackNoiseFloorRX1 = true;
        else
            FastAttackNoiseFloorRX2 = true;
    }
    private static void OnCentreFrequencyChanged(
        int rx, double oldFreq, double newFreq, Band band) {
        if (rx == 1) {
    ...
    public static Control Target {
        ...
        set {
            lock (_objDX2Lock) {
                ...
                if (specready) {
                    console.specRX.GetSpecRX(0).Pixels
                        = displayTargetWidth / m_nDecimation;
                    console.specRX.GetSpecRX(1).Pixels
                        = displayTargetWidth / m_nDecimation;
    ...
    // DisplayEngine.DIRECT_X;//MW0LGE_21g gdi+ DisplayEngine.GDI_PLUS; public
    // static DisplayEngine CurrentDisplayEngine {
    ...
    public static bool RebuildLinearGradientBrushRX1 {
        ...
        set { _bRebuildRX1LinearGradBrush = value; }
    }
    ...
    public static bool RebuildLinearGradientBrushRX2 {
        ...
        set { _bRebuildRX2LinearGradBrush = value; }
    }
    private static bool
    ...
    public static int SpectrumGridMaxMoxModified {
        get {
            bool local_mox
                = mox && (!tx_on_vfob || (tx_on_vfob && !rx2_enabled));
            if (local_mox)
    ...
    public static Color TXDataLineColor {
        ...
        set {
            lock (_objDX2Lock) {
                ...
                tx_data_line_fpen.Color
                    = Color.FromArgb(100, tx_data_line_color);
                buildDX2Resources();
            }
        }
    }
    ...
    private static Color bandstack_overlay_color
        = Color.FromArgb(192, 192, 64, 0);
    private static SolidBrush bandstack_overlay_brush
        = new SolidBrush(bandstack_overlay_color);
    private static SolidBrush bandstack_overlay_brush_lines = new SolidBrush(
        Color.FromArgb(Math.Min(bandstack_overlay_color.A + 64, 255),
            bandstack_overlay_color));
    private static SolidBrush bandstack_overlay_brush_highlight
        = new SolidBrush(
            Color.FromArgb(Math.Min(bandstack_overlay_color.A + 64, 255),
                bandstack_overlay_color));
    public static Color BandstackOverlayColor {
        ...
        set {
            lock (_objDX2Lock) {
                ...
                bandstack_overlay_brush_lines.Color = Color.FromArgb(
                    Math.Min(bandstack_overlay_color.A + 64, 255),
                    bandstack_overlay_color);
                bandstack_overlay_brush_highlight.Color = Color.FromArgb(
                    Math.Min(bandstack_overlay_color.A + 64, 255),
                    bandstack_overlay_color);
                buildDX2Resources();
            }
    ...
    private static System.Drawing.Font font10
...
#region GDI + General Routines
    ...
    private static void initDisplayArrays(int W, int H) {
        lock (_objDX2Lock) {
            ...
            // Parallel.For(0, W, (i) => //for (int i = 0; i <
            // displayTargetWidth; i++)
            //{
            //     histogram_data[i] = Int32.MaxValue;
            //     histogram_history[i] = 0;
            // });
        ...
        lock (_objDX2Lock) {
            if (_bDX2Setup) {
                ...
                if (_waterfall_bmp_dx2d != null
                    && !_waterfall_bmp_dx2d.IsDisposed) {
                    if (displayTargetWidth == _waterfall_bmp_dx2d.Size.Width) {
                        // make copy only if widths equal
                        int h = Math.Min(
                            H - 20, (int)_waterfall_bmp_dx2d.Size.Height);

                        tmp = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget,
                            new Size2((int)_waterfall_bmp_dx2d.Size.Width,
                                h /*(int)_waterfall_bmp_dx2d.Size.Height*/),
                            new BitmapProperties(new SDXPixelFormat(
                                Format.B8G8R8A8_UNorm, _ALPHA_MODE)));

                        tmp.CopyFromBitmap(_waterfall_bmp_dx2d,
                ...
                _waterfall_bmp_dx2d
                    = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget,
                ...
                if (tmp != null) {
                    ...
                    _waterfall_bmp_dx2d.CopyFromBitmap(tmp,
                        new SharpDX.Point(
        ...
        lock (_objDX2Lock) {
            if (_bDX2Setup) {
                ...
                if (_waterfall_bmp2_dx2d != null
                    && !_waterfall_bmp2_dx2d.IsDisposed) {
                    if (displayTargetWidth == _waterfall_bmp2_dx2d.Size.Width) {
                        // make copy only if widths equal
                        int h = Math.Min(
                            H - 20, (int)_waterfall_bmp2_dx2d.Size.Height);

                        tmp = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget,
                            new Size2((int)_waterfall_bmp2_dx2d.Size.Width,
                                h /*(int)_waterfall_bmp2_dx2d.Size.Height*/),
                            new BitmapProperties(new SDXPixelFormat(
                                Format.B8G8R8A8_UNorm, _ALPHA_MODE)));

                        tmp.CopyFromBitmap(_waterfall_bmp2_dx2d,
                            new SharpDX.Point(0, 0),
                            new SharpDX.Rectangle(0, 0, (int)tmp.Size.Width,
                                (int)tmp.Size.Height));
                        //
                ...
                if (tmp != null) {
                    ...
                    // copy old waterfall into new bitmap
                    _waterfall_bmp2_dx2d.CopyFromBitmap(tmp,
                        new SharpDX.Point(
                            0, 0)); // anything outside will be 'ignored'
                    Utilities.Dispose(ref tmp);
                    tmp = null;
...
#region DirectX
    ...
    public static void ShutdownDX2D() {
        lock (_objDX2Lock) {
            ...
            try {
                ...
                if (_device != null && _device.ImmediateContext != null) {
                    SharpDX.Direct3D11.DeviceContext dc
                ...
                MessageBox.Show("Problem Shutting Down DirectX !"
        ...
        lock (_objDX2Lock) {
            ...
            try {
                DeviceCreationFlags debug
                    = DeviceCreationFlags
                          .None; //.Debug;  //MW0LGE_21k9 enabled debug to
                                 //obtain list of objects that are still
                                 //referenced
                ...
                // you need to enable operating system support in the
                // app1.manifest file, otherwise majVers will not report 10+
                // note: windows 10, 11, server 2016, server 2019, server 2022
                // all use the windows 10 os id in the manifest file at this
                // current time
                ...
                if (majVers >= 10) // win10 + 11
                {
                    featureLevels = new SharpDX.Direct3D.FeatureLevel[] {
                        ...
                        SharpDX.Direct3D.FeatureLevel
                            .Level_11_1, // windows 8 and up
                        SharpDX.Direct3D.FeatureLevel
                            .Level_11_0, // windows 7 and up (level 11 was only
                                         // partial on 7, not 11_1)
                    ...
                    && minVers >= 2) // windows 8, windows 8.1
                {
                    featureLevels = new SharpDX.Direct3D.FeatureLevel[] {
                        SharpDX.Direct3D.FeatureLevel
                            .Level_11_1, // windows 8 and up
                    ...
                    && minVers < 2) // windows 7, 2008 R2, 2008, vista
                {
                    featureLevels = new SharpDX.Direct3D.FeatureLevel[] {
                        SharpDX.Direct3D.FeatureLevel
                            .Level_11_0, // windows 7 and up (level 11 was only
                ...
                SharpDX.DXGI.Device1 device1
                    = _device.QueryInterfaceOrNull<SharpDX.DXGI.Device1>();
                if (device1 != null) {
                ...
                // this code should ideally be used to prevent use of flip if
                // vsync is 0 but is not used at this time SharpDX.DXGI.Factory5
                // f5 = factory.QueryInterfaceOrNull<SharpDX.DXGI.Factory5>();
                ...
                //    f5.CheckFeatureSupport(SharpDX.DXGI.Feature.PresentAllowTearing,
                //    pBool, size);
                ...
                SharpDX.DXGI.Factory4 factory4
                    = _factory1.QueryInterfaceOrNull<SharpDX.DXGI.Factory4>();
                ...
                swapEffect = bFlipPresent
                    ? SwapEffect.FlipDiscard
                    : SwapEffect.Discard; // NOTE: FlipSequential should work,
                                          // but is mostly used for storeapps
                ...
                // multiSample) - 1; maxQuality = Math.Max(0, maxQuality);

                ModeDescription md = new ModeDescription(displayTarget.Width,
                    displayTarget.Height, new Rational(console.DisplayFPS, 1),
                ...
                SwapChainDescription desc = new SwapChainDescription() {
                    ...
                    // SampleDescription = new SampleDescription(multiSample,
                    // maxQuality),
                    SampleDescription = new SampleDescription(
                        1, 0), // no multi sampling (1 sample), no antialiasing
                    SwapEffect = swapEffect,
                    Usage
                    = Usage.RenderTargetOutput, // | Usage.BackBuffer,  // dont
                                                // need usage.backbuffer as it
                                                // is implied
                    Flags = SwapChainFlags.None,
                };

                _factory1.MakeWindowAssociation(
                    displayTarget.Handle, WindowAssociationFlags.IgnoreAll);
    ...
    public static void ResetDX2DModeDescription() {
        ...
        try {
            lock (_objDX2Lock) {
                ...
                // MW0LGE_21k9 must resize the back buffers, belts and braces
                // because width/height not likely to change
                ResizeDX2D();
            }
        } catch (Exception) {
        }
    ...
    public static void ResizeDX2D() {
        try {
            lock (_objDX2Lock) {
                ...
                _swapChain1.ResizeBuffers(_nBufferCount, displayTargetWidth,
                    displayTargetHeight,
                    _swapChain.Description.ModeDescription.Format,
                    SwapChainFlags.None);
    ...
    private static void setupAliasing() {
        lock (_objDX2Lock) {
            ...
            if (m_bAntiAlias)
                _d2dRenderTarget.AntialiasMode
                    = AntialiasMode.PerPrimitive; // this will antialias even if
                                                  // multisampling is off
            else
                _d2dRenderTarget.AntialiasMode
                    = AntialiasMode
                          .Aliased; // this will result in non antialiased lines
                                    // only if multisampling = 1
    ...
    public static void RenderDX2D() {
        try {
            lock (_objDX2Lock) {
                ...
                _bNoiseFloorAlreadyCalculatedRX1
                    = false; // keeps track of noise floor processing, only want
                             // to do it once, even if pana + water shown
                ...
                if (_bRebuildRX1LinearGradBrush
                    ...
                    if (_bRebuildRX1LinearGradBrush) {
                        // int s9PixelPos = GetDBMPixelPos(0, -73, 1,
                    ...
                    if (_bRebuildRX2LinearGradBrush) {
                        ...
                        // int s9PixelPos = GetDBMPixelPos(nVertShift, -73, 2,
                        // m_nRX2DisplayHeight, false);
                        buildLinearGradientBrush(
                            nVertShift, tmpHeightRX2 + nVertShift, 2);
                ...
                if (!split_display) {
                    ...
                    switch (current_display_mode) {
                        case DisplayMode.SPECTRUM:
                            DrawSpectrumDX2D(1, displayTargetWidth,
                                m_nRX1DisplayHeight, false);
                            break;
                        case DisplayMode.PANADAPTER:
                ...
                // for linear grad brush rebuilding. Do these all the time, even
                // if m_bUseLinearGradient=false, so we can rebuild if need be
                ...
                if (m_bShowGetPixelsIssue && m_bGetPixelsIssue)
                    _d2dRenderTarget.FillRectangle(
                        new RectangleF(0, 8, 8, 8), m_bDX2_Yellow);

                calcFps();
                if (m_bShowFPS)
                    _d2dRenderTarget.DrawText(m_nFps.ToString(),
                        fontDX2d_callout,
                ...
                // some debug text
                if (m_sDebugText != "") {
                    string[] lines = m_sDebugText.Split(
                        new[] { Environment.NewLine }, StringSplitOptions.None);
                    int xStartX = 32;
                    for (int i = 0; i < lines.Length; i++) {
                        _d2dRenderTarget.DrawText(lines[i].ToString(),
                            fontDX2d_callout,
                            new RectangleF(64, xStartX, float.PositiveInfinity,
                                float.PositiveInfinity),
                            m_bDX2_m_bTextCallOutActive, DrawTextOptions.None);
                        xStartX += 12;
                    }
                ...
                // render
                // note: the only way to have Present non block when using vsync
                // number of blanks 0 , is to use DoNotWait however the gpu will
                // error if it is busy doing something and the data can not be
                // queued It will error and just ignore everything, we try
                // present and ignore the 0x887A000A error
                PresentFlags pf
                    = m_nVBlanks == 0 ? _NoVSYNCpresentFlag : PresentFlags.None;
                Result r = _swapChain1.TryPresent(m_nVBlanks, pf);

                if (r != Result.Ok && r != 0x887A000A) {
                    string sMsg = "";
                    if (r == 0x887A0001)
                        sMsg = "Present Device Invalid Call"
                            + Environment.NewLine + "" + Environment.NewLine
                            + "[ " + r.ToString()
                            + " ]"; // DXGI_ERROR_INVALID_CALL
                    if (r == 0x887A0007)
                        sMsg = "Present Device Reset" + Environment.NewLine + ""
                            + Environment.NewLine + "[ " + r.ToString()
                            + " ]"; // DXGI_ERROR_DEVICE_RESET
                    if (r == 0x887A0005)
                        sMsg = "Present Device Removed" + Environment.NewLine
                            + "" + Environment.NewLine + "[ " + r.ToString()
                            + " ]"; // DXGI_ERROR_DEVICE_REMOVED
                    if (r == 0x88760870)
                        sMsg = "Present Device DD3DDI Removed"
                            + Environment.NewLine + "" + Environment.NewLine
                            + "[ " + r.ToString()
                            + " ]"; // D3DDDIERR_DEVICEREMOVED
                    // if (r == 0x087A0001) sMsg = "Present Device Occluded" +
                    // Environment.NewLine + "" + Environment.NewLine + "[ " +
                    // r.ToString() + " ]";      //DXGI_STATUS_OCCLUDED (ignored
                    //in the preceding if statement) if (r == 0x887A000A) sMsg =
                    //"Present Device Still Drawping" + Environment.NewLine + ""
                    //+ Environment.NewLine + "[ " + r.ToString() + " ]";
            ...
            ShutdownDX2D();
            MessageBox.Show("Problem in DirectX Renderer !"
                    + System.Environment.NewLine + System.Environment.NewLine
                    + "[ " + e.ToString() + " ]",
                "DirectX", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }
    }
    ...
    private static double m_dElapsedFrameStart
        = m_objFrameStartTimer.ElapsedMsec;
    ...
    public static int CurrentFPS {
        get { return m_nFps; }
        // MW0LGE_21k8 used to pre-init fps, before rundisplay has had time to
        // recalculate, called from console mostly when adjusting fps in setup
        // window
        set { m_nFps = value; }
    }
    ...
    static private void processMaximums(
        int rx, float dbm, int nX, int nMaxY_pixel) {
        Maximums[] maximums;
        if (rx == 1)
            maximums = m_nRX1Maximums;
        else
        ...
        if (nOccupiedIndex >= 0) {
            if (dbm >= maximums[nOccupiedIndex].max_dBm) {
                ...
                Array.Sort<Maximums>(
                    maximums, (x, y) => y.max_dBm.CompareTo(x.max_dBm));
            }
            return;
        }
    ...
    static public void ResetSpectrumPeaks(int rx) {
        ...
        // }
        Parallel.For(0, maximums.Length,
            (i) => { maximums[i].max_dBm = float.MinValue; });
    }
    static public void ResetBlobMaximums(int rx, bool bClear = false) {
        ...
        for (int n = 0; n < tot; n++) {
            if (bClear || !m_bBlobPeakHold
                || (m_bBlobPeakHold && !m_bBlobPeakHoldDrop
                    && m_dElapsedFrameStart
    ...
    static private bool isRxDuplex(int rx) {
        ...
        //             display_duplex) || (CurrentDisplayMode ==
        //             DisplayMode.PANADAPTER && display_duplex) ||
        //             (CurrentDisplayMode == DisplayMode.PANASCOPE &&
        //             display_duplex) //MW0LGE_21k8 ) displayduplex = true;
        // }
        // else
        //{
        ...
        //             display_duplex) || (CurrentDisplayModeBottom ==
        //             DisplayMode.PANADAPTER && display_duplex) ) displayduplex
        //             = true;
        // }
    ...
    static private int GetDBMPixelPos(
        int nVerticalShift, float dBm, int rx, int H, bool bIsWaterfall) {
        float fOffset;
        ...
        bool local_mox = mox
            && ((rx == 1 && !tx_on_vfob) || (rx == 2 && tx_on_vfob)
                || (rx == 1 && tx_on_vfob && !console.RX2Enabled));
        ...
        Y = (int)(((grid_max - dBm) * H / yRange)
            - 0.5f); // -0.5 to mimic floor
        // Y = Math.Min(Y, H);
        Y = Y < H ? Y : H;
        Y += nVerticalShift;

        int top; // a shift to include the gap at the top of the grid lines.
                 // This will make sure that this dbm point will match the
                 // scales
        if (bIsWaterfall)
            ...
            top = (int)((double)grid_step * H
                / yRange); // top is based on grid spacing
    ...
    static private float m_fNoiseFloorRX1
        = -160; // the NF exposed outside Display
    static private float m_fNoiseFloorRX2 = -160;
    static private bool m_bNoiseFloorGoodRX1
        = false; // is the noisefloor good? can be used outside Display
    ...
    static private float m_fLerpAverageRX1
        = -200; // the lerped average, over attacktime
    ...
    static private int m_nAttackFastFramesRX1
        = 60; // frames neeeded to complete the lerp. As display is entirely fps
              // base, then
    static private int m_nAttackFastFramesRX2
        = 60; // need to caclulate this each time fast attack is enabled
    ...
    public static float ActiveNoiseFloorRX1 {
        get { return m_fLerpAverageRX1 + _fNFshiftDBM; }
    }
    public static float ActiveNoiseFloorRX2 {
        get { return m_fLerpAverageRX2 + _fNFshiftDBM; }
    }
    ...
    private static void modifyDataForNotches(ref float[] data, int rx,
        bool bottom, bool local_mox, bool displayduplex, int W) {
        int Low, High;
        if (rx == 1) {
            if (local_mox) {
                if (displayduplex) {
        ...
        List<clsNotchCoords> notchData
            = handleNotches(rx, bottom, getCWSideToneShift(rx), Low, High, 0, 0,
                width, W, 0, false); //, 50);
        ...
        foreach (clsNotchCoords nc in notchData) {
            ...
            for (int i = nc._c_x; i > nc._c_x - wL; i--) {
                ...
                float fTmp = 1f
                    / ((float)Math.Pow((double)wL / (double)(wL - x),
            ...
            for (int i = nc._c_x; i < nc._c_x + wR; i++) {
                ...
                float fTmp = 1f
                    / ((float)Math.Pow((double)wR / (double)(wR - x),
                        1.5)); // pow2 quite sharp
                data[xPos] -= (fAttenuation * fTmp);
            }
        }
    ...
    public static float RX1Offset {
        get {
            float fOffset;
            bool local_mox
                = mox && (!tx_on_vfob || (tx_on_vfob && !console.RX2Enabled));
    ...
    unsafe static private bool DrawPanadapterDX2D(
        ...
        if (rx == 1) {
            ...
            if (data_ready) {
                ...
                if (!displayduplex && (local_mox || (mox && tx_on_vfob))
                    ...
                    for (int i = 0;
                         i < nDecimatedWidth /*current_display_data.Length*/;
                         i++)
                    ...
                    fixed(void* rptr = &new_display_data[0]) fixed(void* wptr
                        ...
                        /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
                }

                // make copy of the data so visual notch does not change the
                // average noise floor
                fixed(void* rptr = &current_display_data[0]) fixed(void* wptr
                    = &current_display_data_copy[0]) Win32.memcpy(wptr, rptr,
                    /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
        ...
        {
            bSpectralPeakHold
                = m_bSpectralPeakHoldRX2 && !m_bDelayRX2SpectrumPeaks;
            ...
            if (data_ready_bottom) {
                ...
                if (blank_bottom_display
                    ...
                    for (
                        int i = 0; i
                        < nDecimatedWidth /*current_display_data_bottom.Length*/
                        ;
                        i++)
        ...
        if (local_mox) {
            ...
            if (rx == 1) {
                lineBrush
                    = m_bUseLinearGradientForDataLine && m_bUseLinearGradient
                    ? m_brushLGDataLineRX1
                    : m_bDX2_data_line_pen_brush;
                fillBrush = m_bUseLinearGradient ? m_brushLGDataFillRX1
                                                 : m_bDX2_data_fill_fpen_brush;
            } else {
                lineBrush
                    = m_bUseLinearGradientForDataLine && m_bUseLinearGradient
                    ? m_brushLGDataLineRX2
                    : m_bDX2_data_line_pen_brush;
                fillBrush = m_bUseLinearGradient ? m_brushLGDataFillRX2
                                                 : m_bDX2_data_fill_fpen_brush;
            }
        ...
        // List <(int pos, float val)> maxtab_tmp = new List<(int pos, float
        // val)>();
        ...
        unchecked // we dont expect any overflows
        {
            ...
            if (bDoVisualNotch && m_bShowVisualNotch && !local_mox) {
                modifyDataForNotches(
                    ref data, rx, bottom, local_mox, displayduplex, W);
            }

            float averageSum = 0;
            int averageCount = 0;
            float currentAverage = rx == 1 ? m_fFFTBinAverageRX1 + 2
                                           : m_fFFTBinAverageRX2
                    + 2; // +2db to add some extras above the average
            ...
            for (int i = 0; i < nDecimatedWidth; i++) {
                ...
                if (!local_mox && bSpectralPeakHold
                    && max >= spectralPeaks[i].max_dBm) {
                ...
                ///
                if (bPeakBlobs) {
                    bool bInsideFilter = ((i * m_nDecimation) >= filter_left_x)
                        && ((i * m_nDecimation) <= filter_right_x);
                    if (!m_bInsideFilterOnly
                        || (m_bInsideFilterOnly && bInsideFilter)) {
                        if (max > mx) {
                            mx = max;
                            YPosForMx = Y;
                            mxpos = i;
                ...
                if (pan_fill) // 0 is top
                {
                    ...
                    _d2dRenderTarget.DrawLine(
                        bottomPoint, point, fillBrush, m_nDecimation);
                }

                // spectral peak
                if (!local_mox && bSpectralPeakHold
                    && spectralPeaks[i].max_dBm >= max) {
                    // draw to peak, but re-work Y as we might rescale the
                    // spectrum vertically
                    spectralPeakPoint.X = point.X;
                    spectralPeakPoint.Y
                        = (int)(((grid_max - spectralPeaks[i].max_dBm)
                                    * dbmToPixel)
                            - 0.5f);
                    if (spectralPeakPoint.Y >= H) spectralPeakPoint.Y = H;
                    spectralPeakPoint.Y += nVerticalShift;

                    if (spectralPeakPoint.Y < nVerticalShift)
                        spectralPeakPoint.Y = nVerticalShift; // crop top

                    if (bActivePeakFill) {
                        _d2dRenderTarget.DrawLine(point, spectralPeakPoint,
                            fillPeaksBrush, m_nDecimation);
                    } else {
                        ...
                        _d2dRenderTarget.DrawLine(oldSpectralPeakPoint,
                            spectralPeakPoint, fillPeaksBrush,
                            data_line_pen.Width /*display_line_width*/);
                        oldSpectralPeakPoint = spectralPeakPoint;
                    }

                    double dElapsed
                        = m_dElapsedFrameStart - spectralPeaks[i].Time;
                    if (spectralPeaks[i].max_dBm > max
                        && (dElapsed > dSpectralPeakHoldDelay)) {
                ...
                // lines will get longer if flat, reducing number of total
                // points
                ...
                if (point.Y == previousPoint.Y) {
                    ...
                    if (bIgnoringPoints) {
                        _d2dRenderTarget.DrawLine(previousPoint,
                            lastIgnoredPoint, lineBrush, penWidth);
            ...
            if (!local_mox) {
                ...
                processNoiseFloor(
                    rx, averageCount, averageSum, nDecimatedWidth, false);
                ...
                if (rx == 1) {
                    ...
                    yPixelLerp
                        = (int)dBToPixel(m_fLerpAverageRX1 + _fNFshiftDBM, H);
                    yPixelActual
                        = (int)dBToPixel(m_fFFTBinAverageRX1 + _fNFshiftDBM, H);
                    ...
                    yPixelLerp = (int)dBToRX2Pixel(
                        m_fLerpAverageRX2 + _fNFshiftDBM, H);
                    yPixelActual = (int)dBToRX2Pixel(
                        m_fFFTBinAverageRX2 + _fNFshiftDBM, H);

                    lerp = m_fLerpAverageRX2 + _fNFshiftDBM;
                }

                if ((rx == 1 && m_bShowRX1NoiseFloor)
                    || (rx == 2 && m_bShowRX2NoiseFloor)) {
                    yPixelLerp = yPixelLerp < H ? yPixelLerp : H;
                    yPixelLerp += nVerticalShift;

                    bool bDraw = !(yPixelLerp < nVerticalShift
                        || yPixelLerp
                            >= nVerticalShift + H); // crop anything off the top

                    if (bDraw) {
                        bool bFast = rx == 1 ? m_bFastAttackNoiseFloorRX1
                                             : m_bFastAttackNoiseFloorRX2;

                        yPixelActual = yPixelActual < H ? yPixelActual : H;
                        yPixelActual += nVerticalShift;

                        SharpDX.Direct2D1.Brush brYG
                            = bFast ? m_bDX2_Gray : m_bDX2_noisefloor;
                        SharpDX.Direct2D1.Brush brY
                            = bFast ? m_bDX2_Gray : m_bDX2_Yellow;

                        int yP = (int)yPixelLerp;

                        Rectangle box = new Rectangle(40, yP - 8, 8, 8);
                        drawFillRectangleDX2D(brYG, box);
                        drawLineDX2D(brYG, 40, yP, W - 40, yP, m_styleDots,
                            m_fNoiseFloorLineWidth); // horiz line

                        if (m_bShowNoiseFloorDBM) {
                            drawLineDX2D(brY, box.X - 3, (int)yPixelActual,
                                box.X - 3, yP, 2); // direction up/down line
                            drawStringDX2D(
                                lerp.ToString(_NFDecimal ? "F1" : "F0"),
                                fontDX2d_font9b, brY, box.X + box.Width,
                                box.Y - 6);
                        } else {
                            drawStringDX2D("-NF", fontDX2d_panafont, brYG,
                                box.X + box.Width, box.Y - 4);
                        }
                    }
                }
            }
                ...
                for (int n = 0; n < m_nNumberOfMaximums; n++) {
                    if (maximums[n].Enabled) {
                        if (m_bBlobPeakHold && m_bBlobPeakHoldDrop) {
                            // drop
                            double dElapsed
                                = m_dElapsedFrameStart - maximums[n].Time;
                            if (maximums[n].max_dBm > -200.0
                                && (dElapsed > m_fBlobPeakHoldMS)) {
                                maximums[n].max_dBm
                                    -= m_dBmPerSecondPeakBlobFall
                                    / (float)m_nFps;

                                // recalc Y
                                int nNewY
                                    = (int)(((grid_max - maximums[n].max_dBm)
                                                * dbmToPixel)
                                        - 0.5f);
                                nNewY = nNewY < H ? nNewY + nVerticalShift
                                                  : H + nVerticalShift;
                                maximums[n].MaxY_pixel = nNewY;
                            } else if (maximums[n].max_dBm <= -200.0) {
                                maximums[n].Enabled
                                    = false; // switch any off that fall off the
                                             // bottom same as resetmaximums
                                maximums[n].max_dBm = float.MinValue;
                            }
                        }
                        ...
                        bool bDraw = true;
                        if (m_objEllipse.Point.Y < nVerticalShift)
                            bDraw = false; // crop top
                        if (m_objEllipse.Point.Y >= nVerticalShift + H)
                            bDraw = false; // crop top

                        if (bDraw) {
                            string sAppend;
                            if (rx == 1) {
                                sAppend = m_bShowRX1NoiseFloor && !local_mox
                                    ? " ("
                                        + (maximums[n].max_dBm
                                            - m_fLerpAverageRX1)
                                              .ToString("f1")
                                ...
                                sAppend = m_bShowRX2NoiseFloor && !local_mox
                                    ? " ("
                                        + (maximums[n].max_dBm
                                            - m_fLerpAverageRX2)
                                              .ToString("f1")
                            ...
                            _d2dRenderTarget.DrawEllipse(
                                m_objEllipse, m_bDX2_PeakBlob);
                            _d2dRenderTarget.DrawText(
                                maximums[n].max_dBm.ToString("f1") + sAppend,
                                fontDX2d_callout,
    ...
    private static void processNoiseFloor(
        int rx, int averageCount, float averageSum, int width, bool waterfall) {
        if (rx == 1 && _bNoiseFloorAlreadyCalculatedRX1)
            return; // already done, ignore
        if (rx == 2 && _bNoiseFloorAlreadyCalculatedRX2)
            return; // already done, ignore
        ...
        if (rx == 1) {
            if (averageCount >= requireSamples) //(width / _NFsensitivity))
            {
                m_fFFTBinAverageRX1
                    ...
                    / 2f; // averageSum / (float)averageCount;
            } else {
            ...
            int framesInAttackTime = m_bFastAttackNoiseFloorRX1
                ? 0
                : (int)((fps / 1000f) * (double)m_fAttackTimeInMSForRX1);
            framesInAttackTime += 1;

            if (m_fLerpAverageRX1 > m_fFFTBinAverageRX1)
                m_fLerpAverageRX1 -= (m_fLerpAverageRX1 - m_fFFTBinAverageRX1)
                    / framesInAttackTime;
            else if (m_fLerpAverageRX1 < m_fFFTBinAverageRX1)
                m_fLerpAverageRX1 += (m_fFFTBinAverageRX1 - m_fLerpAverageRX1)
                    / framesInAttackTime;

            if (m_nAttackFastFramesRX1 > 0) m_nAttackFastFramesRX1--;
            if (m_bFastAttackNoiseFloorRX1
                && (Math.Abs(m_fFFTBinAverageRX1 - m_fLerpAverageRX1) < 1f)
                && (m_nAttackFastFramesRX1 == 0)) {
            ...
            // so in attackTime period we need to have moved to where we want
            int framesInAttackTime = m_bFastAttackNoiseFloorRX2
                ? 0
                : (int)((fps / 1000f) * (double)m_fAttackTimeInMSForRX2);
            framesInAttackTime += 1;

            if (m_fLerpAverageRX2 > m_fFFTBinAverageRX2)
                m_fLerpAverageRX2 -= (m_fLerpAverageRX2 - m_fFFTBinAverageRX2)
                    / framesInAttackTime;
            else if (m_fLerpAverageRX2 < m_fFFTBinAverageRX2)
                m_fLerpAverageRX2 += (m_fFFTBinAverageRX2 - m_fLerpAverageRX2)
                    / framesInAttackTime;

            if (m_nAttackFastFramesRX2 > 0) m_nAttackFastFramesRX2--;
            if (m_bFastAttackNoiseFloorRX2
                && (Math.Abs(m_fFFTBinAverageRX2 - m_fLerpAverageRX2) < 1f)
                && (m_nAttackFastFramesRX2 == 0)) {
    ...
    unsafe static private bool DrawWaterfallDX2D(
        ...
        if (console.PowerOn) {
            if (rx == 1 && waterfall_data_ready) {
                ...
                if (!displayduplex && local_mox
                    ...
                    fixed(void* rptr = &new_waterfall_data[0]) fixed(void* wptr
                        ...
                        nDecimatedWidth * sizeof(float));
                }

                // make copy of the data so visual notch does not change the
                // average noise floor
                fixed(void* rptr = &current_waterfall_data[0]) fixed(void* wptr
                    = &current_waterfall_data_copy[0]) Win32.memcpy(wptr, rptr,
                    /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
                ...
                if (local_mox
                    ...
                    fixed(void* rptr = &new_waterfall_data_bottom[0]) fixed(
                        void* wptr = &current_waterfall_data_bottom[0])
                        Win32.memcpy(wptr, rptr,
                            /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
                }
            ...
            if (bRXdraw) {
                ...
                float max;
                float max_copy;
                float fOffset; // = 0; ///MW0LGE - block of code moved out of
                               // for loop +- for now, TODO

                if (!local_mox) {
                    // if (rx == 1) fOffset += rx1_display_cal_offset +
                    // (rx1_preamp_offset - alex_preamp_offset); else if (rx ==
                    // 2) fOffset += rx2_display_cal_offset +
                    // (rx2_preamp_offset);
                ...
                // else
                //{
                // if (rx == 1) fOffset += tx_display_cal_offset +
                // (rx1_preamp_offset - alex_preamp_offset); else if (rx == 2)
                // fOffset += tx_display_cal_offset + (rx2_preamp_offset);
                // }
                ...
                if (!local_mox) {
                    ...
                    bool bPreviousRX2 = _bNoiseFloorAlreadyCalculatedRX2;
                    processNoiseFloor(
                        rx, averageCount, averageSum, nDecimatedWidth, true);
                ...
                if (rx == 1) {
                    topPixels = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget,
                        new Size2((int)_waterfall_bmp_dx2d.Size.Width,
                            (int)_waterfall_bmp_dx2d.Size.Height - 1),
                        new BitmapProperties(new SDXPixelFormat(
                            _waterfall_bmp_dx2d.PixelFormat.Format,
                            _ALPHA_MODE)));

                    topPixels.CopyFromBitmap(_waterfall_bmp_dx2d,
                        new SharpDX.Point(0, 0),
                ...
                } else // rx2
                {
                    topPixels = new SharpDX.Direct2D1.Bitmap(_d2dRenderTarget,
                        new Size2((int)_waterfall_bmp2_dx2d.Size.Width,
                            (int)_waterfall_bmp2_dx2d.Size.Height - 1),
                        new BitmapProperties(new SDXPixelFormat(
                            _waterfall_bmp2_dx2d.PixelFormat.Format,
                            _ALPHA_MODE)));

                    topPixels.CopyFromBitmap(_waterfall_bmp2_dx2d,
                        new SharpDX.Point(0, 0),
                        new SharpDX.Rectangle(0, 0, (int)topPixels.Size.Width,
                            (int)topPixels.Size.Height));
                }
...
#region colours
                switch (cSheme) {
                    case (ColorSheme.original): {
                        //// draw new data
                        // for (int i = 0; i < nDecimatedWidth; i++)	// for each
                        // pixel in the new line
                        //{
                        //     if (waterfall_data[i] <= low_threshold)		// if
                        //     less than low threshold, just use low color
                        //     {
                        //         R = low_color.R;
                        //         G = low_color.G;
                        //         B = low_color.B;
                        ...
                        //     if more than high threshold, just use high color
                        //     {
                        //         R = high_color.R;
                        //         G = high_color.G;
                        //         B = high_color.B;
                        ...
                        //     {
                        //         float percent = (waterfall_data[i] -
                        //         low_threshold) / (high_threshold -
                        //         low_threshold); if (percent <= 0.5)	// use a
                        //         gradient between low and mid colors
                        //         {
                        //             percent *= 2;

                        //            R = (int)((1 - percent) * low_color.R +
                        //            percent * mid_color.R); G = (int)((1 -
                        //            percent) * low_color.G + percent *
                        //            mid_color.G); B = (int)((1 - percent) *
                        //            low_color.B + percent * mid_color.B);
                        //        }
                        //        else				// use a gradient between mid and
                        //        high colors
                        //        {
                        //            percent = (float)(percent - 0.5) * 2;

                        //            R = (int)((1 - percent) * mid_color.R +
                        //            percent * high_color.R); G = (int)((1 -
                        //            percent) * mid_color.G + percent *
                        //            high_color.G); B = (int)((1 - percent) *
                        //            mid_color.B + percent * high_color.B);
                        //        }
                        //    }
                        //    // set pixel color
                        //    row[(i * m_nDecimation) * pixel_size + 0] =
                        //    (byte)B;  // set color in memory row[(i *
                        //    m_nDecimation) * pixel_size + 1] = (byte)G; row[(i
                        //    * m_nDecimation) * pixel_size + 2] = (byte)R;
                        //    row[(i * m_nDecimation) * pixel_size + 3] =
                        //    nbBitmapAlpaha;
                        //}
                    } break;
                ...
                for (int i = 0; i < nDecimatedWidth; i++) {
                    for (int j = 1; j < m_nDecimation; j++) {
                        row[((i * m_nDecimation) + j) * pixel_size + 0]
                            = row[(i * m_nDecimation) * pixel_size + 0];
                        row[((i * m_nDecimation) + j) * pixel_size + 1]
                            = row[(i * m_nDecimation) * pixel_size + 1];
                        row[((i * m_nDecimation) + j) * pixel_size + 2]
                            = row[(i * m_nDecimation) * pixel_size + 2];
                        row[((i * m_nDecimation) + j) * pixel_size + 3]
                            = row[(i * m_nDecimation) * pixel_size + 3];
                    }
                }

                if (rx == 1) {
                    _waterfall_bmp_dx2d.CopyFromMemory(
                        row, W * pixel_size, new SharpDX.Rectangle(0, 0, W, 1));
                    _waterfall_bmp_dx2d.CopyFromBitmap(
                        topPixels, new SharpDX.Point(0, 1));
                } else {
                    _waterfall_bmp2_dx2d.CopyFromMemory(
                        row, W * pixel_size, new SharpDX.Rectangle(0, 0, W, 1));
                    _waterfall_bmp2_dx2d.CopyFromBitmap(
                        topPixels, new SharpDX.Point(0, 1));
                }
            ...
            if (rx == 1) {
                _d2dRenderTarget.DrawBitmap(_waterfall_bmp_dx2d,
                    new RectangleF(0, nVerticalShift + 20,
                        _waterfall_bmp_dx2d.Size.Width,
                        _waterfall_bmp_dx2d.Size.Height),
                    m_fRX1WaterfallOpacity, BitmapInterpolationMode.Linear);
            } else {
                _d2dRenderTarget.DrawBitmap(_waterfall_bmp2_dx2d,
                    new RectangleF(0, nVerticalShift + 20,
                        _waterfall_bmp2_dx2d.Size.Width,
                        _waterfall_bmp2_dx2d.Size.Height),
                    m_fRX2WaterfallOpacity, BitmapInterpolationMode.Linear);
            }
        }
    ...
    private static SharpDX.Direct2D1.SolidColorBrush convertBrush(
        ...
        return new SharpDX.Direct2D1.SolidColorBrush(
            _d2dRenderTarget, convertColour(b.Color));
    }
        ...
        lock (_objDX2Lock) {
            ...
            if (image != null) {
                using (Bitmap graphicsImage = new Bitmap(image)) {
                    _bitmapBackground = SDXBitmapFromSysBitmap(
                        _d2dRenderTarget, graphicsImage);
                }
            }
        }
    }
    ...
    private static SharpDX.Direct2D1.Bitmap SDXBitmapFromSysBitmap(
        RenderTarget rt, System.Drawing.Bitmap bitmap) {
        Rectangle sourceArea
            = new System.Drawing.Rectangle(0, 0, bitmap.Width, bitmap.Height);
        BitmapProperties bitmapProperties
            = new BitmapProperties(new SDXPixelFormat(Format.B8G8R8A8_UNorm,
                _ALPHA_MODE)); // was R8G8B8A8_UNorm  //MW0LGE_21k9
        ...
        for (int y = 0; y < bitmap.Height; y++) {
            ...
            for (int x = 0; x < bitmap.Width; x++) {
                ...
                // //MW0LGE_21k9 tempStream.Write(rgba);
        ...
        SharpDX.Direct2D1.Bitmap dx2bitmap = new SharpDX.Direct2D1.Bitmap(
            rt, size, tempStream, stride, bitmapProperties);
    ...
    private static SharpDX.Direct2D1
        .Brush m_bDX2_bandstack_overlay_brush_highlight;
    ...
    //--------------------------
    private static SharpDX.Direct2D1.LinearGradientBrush m_brushLGDataFillRX1
        = null;
    private static SharpDX.Direct2D1.LinearGradientBrush m_brushLGDataLineRX1
        = null;
    private static SharpDX.Direct2D1.LinearGradientBrush m_brushLGDataFillRX2
        = null;
    private static SharpDX.Direct2D1.LinearGradientBrush m_brushLGDataLineRX2
        = null;
    //
    ...
    public static bool UseLinearGradientForDataLine {
        ...
        set { m_bUseLinearGradientForDataLine = value; }
    }
    private static void buildLinearGradientBrush(int top, int bottom, int rx) {
        ...
        List<ucLGPicker.ColourGradientData> lst
            = console.SetupForm.RX1GradPicker.GetColourGradientDataForDBMRange(
                grid_min, grid_max);
        ...
        for (int n = 0; n < lst.Count; n++) {
            Color dataFillColour = Color.FromArgb(data_fill_color.A,
                lst[n].color.R, lst[n].color.G, lst[n].color.B);
            Color dataLineColour = Color.FromArgb(data_line_color.A,
                lst[n].color.R, lst[n].color.G, lst[n].color.B);

            gradientStopsDataFill[n]
                = new GradientStop() { Color = convertColour(dataFillColour),
                      Position = lst[n].percent };
            gradientStopsDataLine[n]
                = new GradientStop() { Color = convertColour(dataLineColour),
                      Position = lst[n].percent };
        }
        SharpDX.Direct2D1.GradientStopCollection fill
            = new SharpDX.Direct2D1.GradientStopCollection(
                _d2dRenderTarget, gradientStopsDataFill);
        SharpDX.Direct2D1.GradientStopCollection line
            = new SharpDX.Direct2D1.GradientStopCollection(
                _d2dRenderTarget, gradientStopsDataLine);
        ...
        if (rx == 1) {
            ...
            m_brushLGDataFillRX1
                = new SharpDX.Direct2D1.LinearGradientBrush(_d2dRenderTarget,
    ...
    private static void releaseDX2Resources() {
        ...
        // if (m_bDX2_m_bHightlightNumbers != null) Utilities.Dispose(ref
        // m_bDX2_m_bHightlightNumbers); if (m_bDX2_m_bHightlightNumberScale !=
        // null) Utilities.Dispose(ref m_bDX2_m_bHightlightNumberScale);
        ...
        // if (m_bDX2_YellowGreen != null) Utilities.Dispose(ref
        // m_bDX2_YellowGreen); if (m_bDX2_Gray != null) Utilities.Dispose(ref
        // m_bDX2_Gray);

        // if (m_bDX2_PeakBlob != null) Utilities.Dispose(ref m_bDX2_PeakBlob);
        // if (m_bDX2_PeakBlobText != null) Utilities.Dispose(ref
        // m_bDX2_PeakBlobText);
        ...
        // if (m_bDX2_waveform_line_pen != null) Utilities.Dispose(ref
        ...
        // MW0LGE_21k9rc6 just assign to null, but they are disposed of in
        // clearAllDynamicBrushes
    ...
    private static void buildDX2Resources() {
        lock (_objDX2Lock) {
            ...
            m_bDX2_dataPeaks_fill_fpen_brush
                = convertBrush((SolidBrush)dataPeaks_fill_fpen.Brush);
            m_bDX2_data_fill_fpen_brush
            ...
            m_cDX2_display_background_colour = convertColour(
                display_background_brush
                    .Color); // does not need dispose as it is a type

            m_bDX2_m_bHightlightNumbers = getDXBrushForColour(Color.FromArgb(
                255, 255, 255)); // convertBrush(new
                                 // SolidBrush(Color.FromArgb(255, 255, 255)));
            m_bDX2_m_bHightlightNumberScale
                = getDXBrushForColour(Color.FromArgb(192, 64, 64,
                    64)); // convertBrush(new SolidBrush(Color.FromArgb(192, 64,
                          // 64, 64)));
            ...
            m_bDX2_bandstack_overlay_brush
                = convertBrush((SolidBrush)bandstack_overlay_brush);
            m_bDX2_bandstack_overlay_brush_lines
                = convertBrush((SolidBrush)bandstack_overlay_brush_lines);
            m_bDX2_bandstack_overlay_brush_highlight
                = convertBrush((SolidBrush)bandstack_overlay_brush_highlight);
            ...
            m_bDX2_Red = getDXBrushForColour(
                Color.Red); // convertBrush(new SolidBrush(Color.Red));
                            // //MW0LGE_21k9rc6
            m_bDX2_Yellow = getDXBrushForColour(
                Color.Yellow); // convertBrush(new SolidBrush(Color.Yellow));
            m_bDX2_YellowGreen = getDXBrushForColour(
                Color.YellowGreen); // convertBrush(new
                                    // SolidBrush(Color.YellowGreen));
            m_bDX2_Gray = getDXBrushForColour(
                Color.Gray); // convertBrush(new SolidBrush(Color.Gray));

            m_bDX2_PeakBlob = getDXBrushForColour(
                Color.OrangeRed); // convertBrush(new
                                  // SolidBrush(Color.OrangeRed));
            m_bDX2_PeakBlobText = getDXBrushForColour(
                Color.Chartreuse); // convertBrush(new
                                   // SolidBrush(Color.Chartreuse));

            m_bDX2_y1_brush = getDXBrushForColour(Color.FromArgb(
                64, 64, 64)); // convertBrush(new SolidBrush(Color.FromArgb(64,
                              // 64, 64)));
            m_bDX2_y2_brush = getDXBrushForColour(Color.FromArgb(
                48, 48, 48)); // convertBrush(new SolidBrush(Color.FromArgb(48,
                              // 48, 48)));
            m_bDX2_waveform_line_pen = getDXBrushForColour(
                Color.LightGreen); // convertBrush(new
                                   // SolidBrush(Color.LightGreen));

            m_bDX2_dhp = getDXBrushForColour(Color.FromArgb(0, 255,
                0)); // convertBrush(new SolidBrush(Color.FromArgb(0, 255, 0)));
            m_bDX2_dhp1 = getDXBrushForColour(Color.FromArgb(150, 0, 0,
                255)); // convertBrush(new SolidBrush(Color.FromArgb(150, 0, 0,
                       // 255)));
            m_bDX2_dhp2 = getDXBrushForColour(Color.FromArgb(150, 255, 0,
                0)); // convertBrush(new SolidBrush(Color.FromArgb(150, 255, 0,
                     // 0)));
            ...
            StrokeStyleProperties ssp
                = new StrokeStyleProperties() { DashOffset = 2,
        ...
        lock (_objDX2Lock) {
            ...
            fontDX2d_callout = new SharpDX.DirectWrite.TextFormat(fontFactory,
                ...
                (m_fntCallOutFont.Size / 72)
                    * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font9 = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font9.FontFamily.Name,
                (font9.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font9b = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font9b.FontFamily.Name, SharpDX.DirectWrite.FontWeight.Bold,
                SharpDX.DirectWrite.FontStyle.Normal,
                (font9b.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font9c = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font95.FontFamily.Name,
                (font95.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_panafont = new SharpDX.DirectWrite.TextFormat(fontFactory,
                pana_font.FontFamily.Name,
                (pana_font.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font10 = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font10.FontFamily.Name,
                (font10.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font14 = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font14.FontFamily.Name,
                (font14.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font32 = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font32b.FontFamily.Name,
                (font32b.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
            fontDX2d_font1 = new SharpDX.DirectWrite.TextFormat(fontFactory,
                font1.FontFamily.Name,
                (font1.Size / 72) * _d2dRenderTarget.DotsPerInch.Width);
        }
    }
    static void clearBackgroundDX2D(int rx, int W, int H, bool bottom) {
        ...
        if (rx == 1) {
            if (bottom) {
                _d2dRenderTarget.FillRectangle(new RectangleF(0, H, W, H),
                    m_bDX2_display_background_brush);
            } else {
                _d2dRenderTarget.FillRectangle(new RectangleF(0, 0, W, H),
                    m_bDX2_display_background_brush);
            }
        } else if (rx == 2) {
            if (bottom) {
                ...
                _d2dRenderTarget.FillRectangle(new RectangleF(0, H * 2, W, H),
                    m_bDX2_display_background_brush);
            }
        }
    }
    ...
    private static void drawRectangleDX2D(
        SharpDX.Direct2D1.Brush b, float x, float y, float w, float h) {
        RectangleF rect = new RectangleF(x, y, w, h);
    ...
    private static void drawFillElipseDX2D(SharpDX.Direct2D1.Brush b,
        float xMiddle, float yMiddle, float w, float h) {
        Ellipse e = new Ellipse(
            new SharpDX.Vector2(xMiddle /* + 0.5f*/, yMiddle /* + 0.5f*/),
            w / 2, h / 2);
    ...
    private static void drawRectangleDX2D(
        SharpDX.Direct2D1.Brush b, Rectangle r, float lineWidth = 1) {
    ...
    private static void drawChannelBarDX2D(Channel chan, int left, int right,
        ...
        if (width > 2) {
            using (Pen p = new Pen(h, 1)) {
                ...
                drawLineDX2D(convertBrush((SolidBrush)p.Brush), right, top,
                    right, top + height - 1, p.Width);
            }
        }
    }
    private static Dictionary<string, System.Drawing.SizeF> m_stringSizeCache
        = new Dictionary<string, System.Drawing.SizeF>();
    private static System.Drawing.SizeF measureStringDX2D(string s,
        ...
        if (cacheStringLength)
            key = s.Length.ToString() + tf.FontFamilyName
                + tf.FontSize.ToString();
        else
        ...
        if (m_stringSizeCache.Count > 500)
            m_stringSizeCache.Remove(
                m_stringSizeCache.Keys
                    .Last()); // dump oldest, as new one are inserted at the
                              // head of dictionary list
    ...
    private static int getCWSideToneShift(
        int rx, DSPMode forceMode = DSPMode.FIRST) {
        int nRet = 0;
        DSPMode mode;
    ...
    private class clsNotchCoords {
        ...
        public clsNotchCoords(
            int nC_x, int nLeft_X, int nRight_X, bool bUse, int nWidthHz) {
            _c_x = nC_x;
            _left_x = nLeft_X;
            _right_x = nRight_X;
            _Use = bUse;
    ...
    private static List<clsNotchCoords> handleNotches(int rx, bool bottom,
        int cwSideToneShift, int Low, int High, int nVerticalShift, int top,
        int width, int W, int H, bool bDraw) //, int expandHz = 0)
    {
        long rf_freq = vfoa_hz;
        int rit = rit_hz;

        if (/*bottom || */ (current_display_mode_bottom == DisplayMode.PANAFALL
            && rx == 2)) // MW0LGE
        {
            rf_freq = vfob_hz;
        }
        ...
        List<MNotch> notches = MNotchDB.NotchesInBW(rf_freq,
            Low - console.MaxFilterWidth, High + console.MaxFilterWidth);
        ...
        foreach (MNotch n in notches) {
            ...
            if (bDraw) {
                notch_centre_x
                    = (int)((float)((n.FCenter) - rf_freq - Low - rit) / width
                        * W);
                notch_left_x = (int)((float)((n.FCenter) - rf_freq
                                         - n.FWidth / 2 - Low - rit)
                    / width * W);
                notch_right_x = (int)((float)((n.FCenter) - rf_freq
                                          + n.FWidth / 2 - Low - rit)
                    / width * W);
            } else {
                // int expandHz = (int)(_dMNFminSize * 0.5);
                // double nw = n.FWidth < 100 ? 100 : n.FWidth;
                double dNewWidth = n.FWidth < _mnfMinSize
                    ? _mnfMinSize
                    : n.FWidth; // use the min width of filter from WDSP
                dNewWidth
                    += 20; // fudge factor to align better with spectrum notch
                notch_centre_x
                    = (int)((float)((n.FCenter) - rf_freq - Low - rit) / width
                        * W);
                notch_left_x
                    = (int)((float)((n.FCenter) - rf_freq - dNewWidth / 2 - Low
                                - rit /* - expandHz*/)
                        / width * W);
                notch_right_x
                    = (int)((float)((n.FCenter) - rf_freq + dNewWidth / 2 - Low
            ...
            clsNotchCoords nc = new clsNotchCoords(notch_centre_x, notch_left_x,
                notch_right_x, _tnf_active && n.Active, (int)n.FWidth);
            notchData.Add(nc);
            ...
            if (bDraw) {
                ...
                if (n == m_objHightlightedNotch) {
                    ...
                    drawStringDX2D("F: " + temp_text.Insert(nTmp, " "),
        ...
        if ((bIsWaterfall && m_bShowRXFilterOnWaterfall) || !bIsWaterfall) {
                            ...
                            // == DSPMode.CWU))
            {
                ...
                if (!bIsWaterfall) {
                    ...
                    switch (nFilterEdge) {
                        case -1:
                            drawLineDX2D(m_bDX2_cw_zero_pen, filter_left_x,
                                nVerticalShift + top, filter_left_x,
                                nVerticalShift + H, 2);
                            break;
                        case 1:
                            drawLineDX2D(m_bDX2_cw_zero_pen, filter_right_x,
                                nVerticalShift + top, filter_right_x,
                                nVerticalShift + H, 2);
                            break;
                        default: break;
                    }
...
#region Tx filter and tx lines
        ...
        if ((rx == 1 && rx1_dsp_mode != DSPMode.CWL
                && rx1_dsp_mode != DSPMode.CWU)
            || (rx == 2 && rx2_dsp_mode != DSPMode.CWL
                && rx2_dsp_mode != DSPMode.CWU)) // MW0LGE [2.9.0.7] +rx2
        {
            if ((bIsWaterfall
                    && m_bShowTXFilterOnRXWaterfall /*m_bShowTXFilterOnWaterfall*/)
                || !bIsWaterfall) {
                int filter_left_x;
                int filter_right_x;
                int filter_low_tmp;
                ...
                if (!split_enabled) {
                    filter_left_x
                        = (int)((float)(filter_low_tmp - Low - f_diff + xit_hz)
                            / width * W);
                    filter_right_x
                        = (int)((float)(filter_high_tmp - Low - f_diff + xit_hz)
                            / width * W);
                } else // MW0LGE_21k8
                {
                    filter_left_x = (int)((float)(filter_low_tmp - Low + xit_hz
                                              + (vfoa_sub_hz - vfoa_hz))
                        / width * W);
                    filter_right_x
                        = (int)((float)(filter_high_tmp - Low + xit_hz
                                    + (vfoa_sub_hz - vfoa_hz))
                            / width * W);
                }
                ...
                if (local_mox) {
                    ...
                    if ((rx == 2 && tx_on_vfob)
                        ...
                        drawLineDX2D(m_bDX2_tx_filter_pen, filter_left_x,
                            nVerticalShift + top, filter_left_x,
                            nVerticalShift + H, tx_filter_pen.Width);
...
////{
////    if (local_mox && (rx1_dsp_mode != DSPMode.CWL && rx1_dsp_mode !=
///DSPMode.CWU)) /    { /        // draw TX filter /        int filter_left_x;
////        int filter_right_x;

////        if (!split_enabled)
////        {
////            filter_left_x = (int)((float)(filter_low - Low - f_diff +
///xit_hz) / width * W); /            filter_right_x = (int)((float)(filter_high
///- Low - f_diff + xit_hz) / width * W); /        } /        else //
///MW0LGE_21k8 /        { /            filter_left_x = (int)((float)(filter_low
///- Low + xit_hz + (vfoa_sub_hz - vfoa_hz)) / width * W); / filter_right_x =
///(int)((float)(filter_high - Low + xit_hz + (vfoa_sub_hz - vfoa_hz)) / width *
///W); /        }
...
/////MW0LGE /    !/*mox*/local_mox && draw_tx_filter && /    (rx1_dsp_mode !=
///DSPMode.CWL && rx1_dsp_mode != DSPMode.CWU))
////{
...
///xit_hz - rit_hz) / width * W); /            filter_right_x =
///(int)((float)(tx_filter_high - Low - f_diff + xit_hz - rit_hz) / width * W);
////        }
////        else
////        {
////            filter_left_x = (int)((float)(tx_filter_low - Low + xit_hz -
///rit_hz + (vfoa_sub_hz - vfoa_hz)) / width * W); /            filter_right_x =
///(int)((float)(tx_filter_high - Low + xit_hz - rit_hz + (vfoa_sub_hz -
///vfoa_hz)) / width * W); /        } /    } /    else /    { /        if
///(!split_enabled) /        { /            filter_left_x =
///(int)((float)(tx_filter_low - Low - f_diff + xit_hz - rit_hz) / width * W);
////            filter_right_x = (int)((float)(tx_filter_high - Low - f_diff +
///xit_hz - rit_hz) / width * W); /        } /        else /        { /
///filter_left_x = (int)((float)(tx_filter_low - Low + xit_hz - rit_hz +
///(vfoa_sub_hz - vfoa_hz)) / width * W); /            filter_right_x =
///(int)((float)(tx_filter_high - Low + xit_hz - rit_hz + (vfoa_sub_hz -
///vfoa_hz)) / width * W); /        } /    }

////    if ((rx == 2 && tx_on_vfob) || (rx == 1 && !(tx_on_vfob &&
///rx2_enabled))) /    { /        drawLineDX2D(m_bDX2_tx_filter_pen,
///filter_left_x, nVerticalShift + top, filter_left_x, nVerticalShift + H,
///tx_filter_pen.Width);        // draw tx filter overlay /
///drawLineDX2D(m_bDX2_tx_filter_pen, filter_right_x, nVerticalShift + top,
///filter_right_x, nVerticalShift + H, tx_filter_pen.Width);	// draw tx filter
///overlay /    }
////}
        ...
        if (!bIsWaterfall
            ...
            foreach (Channel c in Console.Channels60m) {
                        ...
                        (rf_freq + High) * 1e-6)) // is channel visible?
                {
                    ...
                    Color c1 = on_channel ? channel_background_on
                                          : channel_background_off;
...
#region BandStackOverlay
        if (m_bShowBandStackOverlays && m_bandStackOverlays != null && rx == 1
            && !local_mox && !bIsWaterfall) {
            long rf_freq = vfoa_hz;
            int rit = rit_hz;
            ...
            for (int n = 0; n < m_bandStackOverlays.Length; n++) {
                int filter_left_x
                    = (int)((float)((((m_bandStackOverlays[n].Frequency * 1e6)
                                         - rf_freq)
                                        + m_bandStackOverlays[n].LowFilter)
                                - Low - rit)
                        / width * W);
                int filter_right_x
                    = (int)((float)((((m_bandStackOverlays[n].Frequency * 1e6)
                                         - rf_freq)
                                        + m_bandStackOverlays[n].HighFilter)
                                - Low - rit)
                        / width * W);

                brush = (n == m_nHighlightedBandStackEntryIndex)
                    ? m_bDX2_bandstack_overlay_brush_highlight
                    : m_bDX2_bandstack_overlay_brush;

                // filled rect
                drawFilterOverlayDX2D(brush, filter_left_x, filter_right_x, W,
                    H, rx, top, bottom, nVerticalShift);

                // line either side
                drawLineDX2D(m_bDX2_bandstack_overlay_brush_lines,
                    filter_left_x, nVerticalShift + top, filter_left_x,
                    nVerticalShift + H, 2);
                drawLineDX2D(m_bDX2_bandstack_overlay_brush_lines,
                    filter_right_x, nVerticalShift + top, filter_right_x,
                    nVerticalShift + H, 2);
            }
        }
#endregion
...
#region notches
        ...
        if (!local_mox && !bIsWaterfall) {
            handleNotches(rx, bottom, cwSideToneShift, Low, High,
                nVerticalShift, top, width, W, H,
                true); //, 50); //MW0LGE [2.9.0.7] moved to function so can be
                       //used by drawmpana/drawwater
        } // END NOTCH
#endregion
...
#region CW zero and tx lines
        ...
        if (!bIsWaterfall) {
            ...
            if (draw_tx_cw_freq) {
                ...
                if (rx == 2 && !local_mox && tx_on_vfob
                    && // MW0LGE [2.9.0.7] txonb
                    (rx2_dsp_mode == DSPMode.CWL
                        || rx2_dsp_mode
                            == DSPMode.CWU)) // MW0LGE [2.9.0.7] txonb
                {
                    int cw_line_x1;
                    if (!split_enabled)
                        cw_line_x1 = (int)((float)(cwSideToneShiftInverted - Low
        ...
        if (local_mox) {
            if (!split_enabled) // MW0LGE_21k8
            {
                ...
                center_line_x
                    = (int)((float)(-Low + xit_hz + (vfoa_sub_hz - vfoa_hz))
                        / width * W); // locked 0 line
            }
        } else
            center_line_x
...
#region Band edges, H + V lines and labels
        ...
        if (rx == 1) {
            if (local_mox) {
                ...
                if (split_enabled)
                    vfo = vfoa_sub_hz - (vfoa_sub_hz - vfoa_hz);
                else
                    vfo = vfoa_hz;
                vfo += xit_hz;
        ...
        if (!bIsWaterfall && !local_mox) {
            ...
            if (console.PowerOn
                ...
                if (rx == 1 /* && !local_mox*/) {
                    ...
                    unsafe {
                        double size = (double)console.specRX.GetSpecRX(0)
                                          .FFTSize; // MW0LGE_21k7
                        WDSP.GetRXAAGCThresh(WDSP.id(0, 0), &rx1_thresh,
                            size /*4096.0*/, sample_rate);
                ...
                } else // if (rx == 2/* && !local_mox*/)
                {
                    ...
                    switch (console.RX2AGCMode) {
                        ...
                        default:
                            rx2_cal_offset = 2.0f
                                + (rx2_display_cal_offset + rx2_preamp_offset)
                                - rx2_fft_size_offset;
                            break;
                    }
                    unsafe {
                        double size = (double)console.specRX.GetSpecRX(1)
                                          .FFTSize; // MW0LGE_21k7
                        // get AGC-T level
                        WDSP.GetRXAAGCThresh(WDSP.id(2, 0), &rx2_thresh,
                            size /*4096.0*/, sample_rate);
...
#region Spots
    ...
    private static void DrawCursorInfo(int W) {
        // MHzCursor Display
        if ((m_bAlwaysShowCursorInfo || Common.ShiftKeyDown)
            && display_cursor_x != -1) {
            ...
            if (m_sMHzCursorDisplay != "") {
                width = (int)measureStringDX2D(
                    m_sMHzCursorDisplay, fontDX2d_callout, true)
                            .Width;
                ...
                drawStringDX2D(m_sMHzCursorDisplay, fontDX2d_callout,
                    m_bDX2_m_bTextCallOutActive, xPos, display_cursor_y - 18);
            }
            ...
            if (m_sOtherData1CursorDisplay != "") {
                ...
                if (bLeftSide) {
                    ...
                    width = (int)measureStringDX2D(
                        m_sOtherData1CursorDisplay, fontDX2d_callout, true)
                                .Width;
                    xPos -= width + 24;
                    bLeftSide = true;
                }
                drawStringDX2D(m_sOtherData1CursorDisplay, fontDX2d_callout,
                    m_bDX2_m_bTextCallOutActive, xPos, display_cursor_y + 2);
            }
            ...
            if (m_sOtherData2CursorDisplay != "") {
                ...
                if (bLeftSide) {
                    ...
                    width = (int)measureStringDX2D(
                        m_sOtherData2CursorDisplay, fontDX2d_callout, true)
                                .Width;
                    xPos -= width + 24;
                    bLeftSide = true;
                }
                drawStringDX2D(m_sOtherData2CursorDisplay, fontDX2d_callout,
                    m_bDX2_m_bTextCallOutActive, xPos, display_cursor_y + 18);
            }
        }
        //
    ...
    unsafe static private bool DrawSpectrumDX2D(
        int rx, int W, int H, bool bottom) {
        ...
        if (!bottom && data_ready) {
            if (mox
                && (rx1_dsp_mode == DSPMode.CWL
                    || rx1_dsp_mode == DSPMode.CWU)) {
                for (int i = 0;
                     i < nDecimatedWidth /*current_display_data.Length*/; i++)
                    current_display_data[i] = grid_min - rx1_display_cal_offset;
            } else {
                fixed(void* rptr = &new_display_data[0]) fixed(
                    void* wptr = &current_display_data[0]) Win32.memcpy(wptr,
                    rptr, /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
            }
            data_ready = false;
        } else if (bottom && data_ready_bottom) {
            {
                fixed(void* rptr = &new_display_data_bottom[0]) fixed(void* wptr
                    = &current_display_data_bottom[0]) Win32.memcpy(wptr, rptr,
                    /*BUFFER_SIZE*/ nDecimatedWidth * sizeof(float));
            }
            data_ready_bottom = false;
        }
        ...
        if (!mox || (mox && tx_on_vfob && console.RX2Enabled)) {
            if (rx == 1 /*!bottom*/)
                max += rx1_display_cal_offset; // MW0LGE_21g
            else
        ...
        if (!mox || (mox && tx_on_vfob && console.RX2Enabled)) {
            if (rx == 1 /*!bottom*/)
                max += rx1_preamp_offset
                    - alex_preamp_offset; // MW0LGE_21 change to rx==1
            else
        ...
        for (int i = 0; i < nDecimatedWidth; i++) {
            ...
            Y = (int)Math.Min((Math.Floor((grid_max - max) * H / yRange)), H);
            point.X
                = i * m_nDecimation; // + 0.5f; // the 0.5f pushes it into the
                                     // middle of a 'pixel', so that it is not
                                     // drawn half in one, and half in the other
            point.Y = Y; // + 0.5f;

            _d2dRenderTarget.DrawLine(previousPoint, point,
    ...
    unsafe private static bool DrawScopeDX2D(int W, int H, bool bottom) {
        ...
        // PathGeometry sharpGeometry = new
        // PathGeometry(d2dRenderTarget.Factory); GeometrySink geo =
        // sharpGeometry.Open(); geo.BeginFigure(new SharpDX.Vector2(0, startY),
        // FigureBegin.Filled);
        ...
        // for (int i = W - 1; i >= 0; i--)                     // fill point
        // array
        //{
        ...
        // d2dRenderTarget.FillGeometry(sharpGeometry,
        // m_bDX2_data_line_pen_brush); // sometimes when filled is over top of
        // each other, it doesnt show, so do outline as well below
        // d2dRenderTarget.DrawGeometry(sharpGeometry,
        // m_bDX2_data_line_pen_brush);
        ...
        for (int i = 1; i < nDecimatedWidth; i++) {
            ...
            _d2dRenderTarget.DrawLine(
                previousPointMax, pointMax, m_bDX2_waveform_line_pen);
            _d2dRenderTarget.DrawLine(
                previousPointMin, pointMin, m_bDX2_waveform_line_pen);
            _d2dRenderTarget.DrawLine(
                pointMin, pointMax, m_bDX2_waveform_line_pen, m_nDecimation);
    ...
    unsafe private static bool DrawScope2DX2D(int W, int H, bool bottom) {
        ...
        for (int x = 0; x < nDecimatedWidth; x++) {
            int i = x; // (int)(x * xScale);// (int)Math.Truncate((float)x *
                       // xScale);
    ...
    private static PointF lerpPointF(PointF first, PointF second, float by) {
        return new PointF(
            lerp(first.X, second.X, by), lerp(first.Y, second.Y, by));
    }
    ...
    unsafe private static bool DrawPhaseDX2D(int W, int H, bool bottom) {
        ...
        if (!bottom && data_ready) {
            ...
            fixed(void* rptr = &new_display_data[0])
                fixed(void* wptr = &current_display_data[0]) Win32.memcpy(wptr,
                    rptr, /*BUFFER_SIZE*/ (num_points * 2) * sizeof(float));
            data_ready = false;
        } else if (bottom && data_ready_bottom) {
            // get new data
            fixed(void* rptr = &new_display_data_bottom[0]) fixed(
                void* wptr = &current_display_data_bottom[0]) Win32.memcpy(wptr,
                rptr, /*BUFFER_SIZE*/ (num_points * 2) * sizeof(float));
            data_ready_bottom = false;
        }
        ...
        for (int i = 0 /*, j = 0*/; i < num_points;
             i++ /*, j += 8*/) // fill point array
        {
            int x = 0;
            int y = 0;
            if (bottom) {
        ...
        //
        double dCircularMean = Math.Atan2(
            sinSum, cosSum); // dont need /n as circular nature of sin/cos

        float xx = (float)Math.Sin(dCircularMean);
        float yy = (float)Math.Cos(dCircularMean);

        PointF p = lerpPointF(m_dOldCM, new PointF(xx, yy),
            (float)((m_dElapsedFrameStart - m_dLastAngleLerp) / 50f));
    ...
    unsafe static private bool DrawHistogramDX2D(int W, int H) {
        ...
        for (int i = 0; i < nDecimatedWidth; i++) {
            if (points[i].Y < histogram_data[i]) {
                ...
                drawFillRectangleDX2D(m_bDX2_dhp, i * m_nDecimation,
                    histogram_data[i], m_nDecimation, height);
            }
            if (points[i].Y >= avg) // value is below the average
            {
                drawFillRectangleDX2D(m_bDX2_dhp1, points[i].X, points[i].Y,
                    m_nDecimation, H - points[i].Y);
            } else {
                drawFillRectangleDX2D(m_bDX2_dhp1, points[i].X,
                    (int)Math.Floor(avg), m_nDecimation,
                    H - (int)Math.Floor(avg));
                drawFillRectangleDX2D(m_bDX2_dhp2, points[i].X, points[i].Y,
                    m_nDecimation, (int)Math.Floor(avg) - points[i].Y);
            }
        }
    ...
    // private static List<(int pos, double val)> PeakDet(IList<double> vector,
    // double triggerDelta)
    //{
    ...
    private static Dictionary<Color, SharpDX.Direct2D1.Brush> _DX2Brushes
        = null;
    ...
    private static int getSpotLayer(int rx, int leftX) {
        if (rx == 1) {
            ...
            for (int layer = 0; layer < _spotLayerRightRX1.Count; layer++) {
                ...
                if (leftX > layerRightmostX) return layer;
            }
    ...
    public static void drawSpots(
        int rx, int nVerticalShift, int W, bool bottom) {
        if (bottom) return;
        ...
        List<SpotManager2.smSpot> sortedSpots
            = SpotManager2
                  .GetFrequencySortedSpots(); //_spots.OrderBy(o =>
                                              //o.frequencyHZ).ToList();//.Where(f
                                              //=> f.frequencyHZ >= vfoLow &&
                                              //f.frequencyHZ <= vfoHigh &&
                                              //f.Enabled == true).ToList();
        ...
        foreach (SpotManager2.smSpot spot in sortedSpots) {
            ...
            if (layer > -1) {
                ...
                if (spot.frequencyHZ >= vfoLow && spot.frequencyHZ <= vfoHigh) {
                    ...
                    spot.BoundingBoxInPixels[rx - 1].Width
                        = (int)(spot.Size.Width + 2);
                    spot.BoundingBoxInPixels[rx - 1].Height
                        = (int)(spot.Size.Height + 2);
        ...
        SharpDX.Direct2D1.Brush whiteBrush
            = getDXBrushForColour(Color.White, 255);
        SharpDX.Direct2D1.Brush blackBrush
            = getDXBrushForColour(Color.Black, 255);
        SharpDX.Direct2D1.Brush brightBorder
            = getDXBrushForColour(Color.Yellow, 255);

        List<SpotManager2.smSpot> visibleSpots
            = sortedSpots.Where(o => o.Visible[rx - 1]).ToList();
        ...
        foreach (SpotManager2.smSpot spot in visibleSpots) {
            ...
            if (spot.Highlight[rx - 1]) {
                ...
                drawFillRectangleDX2D(
                    spotColour, spot.BoundingBoxInPixels[rx - 1]);
                drawStringDX2D(sDisplayString, fontDX2d_font9, textBrush,
                    spot.BoundingBoxInPixels[rx - 1].X + 1,
                    spot.BoundingBoxInPixels[rx - 1].Y + 1);
            }
        }
        ...
        if (highLightedSpot != null) {
            ...
            Rectangle r = new Rectangle(
                highLightedSpot.BoundingBoxInPixels[rx - 1].X - 2,
                highLightedSpot.BoundingBoxInPixels[rx - 1].Y - 2,
                highLightedSpot.BoundingBoxInPixels[rx - 1].Width + 4,
                highLightedSpot.BoundingBoxInPixels[rx - 1].Height + 4);

            drawFillRectangleDX2D(spotColour, r);
            drawRectangleDX2D(brightBorder, r, 2);
            drawStringDX2D(sDisplayString, fontDX2d_font9, textBrush,
                highLightedSpot.BoundingBoxInPixels[rx - 1].X + 1,
                highLightedSpot.BoundingBoxInPixels[rx - 1].Y + 1);

            if (/*_bShiftKeyDown && */ highLightedSpot.additionalText != "") {
                // show additional text in bubble below
                SizeF additionalTextSize = measureStringDX2D(
                    highLightedSpot.additionalText, fontDX2d_font10);

                int left = (r.X + (r.Width / 2))
                    - (int)(additionalTextSize.Width / 2);
                int top = r.Y + r.Height + 6;

                Rectangle additionalTextRect = new Rectangle(left - 2, top - 2,
                    (int)(additionalTextSize.Width + 4),
                    (int)(additionalTextSize.Height + 4));

                drawFillRectangleDX2D(
                    getDXBrushForColour(Color.LightGray), additionalTextRect);
                drawRectangleDX2D(
                    getDXBrushForColour(Color.Black), additionalTextRect, 2);
                drawStringDX2D(highLightedSpot.additionalText, fontDX2d_font10,
                    getDXBrushForColour(Color.Black), additionalTextRect.X + 2,
                    additionalTextRect.Y + 2);
            }
        }
    }
    private static int getLuminance(Color c) {
        // https://stackoverflow.com/questions/596216/formula-to-determine-perceived-brightness-of-rgb-color
        // return (int)(0.2126 * (float)c.R + 0.7152 * (float)c.G + 0.0722 *
        // (float)c.B);
        int r = rGBtoLin(c.R);
        int g = rGBtoLin(c.G);
        int b = rGBtoLin(c.B);
    ...
    private static int rGBtoLin(int col) {
        ...
        if (colorChannel <= 0.04045) {
            ...
            return (
                int)(Math.Pow(((colorChannel + 0.055) / 1.055), 2.4) * 255f);
        }
    }
    private static void clearAllDynamicBrushes() {
        if (!_bDX2Setup || _DX2Brushes == null) return;

        foreach (KeyValuePair<Color, SharpDX.Direct2D1.Brush> kvp in
                     _DX2Brushes) {
    ...
    private static SharpDX.Direct2D1.Brush getDXBrushForColour(
        Color c, int replaceAlpha = -1) {
        if (!_bDX2Setup) return null;

        if (_DX2Brushes == null)
            _DX2Brushes = new Dictionary<Color, SharpDX.Direct2D1.Brush>();

        Color newC;
        if (replaceAlpha >= 0 && replaceAlpha <= 255)
            newC = Color.FromArgb(
                replaceAlpha, c.R, c.G, c.B); // override the alpha
        else
            newC = c;
...
#region Ukraine
    private static bool _showFlag
        = false; // set to true to always show at startup
    ...
    private static void ukraineFlag() {
        ...
        //        _showFlag = true; // every two hours, replace the flag if
        //        russian
        //}

        if (_showFlag) {
            RectangleF rectDestTop
                = new RectangleF(0, displayTargetHeight - 100, 175, 50);
            RectangleF rectDestBottom
                = new RectangleF(0, displayTargetHeight - 50, 175, 50);

            Color blue = Color.FromArgb(192, 56, 91, 185);
            Color yellow = Color.FromArgb(192, 241, 214, 36);
            _d2dRenderTarget.FillRectangle(
                rectDestTop, getDXBrushForColour(blue));
            _d2dRenderTarget.FillRectangle(
                rectDestBottom, getDXBrushForColour(yellow));
            drawStringDX2D("Stand up for Ukraine", fontDX2d_font9b,
                getDXBrushForColour(Color.White, 192), 30,
                displayTargetHeight - 75 - 10);
            drawStringDX2D("click to hide", fontDX2d_font9b,
                getDXBrushForColour(Color.White, 48), 54,
                displayTargetHeight - 55 - 10);
        }
    }
#endregion
#endregion
