...
#define SAVERESTORE
// #define CWX_DEBUG (Note: Please do not put all Debug.Writeline()under this.
// Leave them commented off.)
    ...
    private void setptt(bool state) {
        ...
        if (setptt_memory != state) {
            if (!console.CWFWKeyer) {
                // CWPTTItem item = new CWPTTItem(state,
                // CWSensorItem.GetCurrentTime()); CWKeyer.PTTEnqueue(item);
            }
...
#region Morse definition, table builders, and help display
    ...
    private void notesButton_Click(object sender, System.EventArgs e) {
        Thread t = new Thread(new ThreadStart(help)) { Name = "help thread",
    ...
    private void build_mbits2() {
        ...
        for (int i = 0; i < 64; i++) // for each code
        {
            ...
            */
            ...
            // a space will have a 4 element count + 1 element space + 2 letter
            // spaces making a total of 7 elements in a word space

            if (i == 0)
                nel = 4; // value for ' ' character
            else
                nel += 2; // add letter space
    ...
    private void load_alpha() {
        if (!File.Exists(
                console.AppDataPath + sfile)) // create default morsedef.txt
        {
#if (CWX_DEBUG)
            MessageBox.Show(sfile + " not found, creating ...");
#endif
            using (StreamWriter sw
                = new StreamWriter(console.AppDataPath + sfile)) {
        ...
        using (
            StreamReader sr = new StreamReader(console.AppDataPath + sfile)) {
            String line;
            String t;
            int nl = 0;
            ...
            // else
            //{
            //     CBMorseUpdateDelegate objDelegate = new
            //     CBMorseUpdateDelegate(CBMorseUpdate); Invoke(objDelegate, new
            //     object[] { 0 });
            // }
        }
    }
    ...
    private void SendBufferMessage() // CAT Read Thread
    {
        ...
        while (true) // do forever
        {
            ...
            if (!stopSending) {
                ...
                {
                    buffer = new byte[1]; // an array to hold one byte of rb
                                          // data
                    rb.Read(buffer, buffer.Length); // read the ringbuffer
                    chr2send = (char)buffer[0];
                    loadchar(chr2send);
                    while (infifo
                        > 2) // number of elements left in the element fifo
                    {
                        Thread.Sleep(2); // wait for the element fifo to catch
                                         // up
                    }
                }
                Thread.Sleep(2);
...
#region startup / shutdown stuff
    public CWX(Console c) {
        ...
        Thread keyFifoThread = new Thread(
            new ThreadStart(keyboardFifo)) { Name = "keyboard fifo pop thread",
            IsBackground = true, // if app closes, kill this thread
            Priority = ThreadPriority.Normal };
        keyFifoThread.Start();

        Thread keyDisplayThread = new Thread(new ThreadStart(
            keyboardDisplay)) { Name = "keyboard edit box handler thread",
            IsBackground = true, // if app closes, kill this thread
            Priority = ThreadPriority.Normal };
        keyDisplayThread.Start();

        Thread CATReadThread = new Thread(
            new ThreadStart(SendBufferMessage)) { Name = "CAT Read Thread",
...
#region Windows Form Designer generated code
    ...
    private void InitializeComponent() {
        ...
        System.ComponentModel.ComponentResourceManager resources
            = new System.ComponentModel.ComponentResourceManager(typeof(CWX));
        ...
        this.toolTip1.SetToolTip(
            this.keyboardLed, " Keyboard active indicator.");
        //
        // chkAlwaysOnTop
        //
        ...
        this.udWPM.Increment = new decimal(new int[] { 1, 0, 0, 0 });
        this.udWPM.Location = new System.Drawing.Point(240, 8);
        this.udWPM.Maximum = new decimal(new int[] { 99, 0, 0, 0 });
        this.udWPM.Minimum = new decimal(new int[] { 1, 0, 0, 0 });
        this.udWPM.Name = "udWPM";
        this.udWPM.Size = new System.Drawing.Size(56, 20);
        this.udWPM.TabIndex = 56;
        this.udWPM.Value = new decimal(new int[] { 22, 0, 0, 0 });
        this.udWPM.ValueChanged
            += new System.EventHandler(this.udWPM_ValueChanged);
        ...
        this.toolTip1.SetToolTip(this.pttdelaylabel,
            "Set delay from PTT to key down in milliseconds.");
        this.pttdelaylabel.Visible = false;
        //
        // udPtt
        //
        this.udPtt.Increment = new decimal(new int[] { 1, 0, 0, 0 });
        this.udPtt.Location = new System.Drawing.Point(456, 8);
        this.udPtt.Maximum = new decimal(new int[] { 2000, 0, 0, 0 });
        this.udPtt.Minimum = new decimal(new int[] { 50, 0, 0, 0 });
        this.udPtt.Name = "udPtt";
        this.udPtt.Size = new System.Drawing.Size(56, 20);
        this.udPtt.TabIndex = 54;
        this.udPtt.Value = new decimal(new int[] { 50, 0, 0, 0 });
        this.udPtt.Visible = false;
        this.udPtt.ValueChanged
            += new System.EventHandler(this.udPtt_ValueChanged);
        ...
        this.expandButton.Click
            += new System.EventHandler(this.expandButton_Click);
        //
        // keyboardButton
        //
        ...
        this.toolTip1.SetToolTip(this.keyboardButton,
            " Enable keyboard.  This must be selected for keyboard to work.");
        this.keyboardButton.Enter
            += new System.EventHandler(this.keyboardButton_Enter);
        this.keyboardButton.KeyPress
            += new System.Windows.Forms.KeyPressEventHandler(
                this.keyboardButton_KeyPress);
        this.keyboardButton.Leave
            += new System.EventHandler(this.keyboardButton_Leave);
        //
        // clearButton
        //
        ...
        this.toolTip1.SetToolTip(
            this.clearButton, " Clear the keyboard buffer.");
        ...
        this.toolTip1.SetToolTip(
            this.chkPause, " Pause keyboard transmission.");
        this.chkPause.CheckedChanged
            += new System.EventHandler(this.chkPause_CheckedChanged);
        //
        // txtdummy1
        //
        ...
        this.txtdummy1.Text
            = "the actual text box will be a graphic here and this one disabled";
        //
        // txt9
        //
        ...
        // txt6
        //
        this.txt6.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
        ...
        // txt5
        //
        this.txt5.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        // txt4
        //
        this.txt4.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        // txt3
        //
        this.txt3.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        // txt2
        //
        this.txt2.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        // txt1
        //
        this.txt1.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        this.toolTip1.SetToolTip(this.keyButton,
            "Turn on transmitter and key it. (60 second timeout)");
        ...
        this.toolTip1.SetToolTip(this.dropdelaylabel,
            " Set break in drop out in milliseconds. Minimum allowed is PTT Delay * 1.5 .");
        this.dropdelaylabel.Visible = false;
        //
        // udDrop
        //
        this.udDrop.Increment = new decimal(new int[] { 1, 0, 0, 0 });
        this.udDrop.Location = new System.Drawing.Point(384, 8);
        this.udDrop.Maximum = new decimal(new int[] { 5000, 0, 0, 0 });
        this.udDrop.Minimum = new decimal(new int[] { 0, 0, 0, 0 });
        this.udDrop.Name = "udDrop";
        this.udDrop.Size = new System.Drawing.Size(56, 20);
        this.udDrop.TabIndex = 35;
        this.udDrop.Value = new decimal(new int[] { 300, 0, 0, 0 });
        this.udDrop.Visible = false;
        this.udDrop.ValueChanged
            += new System.EventHandler(this.udDrop_ValueChanged);
        ...
        this.s9.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s9_MouseDown);
        //
        // s8
        //
        this.s8.Image = null;
        ...
        this.s8.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s8_MouseDown);
        //
        // s7
        //
        this.s7.Image = null;
        ...
        this.s7.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s7_MouseDown);
        //
        // label6
        //
        ...
        this.toolTip1.SetToolTip(
            this.repeatdelayLabel, " Set repeat message delay in seconds.");
        //
        // udDelay
        //
        this.udDelay.Increment = new decimal(new int[] { 1, 0, 0, 0 });
        this.udDelay.Location = new System.Drawing.Point(312, 8);
        this.udDelay.Maximum = new decimal(new int[] { 3600, 0, 0, 0 });
        this.udDelay.Minimum = new decimal(new int[] { 0, 0, 0, 0 });
        this.udDelay.Name = "udDelay";
        this.udDelay.Size = new System.Drawing.Size(56, 20);
        this.udDelay.TabIndex = 20;
        this.udDelay.Value = new decimal(new int[] { 3, 0, 0, 0 });
        this.udDelay.ValueChanged
            += new System.EventHandler(this.udDelay_ValueChanged);
        this.udDelay.LostFocus
            += new System.EventHandler(this.udDelay_LostFocus);
        //
        // cbMorse
        //
        this.cbMorse.Cursor = System.Windows.Forms.Cursors.Default;
        this.cbMorse.DropDownStyle
            = System.Windows.Forms.ComboBoxStyle.DropDownList;
        this.cbMorse.DropDownWidth = 208;
        this.cbMorse.Font = new System.Drawing.Font("Courier New", 9F,
            System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point,
            ((byte)(0)));
        ...
        this.toolTip1.SetToolTip(this.cbMorse,
            "View and right click to edit Morse definition table.");
        this.cbMorse.SelectedIndexChanged
            += new System.EventHandler(this.CbMorse_SelectedIndexChanged);
        this.cbMorse.MouseDown += new System.Windows.Forms.MouseEventHandler(
            this.cbMorse_MouseDown);
        //
        // notesButton
        //
        ...
        this.notesButton.Click
            += new System.EventHandler(this.notesButton_Click);
        //
        // speedLabel
        //
        ...
        this.toolTip1.SetToolTip(this.speedLabel,
            " Set memory keyer (not paddle) speed in words per minute. (PARIS method)");
        //
        // s6
        //
        this.s6.Image = null;
        ...
        this.s6.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s6_MouseDown);
        //
        // s5
        //
        this.s5.Image = null;
        ...
        this.s5.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s5_MouseDown);
        //
        // s4
        //
        this.s4.Image = null;
        ...
        this.s4.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s4_MouseDown);
        //
        // s3
        //
        this.s3.Image = null;
        ...
        this.s3.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s3_MouseDown);
        //
        // s2
        //
        this.s2.Image = null;
        ...
        this.s2.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s2_MouseDown);
        //
        // s1
        //
        this.s1.Image = null;
        ...
        this.s1.MouseDown
            += new System.Windows.Forms.MouseEventHandler(this.s1_MouseDown);
        //
        // CWX
        //
        ...
        this.Closing
            += new System.ComponentModel.CancelEventHandler(this.CWX_Closing);
        this.Load += new System.EventHandler(this.CWX_Load);
        this.Paint
            += new System.Windows.Forms.PaintEventHandler(this.CWX_Paint);
        this.KeyDown
            += new System.Windows.Forms.KeyEventHandler(this.CWX_KeyDown_1);
        this.KeyUp
            += new System.Windows.Forms.KeyEventHandler(this.CWX_KeyUp_1);
        this.MouseMove
            += new System.Windows.Forms.MouseEventHandler(this.CWX_MouseMove);
...
#region event handlers and callbacks
    ...
    private void CWX_KeyUp_1(
        object sender, System.Windows.Forms.KeyEventArgs e) {

        kkk++;
        label6.Text = kkk.ToString() + " " + e.KeyCode.ToString() + " "
            + e.KeyData.ToString() + " " + e.KeyValue.ToString("x");
    ...
    private void CWX_KeyDown_1(
        object sender, System.Windows.Forms.KeyEventArgs e) {

        char key = (char)e.KeyValue;

        label5.Text = "KeyDown " + key + " " + e.KeyCode.ToString() + " "
            + e.KeyData.ToString() + " " + e.KeyValue.ToString("x");
        ...
        else if (e.KeyCode.ToString().Equals("Menu"))
            altkey = true;

        if (altkey) { // Alt 1 thru 9 load messages 1-9 into the keyboard buffer
    ...
    private void keyboardButton_KeyPress(
    ...
    private void keyButton_Click(
        object sender, System.EventArgs e) // the 'Key' button
    {
        ...
        if (console.RX1DSPMode != DSPMode.CWL
            ...
            MessageBox.Show(
                "Console is not in CW mode.  Please switch to either CWL or CWU and try again.",
    ...
    private void s1_Click(object sender, System.EventArgs e) { queue_start(1); }

    private void s2_Click(object sender, System.EventArgs e) { queue_start(2); }

    private void s3_Click(object sender, System.EventArgs e) { queue_start(3); }

    private void s4_Click(object sender, System.EventArgs e) { queue_start(4); }

    private void s5_Click(object sender, System.EventArgs e) { queue_start(5); }

    private void s6_Click(object sender, System.EventArgs e) { queue_start(6); }

    private void s7_Click(object sender, System.EventArgs e) { queue_start(7); }

    private void s8_Click(object sender, System.EventArgs e) { queue_start(8); }

    private void s9_Click(object sender, System.EventArgs e) { queue_start(9); }

    private void cbMorse_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) editit();
    }

    private void s1_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(1);
    }
    private void s2_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(2);
    }
    private void s3_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(3);
    }
    private void s4_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(4);
    }
    private void s5_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(5);
    }
    private void s6_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(6);
    }
    private void s7_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(7);
    }
    private void s8_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
        if (e.Button.Equals(MouseButtons.Right)) msg2keys(8);
    }
    private void s9_MouseDown(
        object sender, System.Windows.Forms.MouseEventArgs e) {
    ...
    private void chkAlwaysOnTop_CheckedChanged(
    ...
    private void show_keys(Graphics formGraphics = null) {
        ...
        lock (m_objLock) {
            ...
            if (formGraphics == null)
                formGraphics = this.CreateGraphics(); // MW0LGE

            System.Drawing.Font drawFont
                = new System.Drawing.Font("Courier New", 14, FontStyle.Bold);
            System.Drawing.SolidBrush drawBrush
                = new System.Drawing.SolidBrush(System.Drawing.Color.Black);
            System.Drawing.SolidBrush grayBrush
            ...
            formGraphics.FillRectangle(
                whiteBrush, new Rectangle(kylx, kyty, kyxsz + 1, kyysz + 1));
            ...
            for (i = 0; i < NKEYS; i++) {
                ...
                formGraphics.DrawString(
                    s, drawFont, grayBrush, (float)x, (float)y);
                if ((i % NKPL) == (NKPL - 1)) {
                    x = kylx;
                    y += dy;
                } else
            ...
            for (i = 0; i < NKEYS; i++) {
                ...
                formGraphics.DrawString(
                    s, drawFont, drawBrush, (float)x, (float)y);
                if ((i % NKPL) == (NKPL - 1)) {
                    x = kylx;
                    y += dy;
                } else
...
#region Morse code definition editor interface
    ...
    private void editit() {
        ...
        if (editline[5] == '*') //.Substring(5,1).Equals("*"))
        {
            ...
            if (editline.Length == 26)
                ...
                MessageBox.Show(
                    "Edited line has invalid length and is not saved.");
            }
        }
    }

    private void insert_and_reload(
        string s) // replace line s in a2m2 with s then re-write file
    {
        int id;
#if true
    ...
    private void write_a2m2() {
        ...
        using (StreamWriter sw = new StreamWriter(
                   console.AppDataPath + sfile)) // and in with the new
        {
            for (int i = 0; i < 64; i++) {
                sw.WriteLine(a2m2[i]);
            }
...
#region where all the fun work is done
        ...
        while (true) // used while for control; only once ever thru
        {
            ...
            if ((data == EL_KEYDOWN) || (data == EL_KEYUP)) // key command
            {
                if (data == EL_KEYDOWN) // key down?
                {
                    ...
                    ttx = ttdel / tel;
                    if (newptt > 0)
                        return; // the key will get pressed after newptt
                    setkey(true);
                } else {
                    setkey(false);
                    break;
    ...
    private void keyboardFifo() // thread to watch the keyboard fifo
    {
        ...
        while (!stopThreads) {
            if (infifo2 > 0) {
                ...
                Thread.Sleep(20); // this was originally 10 ms
        }
    }

    // the keyboardDisplay() thread pulls keys from the left hand edge (top)
    // of the keyboard display and stuffs them into fifo2 then causes the
    // display to be updated.  The keys are put into the display buffer by the
    // keystroke event handler.

    private void
    keyboardDisplay() // watch and maintain the the keyboard display
    {
        char topkey;
        int i;
    ...
    { // this is the guts of loadmsg and work much the same way
        ...
        ic &= 0x3f; // isolate the 0-63 code  (we did that in the previous
                    // line>)
    ...
    private void loadmsg(string t) // load string t to the element fifo
    {
        ...
        while (nc > 0) {
            ...
            } else if (ic == 4) // a long space
            {
                ...
                v = (uint)mbits.GetValue(
                    ic); // fetch Morse bit pattern for the character
            }
    ...
    private void process_key(
        char key) // keys from keyboardButton_KeyPress event
    {
#if (CWX_DEBUG)
