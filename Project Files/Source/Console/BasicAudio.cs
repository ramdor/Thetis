...
public class BasicAudio {
    ...
    public BasicAudio() {
        ...
        m_objPlayer.LoadCompleted
            += new AsyncCompletedEventHandler(player_LoadCompleted);
        m_objPlayer.SoundLocationChanged
            += new EventHandler(player_LocationChanged);
    }
    ...
    public void Play() {
        ...
        if (m_objThread == null || !m_objThread.IsAlive) {
            // even though m_objPlayer.Play() is async for some reason perhaps
            // on initial play there would be noticabled glitch in specturm.
            // Starting it on this thread seems to reduce the occurance
    ...
    public void Stop() { m_objPlayer.Stop(); }
