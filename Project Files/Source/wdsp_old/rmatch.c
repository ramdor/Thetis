...
PORT void getRMatchDiags(void* b, int* underflows, int* overflows, double* var,
...
PORT void* create_rmatchV(int in_size, int out_size, int nom_inrate,
...
PORT void setRMatchFeedbackGain(void* b, double feedback_gain) {
    RMATCH a = (RMATCH)b;
    EnterCriticalSection(&a->cs_var);
...
PORT void setRMatchSlewTime(void* b, double slew_time) {
    RMATCH a = (RMATCH)b;
    InterlockedBitTestAndReset(
        &a->run, 0); // turn OFF new data coming into the rmatch
    Sleep(10); // wait for processing to cease
    decalc_rmatch(a); // deallocate all memory EXCEPT the data structure holding
...
PORT void setRMatchSlewTime1(void* b, double slew_time) {
    RMATCH a = (RMATCH)b;
    double theta, dtheta;
    int m;
    InterlockedBitTestAndReset(&a->run, 0);
...
PORT void setRMatchPropRingMin(void* ptr, int prop_min) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchPropRingMax(void* ptr, int prop_max) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchFFRingMin(void* ptr, int ff_ringmin) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchFFRingMax(void* ptr, int ff_ringmax) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    decalc_rmatch(a);
...
PORT void setRMatchFFAlpha(void* ptr, double ff_alpha) {
    RMATCH a = (RMATCH)ptr;
    InterlockedBitTestAndReset(&a->run, 0);
    Sleep(10);
    a->ff_alpha = ff_alpha;
...
PORT void getControlFlag(void* ptr, int* control_flag) {
    RMATCH a = (RMATCH)ptr;
    EnterCriticalSection(&a->cs_ring);
