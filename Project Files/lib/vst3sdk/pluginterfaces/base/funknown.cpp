...
void FUID::toString (char8* string) const
{
	...
	sprintf_s (string, 33, "%08X%04X%04X%s", g->Data1, g->Data2, g->Data3, s);
#else
...
bool FUID::fromString (const char8* string)
{
	...
	s[8] = 0;
	sscanf_s (s, "%x", &g.Data1);
	strcpy_s (s, sizeof (s), string + 8);
	s[4] = 0;
	sscanf_s (s, "%hx", &g.Data2);
	strcpy_s (s, sizeof (s), string + 12);
	s[4] = 0;
	sscanf_s (s, "%hx", &g.Data3);
...
// e.g. {c200e360-38c5-11ce-ae62-08002b2b79ef}
	...
	strncpy_s (s, sizeof (s), string + 1, 8);
	sscanf_s (s, "%x", &g.Data1);
	strncpy_s (s, sizeof (s), string + 10, 4);
	sscanf_s (s, "%hx", &g.Data2);
	strncpy_s (s, sizeof (s), string + 15, 4);
	sscanf_s (s, "%hx", &g.Data3);
	memcpy (data, &g, 8);
...
// e.g. {c200e360-38c5-11ce-ae62-08002b2b79ef}
	...
	sprintf_s (string, 39, "{%08X-%04X-%04X-%s-%s}", g->Data1, g->Data2, g->Data3, s1, s2);
#else
	char8 s1[9];
...
{
	*string = 0;
	char8* write_ptr = string;
	for (int32 i = i1; i < i2; i++)
	{
		snprintf (write_ptr, 3, "%02X", (uint8)data[i]);
		write_ptr += 2;
	}
}
