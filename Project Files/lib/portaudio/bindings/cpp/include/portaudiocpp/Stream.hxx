...
/// destruction (i.e. by calling their own close() method in their
/// deconstructor). Following good C++ programming practices, care must be taken
/// to ensure no exceptions are thrown by the deconstructor of these classes. As
/// a consequence, clients need to explicitly call close() to ensure the stream
/// closed successfully.
///
/// The Stream object can be used to manipulate the Stream's state. Also,
/// time-constant and time-varying information about the Stream can be
/// retrieved.
//////
class Stream {
    public:
