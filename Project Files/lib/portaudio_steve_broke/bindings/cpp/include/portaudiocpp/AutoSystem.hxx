...
///
/// To avoid having to type portaudio::System::instance().xyz() all the time,
...
class AutoSystem {
    ...
    AutoSystem(bool initialize = true) {
        if (initialize) System::initialize();
    }

    ~AutoSystem() {
        if (System::exists()) System::terminate();
    }

    void initialize() { System::initialize(); }

    void terminate() { System::terminate(); }
};
