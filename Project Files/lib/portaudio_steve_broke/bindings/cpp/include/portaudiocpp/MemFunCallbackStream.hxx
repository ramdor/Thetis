...
//////
/// @brief Callback stream using a class's member function as a callback.
/// Template argument T is the type of the class of which a member function is
/// going to be used.
///
/// Example usage:
/// @verbatim MemFunCallback<MyClass> stream = MemFunCallbackStream(parameters,
/// *this, &MyClass::myCallbackFunction); @endverbatim
//////
template <typename T> class MemFunCallbackStream : public CallbackStream {
    public:
    typedef int (T::*CallbackFunPtr)(const void*, void*, unsigned long,
    ...
    MemFunCallbackStream(
        const StreamParameters& parameters, T& instance, CallbackFunPtr memFun)
        : adapter_(instance, memFun) {
        open(parameters);
    }

    ~MemFunCallbackStream() { CallbackStream::close(); }

    void open(const StreamParameters& parameters, T& instance,
    ...
    MemFunCallbackStream& operator=(
        const MemFunCallbackStream&); // non-copyable

    //////
    /// @brief Inner class which adapts a member function callback to a
    /// CallbackInterface compliant class (so it can be adapted using the
    /// paCallbackAdapter function).
    //////
    class MemFunToCallbackInterfaceAdapter : public CallbackInterface {
        ...
        MemFunToCallbackInterfaceAdapter(T& instance, CallbackFunPtr memFun)
            : instance_(&instance), memFun_(memFun) {}
