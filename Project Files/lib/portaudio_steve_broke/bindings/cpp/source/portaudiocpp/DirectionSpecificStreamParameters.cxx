...
//////
/// Returns a `nil' DirectionSpecificStreamParameters object. This can be used
/// to specify that one direction of a Stream is not required (i.e. when
/// creating a half-duplex Stream). All fields of the null
/// DirectionSpecificStreamParameters object are invalid except for the device
/// and the number of channel, which are set to paNoDevice and 0 respectively.
//////
...
//////
DirectionSpecificStreamParameters::DirectionSpecificStreamParameters(
    const Device& device, int numChannels, SampleDataFormat format,
    bool interleaved, PaTime suggestedLatency,
...
void DirectionSpecificStreamParameters::setSampleFormat(
    SampleDataFormat format, bool interleaved) {
    paStreamParameters_.sampleFormat = static_cast<PaSampleFormat>(format);

    if (!interleaved) paStreamParameters_.sampleFormat |= paNonInterleaved;
}

void DirectionSpecificStreamParameters::setHostApiSpecificSampleFormat(
    PaSampleFormat format, bool interleaved) {
...
void DirectionSpecificStreamParameters::setHostApiSpecificStreamInfo(
    void* streamInfo) {
...
PaStreamParameters* DirectionSpecificStreamParameters::paStreamParameters() {
    if (paStreamParameters_.channelCount > 0
        && paStreamParameters_.device != paNoDevice)
...
SampleDataFormat DirectionSpecificStreamParameters::sampleFormat() const {
    if (isSampleFormatHostApiSpecific())
        return portaudio::SampleDataFormat::INVALID_FORMAT;
    else
        return static_cast<SampleDataFormat>(
...
/// paCustomFormat or paNonInterleaved flags. Will return 0 if the sample format
/// is not host api-specific.
//////
PaSampleFormat
DirectionSpecificStreamParameters::hostApiSpecificSampleFormat() const {
    if (isSampleFormatHostApiSpecific())
        return paStreamParameters_.sampleFormat & ~paCustomFormat
            & ~paNonInterleaved;
    else
        return 0;
}
