...
//////
StreamParameters::StreamParameters() = default;

//////
/// Sets up the all parameters needed to open either a half-duplex or
/// full-duplex Stream.
///
/// @param inputParameters The parameters for the input direction of the to-be
/// opened Stream or DirectionSpecificStreamParameters::null() for an
/// output-only Stream.
/// @param outputParameters The parameters for the output direction of the to-be
/// opened Stream or DirectionSpecificStreamParameters::null() for an input-only
/// Stream.
/// @param sampleRate The to-be opened Stream's sample rate in Hz.
/// @param framesPerBuffer The number of frames per buffer for a CallbackStream,
/// or the preferred buffer granularity for a BlockingStream.
...
StreamParameters::StreamParameters(
    ...
    double sampleRate, unsigned long framesPerBuffer, PaStreamFlags flags)
...
//////
/// Sets the requested sample rate. If this sample rate isn't supported by the
/// hardware, the Stream will fail to open. The real-life sample rate used might
/// differ slightly due to imperfections in the sound card hardware; use
/// Stream::sampleRate() to retrieve the best known estimate for this value.
//////
...
//////
/// Sets the specified flag or does nothing when the flag is already set.
/// Doesn't `unset' any previously existing flags (use clearFlags() for that).
//////
...
//////
/// Unsets the specified flag or does nothing if the flag isn't set. Doesn't
...
void StreamParameters::setInputParameters(
    const DirectionSpecificStreamParameters& parameters) {
    inputParameters_ = parameters;
}

void StreamParameters::setOutputParameters(
    const DirectionSpecificStreamParameters& parameters) {
...
bool StreamParameters::isSupported() const {
    return (Pa_IsFormatSupported(inputParameters_.paStreamParameters(),
                outputParameters_.paStreamParameters(), sampleRate_)
        == paFormatIsSupported);
}
