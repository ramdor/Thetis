...
class SineGenerator {
    ...
    SineGenerator(int tableSize)
        ...
        for (int i = 0; i < tableSize; ++i) {
            table_[i] = 0.125f
                * (float)sin(((double)i / (double)tableSize) * PI * 2.);
        }
    }
    ...
    int generate(const void* inputBuffer, void* outputBuffer,
        ...
        for (unsigned int i = 0; i < framesPerBuffer; ++i) {
            ...
            leftPhase_ += 1;
            if (leftPhase_ >= tableSize_) leftPhase_ -= tableSize_;

            rightPhase_ += 3;
            if (rightPhase_ >= tableSize_) rightPhase_ -= tableSize_;
        }
...
int sine_main() {
    try {
        ...
        portaudio::DirectionSpecificStreamParameters outParams(
            sys.defaultOutputDevice(), 2, portaudio::SampleDataFormat::FLOAT32,
            false, sys.defaultOutputDevice().defaultLowOutputLatency(), NULL);
